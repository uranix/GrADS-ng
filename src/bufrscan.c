...
enum bufrscan_modes { HEADER_MODE = 0, DATA_MODE };
  ...
  for (msg = dset->msgs; msg != NULL; msg = msg->next) {
    ...
    for (i = 0; i < msg->subcnt; i++) {
      ...
      for (val = msg->subs[i]; val != NULL; val = val->next) {
        printf("%d (%d) [%.3d]    0-%.2d-%.3d     ", msg->fileindex, i, val->z, val->x, val->y);
        if (val->sval) {
          printf("[%s]", val->sval);
        } else {
          if (val->undef) {
        ...
        if (gabufr_valid_varid(0, val->x, val->y) && varinf == gabufr_get_varinf(val->x, val->y)) {
