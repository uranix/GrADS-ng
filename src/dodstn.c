...
gaint dodgvar(char *name1, char *name2, struct gafile *pfi) {
  ...
  if (var < 0)
    var = gadap_d_varindex(pfi->dhandle, name2);
  if (var < 0)
    var = -999;
  return var;
}
...
gaint isepic(GADAP_DATASET handle) { return gadap_d_varindex(handle, "_id") >= 0; }
...
struct dt epict2dt(gadouble val) {
  ...
  ltim = tim;
  if (ltim < 0)
    ltim = (-1) * ltim; /* absolute value of minutes since 01-01-1970 */
...
gaint dappfi(char *url, struct gafile *pfi) {
  ...
  if ((tminid >= 0) && (tsizeid >= 0) && (tstepid >= 0)) {
    ...
    if ((pos = intprs((char *)name, &(pfi->dnum[3]))) == NULL)
      goto tdeferr;
    ...
    tdef.mo = -1000;
    tdef.dy = -1000;
    if ((pos = adtprs((char *)name, &tdef, &dt1)) == NULL)
      goto tdeferr;
    if (dt1.yr == -1000 || dt1.mo == -1000.0 || dt1.dy == -1000)
      goto tdeferr;

    name = gadap_d_attrstr(handle, tvar, tstepid);
    if ((pos = rdtprs((char *)name, &dt2)) == NULL)
      goto tdeferr;
    v1 = (gadouble)((dt2.yr * 12) + dt2.mo);
    v2 = (gadouble)((dt2.dy * 1440) + (dt2.hr * 60) + dt2.mn);
    if (dequal(v1, 0.0, 1e-08) == 0 && dequal(v2, 0.0, 1e-08) == 0)
      goto tdeferr;
