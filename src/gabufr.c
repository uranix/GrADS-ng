...
gaint gabufr_load2mem(const char *path, gabufr_dset *dset) {
  ...
  while (bytesleft > 0) {
    ...
    if (feof(datafile) && bytesleft) {
      if (GABUFR_DEBUG)
        printf("Ran out of data in BUFR file %s!\n", path);
      free(filebuf);
      fclose(datafile);
      return GABUFR_ERR;
    }
...
void gabufr_print_varid(gabufr_varid *varid, gaint indent) {
  ...
  printf("%d %.2d %.3d  ", varid->f, varid->x, varid->y);
  ...
  case 0:
    ...
    printf("(%s) %s\n", (varinf->datatype == GABUFR_STR_TYPE) ? "text" : "numeric", varinf->description);
    break;
  ...
  case 3:
    printf("(sequence)\n");
    for (seq_varid = gabufr_get_seq(varid->x, varid->y); seq_varid; seq_varid = seq_varid->next) {
...
gaint gabufr_all_ones(gaint bitcnt) { return (1 << bitcnt) - 1; }
...
char *gabufr_readbits2str(unsigned char *pos, gaint offset, gaint bitcnt) {
  ...
  if (GABUFR_DEBUG)
    printf("read %2d chars at (%p + %d): [", bytecnt, pos, offset);
...
gaint gabufr_parseval(gabufr_parseinf *inf, gaint x, gaint y, gabufr_val *val) {
  ...
  if (varinf->datatype == GABUFR_STR_TYPE) {
    val->sval = gabufr_readbits2str(inf->datapos, inf->databits, varinf->width);
    val->val = DBL_MIN;
  } else {
    packedval = (double)gabufr_readbits2num(inf->datapos, inf->databits, varinf->width);

    if (packedval == gabufr_all_ones(varinf->width)) {
      val->undef = GABUFR_UNDEF;
      if (GABUFR_DEBUG)
        printf("missing data flag set\n");
    } else {
    ...
    if (GABUFR_DEBUG)
      printf("unpacking: ( %d + %d ) / 10^%d -> %g\n", (gaint)packedval, varinf->offset, varinf->scale, val->val);
    val->sval = NULL;
  }
...
gaint gabufr_parselist(gabufr_parseinf *inf, gabufr_varid *list) {
  gabufr_varid *saved;
  saved = inf->descpos;
  for (inf->descpos = list; inf->descpos; inf->descpos = inf->descpos->next) {
    if (gabufr_parsedesc(inf, inf->descpos->f, inf->descpos->x, inf->descpos->y) == GABUFR_ERR) {
      return GABUFR_ERR;
    }
  }
  inf->descpos = saved;
...
gaint gabufr_replicate(gabufr_parseinf *inf, gaint numdesc, gaint numreps) {
  ...
  if (GABUFR_DEBUG)
    printf("**** replicating %d descriptors %d times\n", numdesc, numreps);
  ...
  if (inf->descpos->next) {
    if (GABUFR_DEBUG)
      printf("using descpos (currently %p)\n", inf->descpos);
    pos_addr = &inf->descpos;
  } else {
    if (GABUFR_DEBUG)
      printf("using parentpos (currently %p)\n", inf->descpos);
  ...
  if (nestedrep) {
    ...
    if (GABUFR_DEBUG)
      printf("warning: nested replication in dataset\n");
  }
  ...
  for (z = 0; z < numreps; z++) {
    ...
    while ((*pos_addr) != end) {
      ...
      if (GABUFR_DEBUG)
        printf("descpos=%d-%d-%d\n", (*pos_addr)->f, (*pos_addr)->x, (*pos_addr)->y);
      if (gabufr_parsedesc(inf, (*pos_addr)->f, (*pos_addr)->x, (*pos_addr)->y) == GABUFR_ERR) {
        return GABUFR_ERR;
      }
    }
  }
  ...
  if (GABUFR_DEBUG)
    printf("**** done replicating %d descriptors %d times\n\n", numdesc, numreps);
  return GABUFR_OK;
}
...
gaint gabufr_parse_f2(gabufr_parseinf *inf, gaint x, gaint y) {
  ...
  case 4: /* associated field */
    if (GABUFR_DEBUG)
      printf("reading %d-bit associated field\n", y);
    val = (gabufr_val *)malloc(sizeof(gabufr_val));
    if (val == NULL) {
      printf("Memory allocation failed during parsing\n");
      return GABUFR_ERR;
  ...
  case 5: /* associated string */
    if (GABUFR_DEBUG)
      printf("reading %d-byte associated string\n", y);
    val = (gabufr_val *)malloc(sizeof(gabufr_val));
    if (val == NULL) {
      printf("Memory allocation failed during parsing\n");
      return GABUFR_ERR;
...
gaint gabufr_parse_f3(gabufr_parseinf *inf, gaint x, gaint y) {
  ...
  */
  ...
  for (inf->descpos = gabufr_get_seq(x, y); inf->descpos; inf->descpos = inf->descpos->next) {
    if (gabufr_parsedesc(inf, inf->descpos->f, inf->descpos->x, inf->descpos->y) == GABUFR_ERR) {
      return GABUFR_ERR;
    }
  }
  if (GABUFR_DEBUG)
    printf("==== finished F=3 X=%d Y=%d\n\n", x, y);
...
gaint gabufr_parsevals(gabufr_msg *msg, gaint parse_data) {
  ...
  if (parse_data == GABUFR_PARSE_DATA || msg->is_new_tbl) {
    ...
    if (GABUFR_DEBUG)
      printf("data position is (%p + %d); end of data is %p\n", inf.datapos, inf.databits, inf.msg->end);
...
gabufr_msg *gabufr_parsehdr(unsigned char *section0) {
  ...
  if (GABUFR_DEBUG)
    printf("sections: %p / %p / %p / %p (end %p)\n", msg->section0, msg->section1, msg->section3, msg->section4, msg->end);

  if (GABUFR_DEBUG)
    printf("lengths: %ld / %ld / %ld / %ld (total %ld)\n", (galint)(msg->section1 - msg->section0),
           (galint)(msg->section3 - msg->section1), (galint)(msg->section4 - msg->section3), (galint)(msg->end - msg->section4),
  ...
  if (GABUFR_DEBUG)
    printf("edition: %d; master #: %d; master v: %d; local v: %d\n", msg->tbl_inf.bufr_edition, msg->tbl_inf.master_tbl_num,
           msg->tbl_inf.master_tbl_version, msg->tbl_inf.local_tbl_version);
  ...
  */
  ...
  if (GABUFR_DEBUG)
    printf("date: %.2d:%.2d %.2d-%.2d-%.2d   subsets: %d\n", msg->hour, msg->min, msg->month, msg->day, msg->year, msg->subcnt);
...
gaint gabufr_decode(gabufr_dset *dset, gaint parse_data) {
  ...
  while (pos < end) {
    ...
    if (memcmp(pos, "BUFR", 4) == 0) {
      ...
      msglen = gagby(pos, 4, 3);
      if (GABUFR_DEBUG)
        printf("\n\n\n\nFound 'BUFR' at %p followed by length %d; ", pos, msglen);
      endofmsg = (pos + msglen) - 4;

      if (memcmp(endofmsg, "7777", 4) == 0) {

        if (GABUFR_DEBUG)
          printf("confirmed end of message.\n");
        ...
        if (next) {
          ...
          if (next->is_new_tbl) {
            if (GABUFR_DEBUG)
              printf("msg %d contains a new BUFR table\n", dset->msgcnt);
          } else {
            if (GABUFR_DEBUG)
              printf("msg %d contains data\n", dset->msgcnt);
          }
        ...
        if (GABUFR_DEBUG)
          printf("no end of message! got %c%c%c%c instead\n", *endofmsg, *(endofmsg + 1), *(endofmsg + 2), *(endofmsg + 3));
      }
    }
