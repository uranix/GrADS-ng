...
gaint gaddes(char *name, struct gafile *pfi, gaint mflag) {
  ...
  static char *errs[9] = {"XDEF", "YDEF", "ZDEF", "TDEF", "UNDEF", "DSET", "VARS", "TITLE", "DTYPE"};
    ...
    if (!isalnum(mrec[0])) {
      ...
      if ((ch = nxtwrd(mrec)) == NULL) {
        ...
        while (*(ch + len) != ' ' && *(ch + len) != '\n' && *(ch + len) != '\t')
          len++;
        sz = len + 3;
        if ((pfi->scattr = (char *)galloc(sz, "scattr")) == NULL)
          goto err8;
        for (i = 0; i < len; i++)
          *(pfi->scattr + i) = *(ch + i);
        *(pfi->scattr + len) = '\0';
        havesf = 1;
        if (!strncmp(pfi->scattr, "NULL", 4) || !strncmp(pfi->scattr, "null", 4))
          havesf = 0;
        ...
        if ((ch = nxtwrd(ch)) == NULL) {
          ...
          while (*(ch + len) != ' ' && *(ch + len) != '\n' && *(ch + len) != '\t')
            len++;
          sz = len + 3;
          if ((pfi->ofattr = (char *)galloc(sz, "ofattr")) == NULL)
            goto err8;
          for (i = 0; i < len; i++)
            *(pfi->ofattr + i) = *(ch + i);
          *(pfi->ofattr + len) = '\0';
          haveao = 1;
          if (!strncmp(pfi->ofattr, "NULL", 4) || !strncmp(pfi->ofattr, "null", 4))
            haveao = 0;
        }
      ...
      while (1) {
        ...
        len = wrdlen(ch);
        sz = len + 1;
        if ((pchsub->ch = (char *)galloc(sz, "chsubstr")) == NULL)
          goto err8;
      ...
      if (cmpwrd("station", ch)) {
        ...
        if ((pfi->bufrinfo = (struct bufrinfo *)galloc(sz, "bufrinfo")) == NULL)
          goto err8;
...
#if GRIB2
      else if (cmpwrd("grib2", ch))
        pfi->idxflg = 2;
#endif
#if USENETCDF
      else if (cmpwrd("netcdf", ch))
        pfi->ncflg = 1;
#endif
#if USEHDF
      else if (cmpwrd("hdfsds", ch) || cmpwrd("hdf4", ch))
        pfi->ncflg = 2;
#endif
#if USEHDF5
      else if (cmpwrd("hdf5_grid", ch))
        pfi->ncflg = 3;
#endif
      else {
        gaprnt(0, "Open Error:  Data file type invalid\n");
      ...
      if ((ch = nxtwrd(rec)) == NULL) {
        ...
        j = 0;
        while (1) {
          if ((ch = intprs(ch, &(pfi->bufrinfo->lonxy[j]))) == NULL)
            goto err6a;
          while (*ch == ' ')
            ch++;
          if (*ch != ',')
        ...
        if (pfi->bufrinfo->lonxy[0] == -999 || pfi->bufrinfo->lonxy[1] == -999)
          goto err6a;
      }
      ...
      if ((ch = nxtwrd(rec)) == NULL) {
        ...
        j = 0;
        while (1) {
          if ((ch = intprs(ch, &(pfi->bufrinfo->latxy[j]))) == NULL)
            goto err6a;
          while (*ch == ' ')
            ch++;
          if (*ch != ',')
        ...
        if (pfi->bufrinfo->latxy[0] == -999 || pfi->bufrinfo->latxy[1] == -999)
          goto err6a;
      }
      ...
      if ((ch = nxtwrd(rec)) == NULL) {
        ...
        j = 0;
        while (1) {
          if ((ch = intprs(ch, &(pfi->bufrinfo->levxy[j]))) == NULL)
            goto err6a;
          while (*ch == ' ')
            ch++;
          if (*ch != ',')
        ...
        if (pfi->bufrinfo->levxy[0] == -999 || pfi->bufrinfo->levxy[1] == -999)
          goto err6a;
      }
        ...
        while (nxtwrd(ch) != NULL) {
          if (cmpwrd("yr", ch)) {
            if ((ch = nxtwrd(ch)) == NULL) {
              ...
              j = 0;
              while (1) {
                if ((ch = intprs(ch, &(pfi->bufrinfo->base.yrxy[j]))) == NULL)
                  goto err6a;
                while (*ch == ' ')
                  ch++;
                if (*ch != ',')
              ...
              if (pfi->bufrinfo->base.yrxy[0] == -999 || pfi->bufrinfo->base.yrxy[1] == -999)
                goto err6a;
            }
            ...
            if ((ch = nxtwrd(ch)) == NULL) {
              ...
              j = 0;
              while (1) {
                if ((ch = intprs(ch, &(pfi->bufrinfo->base.moxy[j]))) == NULL)
                  goto err6a;
                while (*ch == ' ')
                  ch++;
                if (*ch != ',')
              ...
              if (pfi->bufrinfo->base.moxy[0] == -999 || pfi->bufrinfo->base.moxy[1] == -999)
                goto err6a;
            }
            ...
            if ((ch = nxtwrd(ch)) == NULL) {
              ...
              j = 0;
              while (1) {
                if ((ch = intprs(ch, &(pfi->bufrinfo->base.dyxy[j]))) == NULL)
                  goto err6a;
                while (*ch == ' ')
                  ch++;
                if (*ch != ',')
              ...
              if (pfi->bufrinfo->base.dyxy[0] == -999 || pfi->bufrinfo->base.dyxy[1] == -999)
                goto err6a;
            }
            ...
            if ((ch = nxtwrd(ch)) == NULL) {
              ...
              j = 0;
              while (1) {
                if ((ch = intprs(ch, &(pfi->bufrinfo->base.hrxy[j]))) == NULL)
                  goto err6a;
                while (*ch == ' ')
                  ch++;
                if (*ch != ',')
              ...
              if (pfi->bufrinfo->base.hrxy[0] == -999 || pfi->bufrinfo->base.hrxy[1] == -999)
                goto err6a;
            }
            ...
            if ((ch = nxtwrd(ch)) == NULL) {
              ...
              j = 0;
              while (1) {
                if ((ch = intprs(ch, &(pfi->bufrinfo->base.mnxy[j]))) == NULL)
                  goto err6a;
                while (*ch == ' ')
                  ch++;
                if (*ch != ',')
              ...
              if (pfi->bufrinfo->base.mnxy[0] == -999 || pfi->bufrinfo->base.mnxy[1] == -999)
                goto err6a;
            }
            ...
            if ((ch = nxtwrd(ch)) == NULL) {
              ...
              j = 0;
              while (1) {
                if ((ch = intprs(ch, &(pfi->bufrinfo->base.scxy[j]))) == NULL)
                  goto err6a;
                while (*ch == ' ')
                  ch++;
                if (*ch != ',')
              ...
              if (pfi->bufrinfo->base.scxy[0] == -999 || pfi->bufrinfo->base.scxy[1] == -999)
                goto err6a;
            }
          } else {
            goto err6a;
        ...
        while (nxtwrd(ch) != NULL) {
          if (cmpwrd("yr", ch)) {
            if ((ch = nxtwrd(ch)) == NULL) {
              ...
              j = 0;
              while (1) {
                if ((ch = intprs(ch, &(pfi->bufrinfo->offset.yrxy[j]))) == NULL)
                  goto err6a;
                while (*ch == ' ')
                  ch++;
                if (*ch != ',')
            ...
            if ((ch = nxtwrd(ch)) == NULL) {
              ...
              j = 0;
              while (1) {
                if ((ch = intprs(ch, &(pfi->bufrinfo->offset.moxy[j]))) == NULL)
                  goto err6a;
                while (*ch == ' ')
                  ch++;
                if (*ch != ',')
            ...
            if ((ch = nxtwrd(ch)) == NULL) {
              ...
              j = 0;
              while (1) {
                if ((ch = intprs(ch, &(pfi->bufrinfo->offset.dyxy[j]))) == NULL)
                  goto err6a;
                while (*ch == ' ')
                  ch++;
                if (*ch != ',')
            ...
            if ((ch = nxtwrd(ch)) == NULL) {
              ...
              j = 0;
              while (1) {
                if ((ch = intprs(ch, &(pfi->bufrinfo->offset.hrxy[j]))) == NULL)
                  goto err6a;
                while (*ch == ' ')
                  ch++;
                if (*ch != ',')
            ...
            if ((ch = nxtwrd(ch)) == NULL) {
              ...
              j = 0;
              while (1) {
                if ((ch = intprs(ch, &(pfi->bufrinfo->offset.mnxy[j]))) == NULL)
                  goto err6a;
                while (*ch == ' ')
                  ch++;
                if (*ch != ',')
            ...
            if ((ch = nxtwrd(ch)) == NULL) {
              ...
              j = 0;
              while (1) {
                if ((ch = intprs(ch, &(pfi->bufrinfo->offset.scxy[j]))) == NULL)
                  goto err6a;
                while (*ch == ' ')
                  ch++;
                if (*ch != ',')
      ...
      if ((ch = nxtwrd(rec)) == NULL) {
        ...
        j = 0;
        while (1) {
          if ((ch = intprs(ch, &(pfi->bufrinfo->stidxy[j]))) == NULL)
            goto err6a;
          while (*ch == ' ')
            ch++;
          if (*ch != ',')
        ...
        if (pfi->bufrinfo->stidxy[0] == -999 || pfi->bufrinfo->stidxy[1] == -999)
          goto err6a;
      }
      ...
      while (*(pout + len))
        len++;
      sz = len + 3;
      if ((pfi->mnam = (char *)galloc(sz, "mnam")) == NULL)
        goto err8;
      ...
      if ((ch = nxtwrd(ch)) != NULL) {
        ...
        while (*(ch + len) != ' ' && *(ch + len) != '\n' && *(ch + len) != '\t')
          len++;
        sz = len + 3;
        if ((pfi->undefattr = (char *)galloc(sz, "undefattr3")) == NULL)
          goto err8;
        for (i = 0; i < len; i++)
          *(pfi->undefattr + i) = *(ch + i);
        *(pfi->undefattr + len) = '\0';
        ...
        if ((ch = nxtwrd(ch)) != NULL) {
          ...
          while (*(ch + len) != ' ' && *(ch + len) != '\n' && *(ch + len) != '\t')
            len++;
          sz = len + 3;
          if ((pfi->undefattr2 = (char *)galloc(sz, "undefattr4")) == NULL)
            goto err8;
          for (i = 0; i < len; i++)
            *(pfi->undefattr2 + i) = *(ch + i);
      ...
      if ((pos = intprs(ch, &(pfi->ppisiz))) == NULL)
        goto errm;
      if ((ch = nxtwrd(ch)) == NULL)
        goto errm;
      if ((pos = intprs(ch, &(pfi->ppjsiz))) == NULL)
        goto errm;
      if ((ch = nxtwrd(ch)) == NULL)
        goto errm;
      ...
      if (cmpwrd("nps", ch)) {
        ...
        pfi->ppwrot = 1;
        cnt = 4;
      } else if (cmpwrd("sps", ch)) {
        pfi->ppflag = 2;
        pfi->ppwrot = 1;
        cnt = 4;
      } else if (cmpwrd("lcc", ch)) {
        pfi->ppflag = 3;
        pfi->ppwrot = 0;
        cnt = 9;
      } else if (cmpwrd("lccr", ch)) {
        pfi->ppflag = 3;
        pfi->ppwrot = 1;
        cnt = 9;
      } else if (cmpwrd("eta.u", ch)) {
        pfi->ppflag = 4;
        pfi->ppwrot = 1;
        cnt = 4;
      } else if (cmpwrd("pse", ch)) {
        pfi->ppflag = 5;
        pfi->ppwrot = 0;
        cnt = 7;
      } else if (cmpwrd("ops", ch)) {
        pfi->ppflag = 6;
        pfi->ppwrot = 0;
        cnt = 8;
      } else if (cmpwrd("bilin", ch)) {
        pfi->ppflag = 7;
        pfi->ppwrot = 1;
        cnt = 0;
      } else if (cmpwrd("file", ch)) {
        pfi->ppflag = 8;
        pfi->ppwrot = 1;
        cnt = 1;
        pfi->pdefgnrl = 0;
      } else if (cmpwrd("general", ch)) {
        pfi->ppflag = 8;
        pfi->ppwrot = 1;
        cnt = 1;
        pfi->pdefgnrl = 1;
      } else if (cmpwrd("rotll", ch)) {
        pfi->ppflag = 9;
        pfi->ppwrot = 0;
        cnt = 6;
      } else if (cmpwrd("rotllr", ch)) {
        pfi->ppflag = 9;
        pfi->ppwrot = 1;
        cnt = 6;
      } else
      ...
      for (i = 0; i < cnt; i++) {
        if ((ch = nxtwrd(ch)) == NULL)
          goto errm;
        if ((pos = getdbl(ch, &(pfi->ppvals[i]))) == NULL)
          goto errm;
      }
      ...
      if (pfi->ppflag == 7 || pfi->ppflag == 8) {
        ...
        if (cmpwrd("binary", ch))
          pdefop2 = 1;
        else if (cmpwrd("binary-big", ch))
          pdefop2 = 2;
        else if (cmpwrd("binary-little", ch))
          pdefop2 = 3;
        else if (cmpwrd("packed", ch))
          pdefop2 = 4;
        else
          goto errm;
      ...
      if ((ch = nxtwrd(mrec)) == NULL) {
        ...
        sz = strlen(ch) + 1;
        if ((vectorpairs = (char *)galloc(sz, "vecpairs")) == NULL)
          goto err8;
      ...
      if (cmpwrd("linear", ch)) {
        ...
        if ((ch = nxtwrd(ch)) == NULL)
          goto err3;
        if ((pos = intprs(ch, &i)) == NULL)
          goto err3;
        pfi->grvals[1] = gagaus(i, pfi->dnum[1]);
        if (pfi->grvals[1] == NULL)
          goto err9;
      ...
      if (cmpwrd("linear", ch)) {
        ...
        tdef.mo = -1000;
        tdef.dy = -1000;
        if ((pos = adtprs(ch, &tdef, &dt1)) == NULL)
          goto err3b_tdef;
        if (*pos != ' ' || dt1.yr == -1000 || dt1.mo == -1000.0 || dt1.dy == -1000)
          goto err3c_tdef;
        if ((ch = nxtwrd(ch)) == NULL)
          goto err4a_tdef;
        if ((pos = rdtprs(ch, &dt2)) == NULL)
          goto err4b_tdef;
        v1 = (dt2.yr * 12) + dt2.mo;
        ...
        if ((vals = (gadouble *)galloc(sz, "valslin3")) == NULL)
          goto err8;
        *(vals) = dt1.yr;
        *(vals + 1) = dt1.mo;
        *(vals + 2) = dt1.dy;
      ...
      if ((ch = nxtwrd(mrec)) == NULL)
        goto err1;
      if ((pos = intprs(ch, &(pfi->dnum[4]))) == NULL)
        goto err1;
      if (pfi->dnum[4] < 1) {
      ...
      if ((vals = (gadouble *)galloc(sz, "evals2")) == NULL)
        goto err8;
      ev1 = ev2 = 1;
      *(vals + 1) = ev1 - ev2;
      *(vals) = ev2;
      ...
      *(vals + 5) = -999.9;
      ...
      if (ch != NULL) {
          ...
          while (j < pfi->dnum[4]) {
            if ((ch = nxtwrd(ch)) == NULL) {
              ...
              if (fgets(rec, 512, descr) == NULL)
                goto err7a; /* read line, keep as mixed case */
              ch = rec;
              while (*ch == ' ' || *ch == '\t')
                ch++; /* advance through white space */
              if (*ch == '\0' || *ch == '\n')
        ...
        while (j < pfi->dnum[4]) {
          ...
          /* get the ensemble name */
          if ((getenm(ens, mrec)) != 0)
            goto err7d;
          /* get the length of the time axis */
          if ((ch = nxtwrd(rec)) == NULL)
            goto err7e;
          if ((pos = intprs(ch, &(ens->length))) == NULL)
            goto err7e;
          ...
          tdef.mo = -1000;
          tdef.dy = -1000;
          if ((pos = adtprs(ch, &tdef, &ens->tinit)) == NULL)
            goto err7e;
          if (ens->tinit.yr == -1000 || ens->tinit.mo == -1000 || ens->tinit.dy == -1000)
            goto err7e;
          ...
          if ((ch = nxtwrd(ch)) != NULL) {
            jj = 0;
            while (1) {
              if ((ch = intprs(ch, &(ens->grbcode[jj]))) == NULL)
                goto err7c;
              while (*ch == ' ')
                ch++;
              if (*ch != ',')
      ...
      if ((ch = nxtwrd(rec)) == NULL)
        goto err5;
      if ((pos = intprs(ch, &(pfi->vnum))) == NULL)
        goto err5;
      size = pfi->vnum * (sizeof(struct gavar) + 7);
      sz = size;
      if ((pvar = (struct gavar *)galloc(sz, "pvar2")) == NULL)
        goto err8;
      pfi->pvar1 = pvar;
      i = 0;
      while (i < pfi->vnum) {
        ...
        while (1) {
          if (j == 8) {
            ...
            if ((ch = intprs(ch, &(pvar->levels))) == NULL)
              goto err6;
          } else {
  ...
  if (pfi->ens1 == NULL) {
    ...
    sz = sizeof(gadouble) * 6;
    if ((vals = (gadouble *)galloc(sz, "valslin4")) == NULL)
      goto err8;
    v1 = v2 = 1;
    *(vals + 1) = v1 - v2;
    *(vals) = v2;
    ...
    if (pfi->type != 2) {
      ...
      if (pfi->idxflg == 1) {
        ...
        if ((vermap == 2) || (vermap == 3)) {
          ...
          /* header integers */
          if (pindx->hinum > 0) {
            if ((pindx->hipnt = (gaint *)galloc(pindx->hinum * sizeof(gaint), "hipnt")) == NULL)
              goto err8;
          ...
          if (pindx->hfnum > 0) {
            if ((pindx->hfpnt = (gafloat *)galloc(pindx->hfnum * sizeof(gafloat), "hfpnt")) == NULL)
              goto err8;
          ...
          /* index data: integer */
          if (pindx->intnum > 0) {
            if ((pindx->intpnt = (gaint *)galloc(pindx->intnum * sizeof(gaint), "intpnt")) == NULL)
              goto err8;
            for (i = 0; i < pindx->intnum; i++) {
              ...
              idum = gagby(urec, 0, 4);
              if (gagbb(urec, 0, 1))
                idum = -gagbb(urec, 1, 31);
              *(pindx->intpnt + i) = idum;
            }
          }
          /* index data: float */
          if (pindx->fltnum > 0) {
            if ((pindx->fltpnt = (gafloat *)galloc(pindx->fltnum * sizeof(gafloat), "fltpnt")) == NULL)
              goto err8;
          ...
          if (pindx->type >> 24 > 0)
            swpflg = 1;
          if (swpflg)
            gabswp((gafloat *)pindx, 5); /* swap the 5 integers in the gaindx structure */
          pindx->hipnt = NULL;
          pindx->hfpnt = NULL;
          pindx->intpnt = NULL;
          pindx->fltpnt = NULL;
          ...
          /* header integers */
          if (pindx->hinum > 0) {
            if ((pindx->hipnt = (gaint *)galloc(sizeof(gaint) * pindx->hinum, "hipnt2")) == NULL)
              goto err8;
            ...
            if (swpflg)
              gabswp((gafloat *)(pindx->hipnt), pindx->hinum);
          }
          /* header floats --  these are not used, hfnum is always zero */
          if (pindx->hfnum > 0) {
            if ((pindx->hfpnt = (gafloat *)galloc(sizeof(gafloat) * pindx->hfnum, "hfpnt2")) == NULL)
              goto err8;
            rc = fread(pindx->hfpnt, sizeof(gafloat), pindx->hfnum, mfile);
            if (rc != pindx->hfnum) {
            ...
            if (swpflg)
              gabswp(pindx->hfpnt, pindx->hfnum);
          }
          /* index data: integer */
          if (pindx->intnum > 0) {
            if ((pindx->intpnt = (gaint *)galloc(sizeof(gaint) * pindx->intnum, "ipnt2")) == NULL)
              goto err8;
            ...
            if (swpflg)
              gabswp((gafloat *)(pindx->intpnt), pindx->intnum);
          }
          /* index data: float */
          if (pindx->fltnum > 0) {
            if ((pindx->fltpnt = (gafloat *)galloc(sizeof(gafloat) * pindx->fltnum, "fpnt2")) == NULL)
              goto err8;
            ...
            if (swpflg)
              gabswp(pindx->fltpnt, pindx->fltnum);
          }
          /* index data: off_t */
          if ((pindx->type == 4) ||                         /* version=4 --> off_t in use */
            ...
            if (sizeof(off_t) != 8)
              goto err8a;
            if ((pindxb = (struct gaindxb *)galloc(sizeof(struct gaindxb), "pindxb")) == NULL)
              goto err8;
            pindxb->bignum = *(pindx->hipnt + 4);
            pindxb->bigpnt = NULL;
            if ((pindxb->bigpnt = (off_t *)galloc(sizeof(off_t) * pindxb->bignum, "offpnt")) == NULL)
              goto err8;
            ...
            if (swpflg)
              gabswp8(pindxb->bigpnt, pindxb->bignum);
      ...
      /* * * GRIB2 * * */
        ...
        if ((g2indx = (struct gag2indx *)galloc(sizeof(struct gag2indx), "g2indx")) == NULL)
          goto err8;
        g2indx->bigflg = 0;
        ...
        if (g2indx->version >> 24 > 0)
          swpflg = 1;
        else
          swpflg = 0;
        if (swpflg)
          gabswp(&g2indx->version, 1);
        if (g2indx->version == 3) {
        ...
        if (g2indx->version == 1 || g2indx->version == 2 || g2indx->version == 3) {
          if (g2indx->bigflg && sizeof(off_t) != 8)
            goto err8a;
          ...
          if (g2indx->g2intnum > 0) {
            ...
            if (swpflg)
              gabswp(g2indx->g2intpnt, g2indx->g2intnum);
            ...
            if (g2indx->bigflg) {
              ...
              if (swpflg)
                gabswp8(g2indx->g2bigpnt, g2indx->g2intnum);
            }
          }
        } else {
      ...
      if (vermap == 2) {
        ...
        if ((pfi->tstrt = (gaint *)galloc(sz, "tstrt")) == NULL)
          goto err8;
        if ((pfi->tcnt = (gaint *)galloc(sz, "tcnt")) == NULL)
          goto err8;
        for (i = 0; i < mcnt; i++) {
          fread(urec, sizeof(unsigned char), 4, mfile);
          idum = gagby(urec, 0, 4);
        ...
        sz = sizeof(gaint) * mcnt;
        if ((pfi->tstrt = (gaint *)galloc(sz, "tstrt1")) == NULL)
          goto err8;
        if ((pfi->tcnt = (gaint *)galloc(sz, "tcnt1")) == NULL)
          goto err8;
  ...
  for (j = 1; j <= pfi->vnum; j++) {
    ...
    if (pfi->idxflg == 2) {
      ...
      if ((pvar->vecpair < 0) && ((pvar->units[0] == 0 && pvar->units[1] == 2 && pvar->units[2] == 2) /* variabls is u */ ||
                                  (pvar->units[0] == 0 && pvar->units[1] == 2 && pvar->units[2] == 3))) /* variabls is v */ {
        if (pvar->units[2] == 2)
          rc = 3;
        else
          rc = 2;
        ...
        i = 0;
        while (i < pfi->vnum) {
          if ((pvar2->vecpair < 0) && (pvar2->units[0] == pvar->units[0]) && (pvar2->units[1] == pvar->units[1]) &&
              (pvar2->units[2] == rc) && (pvar2->units[8] == pvar->units[8]) && (pvar2->units[9] == pvar->units[9]) &&
              (pvar2->units[10] == pvar->units[10]))
            break;
          pvar2++;
          i++;
        }
    ...
    /* for GRIB1 and binary data sets */
      ...
      if ((pvar->units[0] == 33 || pvar->units[0] == 34) && (pvar->vecpair < 0)) {
        ...
        i = 0;
        while (i < pfi->vnum) {
          if ((pvar2->vecpair < 0) && (pvar2->units[0] == rc) && (pvar2->units[8] == pvar->units[8]) &&
              (pvar2->units[9] == pvar->units[9]) && (pvar2->units[10] == pvar->units[10]))
            break;
          pvar2++;
          i++;
        }
  ...
  if (err)
    goto retrn; /* end of vector pair management */
  ...
  if (pfi->type == 1) {
    ...
    if (pfi->ppflag)
      pfi->gsiz = pfi->ppisiz * pfi->ppjsiz;
  ...
  if (mflag) {
    if (pfi->type > 1) {
      ...
      if (pfi->bufrflg)
        maxlv = 1; /* maxlv not used for BUFR interface, set to 1 */
...
struct gaattr *parseattr(char *ch) {
  ...
  if (!(strncmp(attrtype, "Byte", 4)) || !(strncmp(attrtype, "byte", 4)))
    nctype = 1;
  else if (!(strncmp(attrtype, "String", 6)) || !(strncmp(attrtype, "Str", 3)) || !(strncmp(attrtype, "Url", 3)) ||
           !(strncasecmp(attrtype, "char", 4)))
    nctype = 2;
  else if (!(strncmp(attrtype, "Int16", 5)) || !(strncmp(attrtype, "UInt16", 6)) || !(strncasecmp(attrtype, "short", 5)))
    nctype = 3;
  else if (!(strncmp(attrtype, "Int32", 5)) || !(strncmp(attrtype, "UInt32", 6)) || !(strncasecmp(attrtype, "int", 3)) ||
           !(strncasecmp(attrtype, "long", 4)))
    nctype = 4;
  else if (!(strncmp(attrtype, "Float32", 7)) || !(strncasecmp(attrtype, "float", 5)))
    nctype = 5;
  else if (!(strncmp(attrtype, "Float64", 7)) || !(strncasecmp(attrtype, "double", 6)))
    nctype = 6;
  else
    nctype = 0;
...
void frepfi(struct gafile *pfi, gaint flag) {
  ...
  if (!flag)
    for (i = 0; i < 5; i++)
      if (pfi->grvals[i])
        gree(pfi->grvals[i], "f69");
  if (pfi->pindx) {
    pindx = pfi->pindx;
    if (pindx->hipnt)
      gree(pindx->hipnt, "f70");
    if (pindx->hfpnt)
      gree(pindx->hfpnt, "f71");
    if (pindx->intpnt)
      gree(pindx->intpnt, "f72");
    if (pindx->fltpnt)
      gree(pindx->fltpnt, "f73");
    gree(pindx, "f74");
    pfi->pindx = NULL;
  }
  if (pfi->pindxb) {
  ...
  if (pfi->g2indx) {
    g2indx = pfi->g2indx;
    if (g2indx->g2intpnt)
      gree(g2indx->g2intpnt, "f75");
    if (g2indx->g2bigpnt)
      gree(g2indx->g2bigpnt, "b75");
    gree(g2indx, "f76");
    pfi->g2indx = NULL;
  }
#endif
  ...
  if (pfi->ofattr)
    gree(pfi->ofattr, "f81");
  if (pfi->undefattr)
    gree(pfi->undefattr, "f82");
  if (pfi->undefattr2)
    gree(pfi->undefattr2, "f82b");
  if (pfi->bufrinfo)
    gree(pfi->bufrinfo, "f83");
#ifndef STNDALN
  if (pfi->bufrdset)
...
gaint gappcn(struct gafile *pfi, gaint pdefop1, gaint pdefop2) {
  ...
  if (pfi->ppflag != 8) {
    ...
    if ((pfi->ppi[0] = (gaint *)galloc(sz, "ppi0")) == NULL)
      goto merr;
    sz = sizeof(gadouble) * size;
    if ((pfi->ppf[0] = (gadouble *)galloc(sz, "ppf0")) == NULL)
      goto merr;
    sz = sizeof(gadouble) * size;
    if ((pfi->ppf[1] = (gadouble *)galloc(sz, "ppf1")) == NULL)
      goto merr;
    if (pfi->ppwrot) {
      sz = sizeof(gadouble) * size;
      if ((pfi->ppw = (gadouble *)galloc(sz, "ppw")) == NULL)
        goto merr;
    }
  }
  ...
  if (pfi->ppflag == 7) {
    ...
    if ((fvals = (gafloat *)galloc(sz, "ppfvals")) == NULL)
      goto merr;
    ...
    for (i = 0; i < pnum; i++) {
      ...
      sz = sizeof(gaint) * size;
      if ((pfi->ppi[i] = (gaint *)galloc(sz, "ppi3")) == NULL)
        goto merr;
      /* sequential -- header */
      if (pdefop1 == 2) {
        rc = fread(&rdw, sizeof(gaint), 1, pdfi);
      ...
      /* read the offsets */
      rc = fread(pfi->ppi[i], sizeof(gaint), size, pdfi);
      if (rc != size)
        goto merr2;
      /* byte swap if necessary */
      if ((pdefop2 == 2 && !BYTEORDER) || (pdefop2 == 3 && BYTEORDER))
      ...
      sz = sizeof(gadouble) * size;
      if ((pfi->ppf[i] = (gadouble *)galloc(sz, "ppf2")) == NULL)
        goto merr;
      /* sequential -- header */
      if (pdefop1 == 2) {
        rc = fread(&rdw, sizeof(gaint), 1, pdfi);
    ...
    sz = sizeof(gadouble) * size;
    if ((pfi->ppw = (gadouble *)galloc(sz, "ppw2")) == NULL)
      goto merr;
    /* sequential -- header */
    if (pdefop1 == 2) {
      rc = fread(&rdw, sizeof(gaint), 1, pdfi);
...
void ll2lc(gadouble *vals, gadouble grdlat, gadouble grdlon, gadouble *grdi, gadouble *grdj, gadouble *wrot) {
  ...
  */
  ...
  if (stdlt1 == stdlt2) {
    ...
    gcon = (log(sin((90.0 - fabs(stdlt1)) * d2r)) - log(sin((90.0 - fabs(stdlt2)) * d2r))) /
           (log(tan((90.0 - fabs(stdlt1)) * 0.5 * d2r)) - log(tan((90.0 - fabs(stdlt2)) * 0.5 * d2r)));
  }
  ogcon = 1.0 / gcon;
...
void ll2pse(gaint im, gaint jm, gadouble *vals, gadouble lon, gadouble lat, gadouble *grdi, gadouble *grdj) {
  ...
  if (fabs(lat) > 90.) {
    ...
    *grdj = -1;
    ...
    t = tan(pi / 4. - alat / 2.) / pow((1. - e * sin(alat)) / (1. + e * sin(alat)), e / 2.);

    if (fabs(90. - slat) < 1.E-3) {
      rho = 2. * rearth * t / pow(pow(1. + e, 1. + e) * pow(1. - e, 1. - e), e / 2.);
    } else {
      sl = slat / cdr;
      tc = tan(pi / 4. - sl / 2.) / pow((1. - e * sin(sl)) / (1. + e * sin(sl)), (e / 2.));
...
void ll2ops(gadouble *vals, gadouble lni, gadouble lti, gadouble *grdi, gadouble *grdj) {
  ...
  arg2a = cos(c2) * cos(c3);
  if (-c1 > arg2a)
    arg2a = -c1; /* arg2a = max1(arg2a,-c1)  */
  if (c1 < arg2a)
    arg2a = c1; /* min1(arg2a, c1)         */
  bb = acos(arg2a);

  c4 = hsign * lti * pi180;
  arg2a = sin(c4) / sin(bb);
  if (-c1 > arg2a)
    arg2a = -c1; /* arg2a = dmax1(arg2a,-c1) */
  if (c1 < arg2a)
    arg2a = c1; /* arg2a = dmin1(arg2a, c1) */
  alpha = asin(arg2a);
...
void ll2rotll(gadouble *vals, gadouble grdlat, gadouble grdlon, gadouble *grdi, gadouble *grdj, gadouble *wrot) {
  ...
  lat_rotated = asin(sin(lat_RW) * sin(lat_pole) + cos(lat_RW) * cos(lat_pole) * cos(lon_RW - lon_pole));
  lon_rotated = atan(cos(lat_RW) * sin(lon_RW - lon_pole) /
                     (cos(lat_RW) * sin(lat_pole) * cos(lon_RW - lon_pole) - sin(lat_RW) * cos(lat_pole)));
  ...
  *wrot = -atan(cos(lat_pole) * sin(lon_pole - lon_RW) /
                (cos(lat_RW) * sin(lat_pole) - sin(lat_RW) * cos(lat_pole) * cos(lon_pole - lon_RW)));
}
