    ...
    if (state) { /* Expect operand or '('       */
      ...
      /* logical operator */
        ...
        if (*(pos + 1) == '=') {
          ...
          stack[curr].type = 1;
          if ((*pos == '=') && (*(pos + 1) == '='))
            stack[curr].obj.op = 20;
          if ((*pos == '<') && (*(pos + 1) == '='))
            stack[curr].obj.op = 23;
          if ((*pos == '>') && (*(pos + 1) == '='))
            stack[curr].obj.op = 24;
          if ((*pos == '!') && (*(pos + 1) == '='))
            stack[curr].obj.op = 25;
          pos += 2;
          state = 1;
        } else if ((*pos == '|') && (*(pos + 1) == '|')) {
          curr++;
          ...
          if (*pos == '=')
            stack[curr].obj.op = 20;
          if (*pos == '<')
            stack[curr].obj.op = 21;
          if (*pos == '>')
            stack[curr].obj.op = 22;
          if (*pos == '|')
            stack[curr].obj.op = 26;
          if (*pos == '&')
            stack[curr].obj.op = 27;
...
gaint gaoper(struct smem *stack, gaint c1, gaint c2, gaint c, gaint op) {
  ...
  if (stack[c1].type == -1)
    pgr = stack[c1].obj.pgr;
  if (stack[c2].type == -1)
    pgr = stack[c2].obj.pgr;
...
struct gagrid *gagrop(struct gagrid *pgr1, struct gagrid *pgr2, gaint op, gaint rel) {
  ...
  if (dnum1 == dnum2) {
    if (pgr1->idim != pgr2->idim || pgr1->jdim != pgr2->jdim)
      goto err1;
    i = pgr1->idim;
    if (dnum1 > 0 && gagchk(pgr1, pgr2, pgr1->idim))
      goto err2;
    i = pgr1->jdim;
    if (dnum1 > 1 && gagchk(pgr1, pgr2, pgr1->jdim))
      goto err2;
    incr = 0;
      ...
      *uval2 = 0;
      ...
      if (op == 2)
        ...
        *val2 = *val1 * *val2;
        ...
        if (swap) {
          if (isnan(pow(*val2, *val1)))
            *uval2 = 0;
          else
            *val2 = pow(*val2, *val1);
        } else {
          if (isnan(pow(*val1, *val2)))
            *uval2 = 0;
          else
            *val2 = pow(*val1, *val2);
        }
      } else if (op == 11)
        *val2 = hypot(*val1, *val2);
      else if (op == 12) {
        if (*val1 == 0.0 && *val2 == 0.0)
          *val2 = 0.0;
        else {
          if (swap)
            *val2 = atan2(*val2, *val1);
          else
            *val2 = atan2(*val1, *val2);
        }
      } else if (op == 13) {
        if (swap) {
          if (*val1 < 0.0)
...
struct gastn *gastop(struct gastn *stn1, struct gastn *stn2, gaint op, gaint rel) {
  struct gastn *stn;
  ...
  if (stn1->idim == 0 && stn1->jdim == 1 && stn2->idim == 0 && stn2->jdim == 1)
    dimtyp = 1; /* X and Y are varying */
  else if (stn1->idim == 2 && stn1->jdim == -1 && stn2->idim == 2 && stn2->jdim == -1)
    dimtyp = 2; /* Z is varying */
  else if (stn1->idim == 3 && stn1->jdim == -1 && stn2->idim == 3 && stn2->jdim == -1)
  ...
  for (i = 0; i < stn1->rnum; i++, rpt1 = rpt1->rpt) {
    ...
    for (j = 0; j < stn2->rnum; j++, rpt2 = rpt2->rpt) {
      if (rpt2->umask == 0)
        continue;
      if (dimtyp == 1 && dequal(rpt1->lat, rpt2->lat, 1e-08) != 0)
        continue;
      if (dimtyp == 1 && dequal(rpt1->lon, rpt2->lon, 1e-08) != 0)
        continue;
      if (dimtyp == 2 && dequal(rpt1->lev, rpt2->lev, 1e-08) != 0)
        continue;
      if (dimtyp == 3 && dequal(rpt1->tim, rpt2->tim, 1e-08) != 0)
        continue;
      if (op == 2)
        ...
        if (swap) {
          if (dequal(rpt1->val, 0.0, 1e-08) == 0)
            rpt1->umask = 0;
          else
            rpt1->val = rpt2->val / rpt1->val;
        } else {
          if (dequal(rpt2->val, 0.0, 1e-08) == 0)
            rpt1->umask = 0;
          else
            rpt1->val = rpt1->val / rpt2->val;
        }
        ...
        if (swap) {
          if (isnan(pow(rpt2->val, rpt1->val)))
            rpt1->umask = 0;
          else
            rpt1->val = pow(rpt2->val, rpt1->val);
        } else {
          if (isnan(pow(rpt1->val, rpt2->val)))
            rpt1->umask = 0;
          else
            rpt1->val = pow(rpt1->val, rpt2->val);
        }
      } else if (op == 11)
        ...
        if ((dequal(rpt1->val, 0.0, 1e-08) == 0) && (dequal(rpt2->val, 0.0, 1e-08) == 0))
          ...
          rpt1->val = atan2(rpt1->val, rpt2->val);
      } else if (op == 13) {
        if (swap) {
          if (rpt1->val < 0.0)
            rpt1->umask = 0;
...
struct gastn *gascop(struct gastn *stn, gadouble val, gaint op, gaint swap) {
  ...
  for (i = 0; i < stn->rnum; i++, rpt = rpt->rpt) {
    ...
    if (op == 2)
      ...
      if (swap) {
        if (dequal(rpt->val, 0.0, 1e-08) == 0)
          rpt->umask = 0;
        else
          rpt->val = val / rpt->val;
      } else {
        if (dequal(val, 0.0, 1e-08) == 0)
          rpt->umask = 0;
        else
          rpt->val = rpt->val / val;
      }
      ...
      if (swap) {
        if (isnan(pow(val, rpt->val)))
          rpt->umask = 0;
        else
          rpt->val = pow(val, rpt->val);
      } else {
        if (isnan(pow(rpt->val, val)))
          rpt->umask = 0;
        else
          rpt->val = pow(rpt->val, val);
      }
    } else if (op == 11)
      ...
      if (dequal(rpt->val, 0.0, 1e-08) == 0 && dequal(val, 0.0, 1e-08) == 0)
        ...
        if (swap)
          rpt->val = atan2(val, rpt->val);
        else
          rpt->val = atan2(rpt->val, val);
      }
    } else if (op == 13) {
      if (rpt->val < 0.0)
        rpt->umask = 0;
...
char *varprs(char *ch, struct gastat *pst) {
  ...
  if (pfi == NULL) {
    ...
    if (cmpwrd(name, "lat") || cmpwrd(name, "lon") || cmpwrd(name, "lev")) {
      pvar = &vfake;
      vfake.levels = -999;
      vfake.vecpair = -999;
      if (cmpwrd(name, "lon")) {
        vfake.offset = 0;
        snprintf(vfake.abbrv, 5, "lon");
      }
      if (cmpwrd(name, "lat")) {
        vfake.offset = 1;
        snprintf(vfake.abbrv, 5, "lat");
      }
      if (cmpwrd(name, "lev")) {
        vfake.offset = 2;
        snprintf(vfake.abbrv, 5, "lev");
      }
   ...
   or check for a function call via rtnprs.   */
      pvar = pfi->pvar1;
      for (i = 0; (i < pfi->vnum) && (!cmpwrd(name, pvar->abbrv)); i++)
        pvar++;
      if (i >= pfi->vnum) {
        if (dotflg) {
  ...
  if (pfi && pvar && pfi->ppflag && pfi->ppwrot && pvar->vecpair > 0) {
    ...
    if (pvar->isu)
      sbu = 0; /* if pvar is u, then matching component should not be u */
    else
      sbu = 1; /* pvar is v, so matching component should be u */
    pvar2 = pfi->pvar1;
    i = 0;
    while (i < pfi->vnum) {
      if ((pvar2->vecpair == pvar->vecpair) && (pvar2->isu == sbu))
        break;
      pvar2++;
      i++;
    }
        ...
        for (i = 0; i < pgr->isiz; i++) {
          if (*ru == 0 || *r2u == 0) { /* u or v is undefined */
            ...
            *r2u = 0;
            ...
            if (ii < 1 || ii > pfi->dnum[0] || jj < 1 || jj > pfi->dnum[1]) { /* outside file's grid dimensions */
              *ru = 0;
              *r2u = 0;
            } else {
...
gaint gagchk(struct gagrid *pgr1, struct gagrid *pgr2, gaint dim) {
  ...
  if (dim == 3) { /* Dimension is time.      */
    ...
    if (dtim1.yr != dtim2.yr)
      rc = 1;
    if (dtim1.mo != dtim2.mo)
      rc = 1;
    if (dtim1.dy != dtim2.dy)
      rc = 1;
    if (dtim1.hr != dtim2.hr)
      rc = 1;
    if (dtim1.mn != dtim2.mn)
      rc = 1;
  ...
  rc = 0;
  if (fabs((conv1(vals1, gmin1)) - (conv2(vals2, gmin2))) > fuzz)
    rc = 1;
  if (fabs((conv1(vals1, gmax1)) - (conv2(vals2, gmax2))) > fuzz)
    rc = 1;
  if (rc) {
  ...
  if (i1)
    return (0); /* If linear then matches  */
...
char *stnvar(char *ch, char *vnam, struct gafile *pfi, struct gavar *pvar, struct gastat *pst) {
  struct gastn *stn;
  ...
  if (*ch == '(') {
    ...
    while (*ch != ')') {
      ...
      if (dim == pst->idim || dim == pst->jdim || (dim > 3 && (pst->idim == 0 || pst->idim == 1 || pst->jdim == 1))) {
        gaprnt(0, "Data Request Error: Invalid dimension expression\n");
        gaprnt(0, "  Attempt to set or modify varying dimension\n");
        snprintf(pout, 1255, "  Variable = %s, Dimension = %i \n", vnam, dim);
        gaprnt(0, pout);
