...
void setupba(struct gaupb *upb) { upba = upb; }
...
char *rtnprs(char *ch, char *name, struct gastat *pst) {
  ...
  if (cmpwrd("mag", name))
    fpntr = ffmag;
  if (cmpwrd("atan2", name))
    fpntr = ffatan;
  if (cmpwrd("hdivg", name))
    fpntr = ffhdiv;
  if (cmpwrd("hcurl", name))
    fpntr = ffhcrl;
  if (cmpwrd("vint", name))
    fpntr = ffvint;
  if (cmpwrd("tloop", name))
    fpntr = fftlp;
  if (cmpwrd("eloop", name))
    fpntr = ffelp;
  if (cmpwrd("aave", name))
    fpntr = ffaav;
  if (cmpwrd("scorr", name))
    fpntr = ffscor;
  if (cmpwrd("tcorr", name))
    fpntr = fftcor;
  if (cmpwrd("tmave", name))
    fpntr = fftmav;
  if (cmpwrd("maskout", name))
    fpntr = ffmask;
  if (cmpwrd("gr2stn", name))
    fpntr = ffg2s;
  if (cmpwrd("tvrh2q", name))
    fpntr = fftv2q;
  if (cmpwrd("tvrh2t", name))
    fpntr = fftv2t;
  if (cmpwrd("gint", name))
    fpntr = ffgint;
  if (cmpwrd("oacres", name))
    fpntr = ffoacr;
  if (cmpwrd("oabin", name))
    fpntr = ffoabn;
  if (cmpwrd("smth9", name))
    fpntr = ffsmth;
  if (cmpwrd("stnave", name))
    fpntr = ffsave;
  if (cmpwrd("stnmin", name))
    fpntr = ffsmin;
  if (cmpwrd("stnmax", name))
    fpntr = ffsmax;
  if (cmpwrd("skip", name))
    fpntr = ffskip;
  if (cmpwrd("const", name))
    fpntr = ffcnst;
  if (cmpwrd("cdiff", name))
    fpntr = ffcdif;
  if (cmpwrd("mean", name))
    fpntr = ffmn;
  if (cmpwrd("amean", name))
    fpntr = ffamn;
  if (cmpwrd("sum", name))
    fpntr = ffsum;
  if (cmpwrd("sumg", name))
  ...
  if (cmpwrd("asum", name))
    fpntr = ffasum;
  if (cmpwrd("asumg", name))
    fpntr = ffasumg;
  if (cmpwrd("atot", name))
    fpntr = ffatot;
  if (cmpwrd("grarea", name))
    fpntr = ffgrarea;
  if (cmpwrd("coll2gr", name))
    fpntr = ffclgr;
  if (cmpwrd("min", name))
    fpntr = ffmin;
  if (cmpwrd("max", name))
    fpntr = ffmax;
  if (cmpwrd("minloc", name))
    fpntr = ffminl;
  if (cmpwrd("maxloc", name))
    fpntr = ffmaxl;
  if (cmpwrd("fndlvl", name))
    fpntr = ffflvl;
  if (cmpwrd("sregr", name))
    fpntr = ffsreg;
  if (cmpwrd("tregr", name))
    fpntr = fftreg;
  if (cmpwrd("s2g1d", name))
    fpntr = ffs2g1d;
  if (cmpwrd("lterp", name))
    fpntr = fflterp;
  if (cmpwrd("amin", name))
    fpntr = ffamin;
  if (cmpwrd("amax", name))
    fpntr = ffamax;
  if (cmpwrd("aminlocx", name))
    fpntr = ffaminlocx;
  if (cmpwrd("aminlocy", name))
    fpntr = ffaminlocy;
  if (cmpwrd("amaxlocx", name))
    fpntr = ffamaxlocx;
  if (cmpwrd("amaxlocy", name))
    fpntr = ffamaxlocy;
  ...
  if (fpntr == NULL) {
    upb = upba;
    while (upb) {
      if (cmpwrd(upb->name, name) && upb->type <= 2)
        break;
      upb = upb->upb;
    }
  }
  ...
  *pst2 = *pst; /* Provide copy of gastat  */
  ...
  if (upb)
    rc = ffudpi(pfc, pst2, upb); /* Call the plug-in        */
  else
...
gaint ffasin(struct gafunc *pfc, struct gastat *pst) {
  ...
  if (pst->type == 1) {
    ...
    while (rpt != NULL) {
      if (rpt->umask != 0) {
        if (rpt->val > 1.0 || rpt->val < -1.0)
          rpt->umask = 0;
        else
          rpt->val = asin(rpt->val);
      }
      rpt = rpt->rpt;
    }
  }
...
gaint ffif(struct gafunc *pfc, struct gastat *pst) {
  ...
  *mypgr = *pgr1;
  ...
  if (pgrres1 == NULL)
    rc = 1;
  if (rc == 0)
    pgrres2 = gagrop(mypgr, pst3.result.pgr, 15, 0);
  if (pgrres2 == NULL)
    rc = 1;
  if (rc == 0)
    pgrres3 = gagrop(pgrres2, pgrres1, 2, 0);
  if (pgrres3 == NULL)
    rc = 1;
  ...
  if (pgrres3 == mypgr)
    mypgr->alocf = savaloc; /* propagate alocf... */
  else
    pgr1->alocf = savaloc; /*  or restore it */

  if (pgrres3 != pst->result.pgr)
    gafree(pst);
  if (pgrres3 != pst2.result.pgr)
    gafree(&pst2);
  if (pgrres3 != pst3.result.pgr)
    gafree(&pst3);
  if (pgrres3 != mypgr)
    gagfre(mypgr);
...
gaint fflterp(struct gafunc *pfc, struct gastat *pst) {
  ...
  if (pfc->argnum >= 3) {
    if (strcmp("bilin", pfc->argpnt[2]) == 0) {
      opt = 0;
      bessel = 0;
    } /* bilinear interpolation */
    if (strcmp("bessel", pfc->argpnt[2]) == 0) {
      opt = 0;
      bessel = 1;
    } /* bilinear interpolation with bessel */
    if (strcmp("aave", pfc->argpnt[2]) == 0)
      opt = 1; /* area average */
    if (strcmp("amean", pfc->argpnt[2]) == 0)
      opt = 2; /* area mean */
  ...
  if (pst->idim == 3) {
    if ((*(pgr1->ivals + 6) < 0.5 && *(pgr2->ivals + 6) > 0.8) || (*(pgr1->ivals + 6) > 0.8 && *(pgr2->ivals + 6) < 0.5)) {
  ...
  if (pst->jdim == 3) {
    if ((*(pgr1->jvals + 6) < 0.5 && *(pgr2->jvals + 6) > 0.8) || (*(pgr1->jvals + 6) > 0.8 && *(pgr2->jvals + 6) < 0.5)) {
      gaprnt(0, "Error from LTERP: Grids do not have matching time increments \n");
      error = 1;
      goto err;
    }
  ...
  if (pst->idim == 3 || pst->jdim == 3) {
    ...
    if (pst->idim == 3) {
      ...
      /* normalize */
      for (i = 0; i < pgr2->jsiz; i++)
        *(y2 + i) = tval + tscl * (gadouble)i;
    }
  }
        ...
        for (i = 0; i < pgr2->isiz; i++) {
          ...
          if (gxout[i] < 0 || gyout[j] < 0) {
            *(gr2u + ij2) = 0;
            ...
            ij1 = ic + jc * pgr1->isiz;
            /* make sure surrounding points are valid */
            if (*(gr1u + ij1) == 1 && *(gr1u + ij1 + 1) == 1 && *(gr1u + ij1 + pgr1->isiz) == 1 &&
            ...
            if (bessel == 1) {
              ...
              for (ii = ic - 1; ii <= ic + 2; ii++) {
                ...
                if (ic > pgr1->isiz - 3 || jc > pgr1->jsiz - 3) {
                  flag = 0;
                  break;
                }
                ...
                if (uijm1 == 0 || uij == 0 || uijp1 == 0 || uijp2 == 0) {
                  flag = 0;
                  break;
                }
                u = (fij + fijp1) * 0.5;
...
gaint aave(struct gafunc *pfc, struct gastat *pst, int sel) {
  ...
  if (pfc->argnum == 2 && !strncmp(pfc->argpnt[1], "global", 1))
    gflag = 1;
...
gaint scorr(struct gafunc *pfc, struct gastat *pst, gaint sel) {
  ...
  /* Check for valid number of args       */
  if (pfc->argnum == 3 && !strncmp(pfc->argpnt[2], "global", 1))
    gflag = 1;
...
char doaave(struct gagrid *pgr, gadouble dmin0, gadouble dmax0, gadouble dmin1, gadouble dmax1, gaint sel, gadouble *result) {
  gadouble (*iconv)(gadouble *, gadouble);
  gadouble (*jconv)(gadouble *, gadouble);
...
gaint tmaskf(struct gafunc *pfc, struct gastat *pst, gaint sel) {
  ...
  if (ch == NULL || dim != 3 || wflag == 2) {
    ...
    gaprnt(0, pout);
    if (dim != 3)
      gaprnt(0, "  expr does not describe time dimension\n");
    if (wflag == 2) {
  ...
  if (ch == NULL || dim != 3 || t2 < t1 || wflag == 2) {
    ...
    if (dim != 3)
      gaprnt(0, "  expr does not describe time dimension\n");
    if (t2 < t1)
      gaprnt(0, "  end time is earlier than start time\n");
    if (wflag == 2) {
      snprintf(pout, 1255, "  offt expression not supported as an arg to %s\n", fnam);
      gaprnt(0, pout);
    }
...
gaint ave(struct gafunc *pfc, struct gastat *pst, gaint sel) {
  ...
  if (ch == NULL || dim2 != dim || gr2 < gr1 || wflag == 2) {
    ...
    gaprnt(0, pout);
    if (dim2 != dim)
      gaprnt(0, "  start and end points have different dimensions\n");
    if (gr2 < gr1)
      gaprnt(0, "  end grid point is less than start grid point \n");
    if (wflag == 2) {
      snprintf(pout, 1255, "  offt expression not supported as an arg to %s\n", fnam);
      gaprnt(0, pout);
    }
  ...
  if (pfc->argnum == 5) {
    if (*(pfc->argpnt[4]) == '-' && *(pfc->argpnt[4] + 1) == 'b')
      bndflg = 1;
    else {
      snprintf(pout, 1255, "Error from %s: Invalid option flags\n", fnam);
      gaprnt(0, pout);
      ...
      for (i = 0; i < siz; i++) {
        if (sel >= 5 && sel <= 8) {
          if (*sumu == 0 || *valu == 0) {
            ...
            if ((sel == 5 || sel == 7) && *val < *sum) {
              *sum = *val;
              *cnt = d;
            }
            if ((sel == 6 || sel == 8) && *val > *sum) {
              *sum = *val;
              *cnt = d;
            }
          }
...
gaint ffgint(struct gafunc *pfc, struct gastat *pst) {
  ...
  if (ch == NULL || wflag == 2) {
    ...
    if (wflag == 2)
      gaprnt(0, "  offt expression not supported as an arg to GINT\n");
    return (1);
  }
  ...
  d = d1 + 1;
  if (d > d2)
    return (0); /* If only one grid, return  */
...
gaint ffhdiv(struct gafunc *pfc, struct gastat *pst) {
  ...
  if (gagchk(pgr1, pgr2, pst->idim) || gagchk(pgr1, pgr2, pst->jdim)) {
...
gaint fftv2(struct gafunc *pfc, struct gastat *pst, gaint tflag) {
  ...
  if ((pgrrh->idim != pgrtv->idim) || (pgrrh->jdim != pgrtv->jdim) || (pgrrh->idim > -1 && gagchk(pgrrh, pgrtv, pgrrh->idim)) ||
...
gaint ffg2s(struct gafunc *pfc, struct gastat *pst) {
  ...
  nearn = 0;
  if (pfc->argnum == 3 && strcmp("-n", pfc->argpnt[2]) == 0)
    nearn = 1;
  if (nearn)
    gaprnt(2, "Notice: Using nearest neighbor instead of bilinear interpolation\n");
...
gaint ffg2s2(struct gafunc *pfc, struct gastat *pst) {
  ...
  if (pst->type == 0) {
    ...
    if (pfc->argnum == 4 && strcmp("-n", pfc->argpnt[3]) == 0)
      nearn = 1;
  }
...
gaint ffclgr(struct gafunc *pfc, struct gastat *pst) {
  ...
  if (pfc->argnum > 1) {
    if (cmpwrd("-u", pfc->argpnt[1]))
      lflg = 2;
    else if (intprs(pfc->argpnt[1], &i) != NULL)
      ucnt = i;
    else
      gaprnt(1, "COLL2GR Warning:  2nd arg Invalid; Ignored\n");
  }
  /* JMA this option not documented */
  if (pfc->argnum > 2) {
    if (cmpwrd("-n0", pfc->argpnt[2]))
      noundef = 0;
    else if (cmpwrd("-n1", pfc->argpnt[2]))
      noundef = 1;
    else
      gaprnt(1, "COLL2GR Warning:  3nd arg Invalid; Ignored\n");
  }
  ...
  if (lflg == 1) {
    ...
    while (clct) {
      ...
      while (rpt) {
        ...
        if (noundef && (rpt->umask == 0))
          flag = 0;
        /* don't use this level if out of range */
        if ((lev > pst->dmin[2] || lev < pst->dmax[2]) && rev == 0)
          flag = 0;
        if ((lev < pst->dmin[2] || lev > pst->dmax[2]) && rev == 1)
          flag = 0;
        while (i < lcnt && flag) {
  ...
  *jv = (gadouble)lcnt;
  ...
  for (i = 1; i <= scnt; i++)
    *(iv + i) = pst->dmin[0] + (gadouble)(i - 1) * xdiff;
  for (i = 0; i < lcnt; i++)
    *(jv + i + 1) = *(levs + i);
  *(jv + lcnt + 1) = -999.9;
  *(iv + scnt + 1) = -999.9;
...
gaint ffoacr(struct gafunc *pfc, struct gastat *pst) {
  ...
  while (rpt != NULL) {
    ...
    i = (gaint)rpt->lon;
    j = (gaint)rpt->lat;
    if (i < 0 || i > (pgr->isiz - 1) || j < 0 || j > (pgr->jsiz - 1) || rpt->umask == 0)
      rpt->work = -999;
    else {
  ...
  for (p = 0; p < irad; p++) {
    ...
    for (j = 0; j < pgr->jsiz; j++) {
      ...
      for (i = 0; i < pgr->isiz; i++) {
        ...
        rpt = stn->rpt;
        while (rpt) {
          if (rpt->work == -999 || rpt->lon < xmin || rpt->lon > xmax || rpt->lat < ymin || rpt->lat > ymax ||
              (d = hypot(x - rpt->lon, y - rpt->lat)) > rad)
            rpt = rpt->rpt;
          else {
            d2 = d * d;
...
gaint ffoabn(struct gafunc *pfc, struct gastat *pst) {
  ...
  if (pfc->argnum > 2) {
    ...
    if (*ch == '-' && *(ch + 1) == 'c')
      ...
      gaprnt(1, "Warning from OABIN: Invalid flag.  Ignored.\n");
  }
...
gaint ffsmth(struct gafunc *pfc, struct gastat *pst) {
  ...
  cor = 0.3;
  if (pfc->argnum == 4) {
    if (getdbl(pfc->argpnt[1], &mid) == NULL || getdbl(pfc->argpnt[2], &sid) == NULL || getdbl(pfc->argpnt[3], &cor) == NULL) {
...
gaint ffsave(struct gafunc *pfc, struct gastat *pst) {
  ...
  if (ch == NULL || dim != 3 || wflag == 2) {
    ...
    if (wflag == 2)
      gaprnt(0, "  offt expression not supported as an arg to STNAVE\n");
    return (1);
  }
  ...
  while (d <= d2 && !rc) {
    ...
    while (rpt) {
      if (rpt->work && rpt->umask != 0) {
        ...
        while (rpt2) {
          ...
          if (rpt2->umask != 0 && !cmpch(rpt->stid, rpt2->stid, 8) && rpt->lat == rpt2->lat && rpt->lon == rpt2->lon) {
            sum = sum + rpt2->val;
            cnt = cnt + 1.0;
    ...
    while (rpt) {
      if (rpt->work) {
        ...
        while (rpt2) {
          if (!cmpch(rpt->stid, rpt2->stid, 8) && rpt->lat == rpt2->lat && rpt->lon == rpt2->lon)
            break;
          rpt2 = rpt2->rpt;
        }
        if (rpt2 == NULL) {
...
gaint ffcnst(struct gafunc *pfc, struct gastat *pst) {
  ...
  if (pfc->argnum > 2) {
    ...
    if (*ch == '-' && *(ch + 1) == 'u')
      ...
      gaprnt(1, "Warning from CONST: Invalid flag.  Ignored.\n");
  }
...
gaint ffcdif(struct gafunc *pfc, struct gastat *pst) {
  ...
  if (*(pfc->argpnt[1]) == 'x')
    dim = 0;
  else if (*(pfc->argpnt[1]) == 'y')
    dim = 1;
  else if (*(pfc->argpnt[1]) == 'z')
    dim = 2;
  else if (*(pfc->argpnt[1]) == 't')
    dim = 3;
  else if (*(pfc->argpnt[1]) == 'e')
    dim = 4;
  else {
...
gaint ffflvl(struct gafunc *pfc, struct gastat *pst) {
  ...
  if ((clev < ulev && flev < llev) || (clev > ulev && flev > llev)) { /* User ordering is same as file */
    for (i = 1; i <= lvt; i++) {
      lev = lvconv(lvvals, (gadouble)i);
      if ((flev < llev && (lev >= flev && lev <= llev)) || (flev > llev && (lev <= flev && lev >= llev))) {
        *(levs + cnt) = lev;
        cnt++;
      }
    }
  ...
  if (pgrv->idim != pgr1->idim || pgrv->jdim != pgr1->jdim || gagchk(pgrv, pgr1, pgrv->idim) || gagchk(pgrv, pgr1, pgrv->jdim)) {
...
void gaprntupb(void) {
  ...
  while (upb) {
    ...
    if (upb->type == 2)
      sprintf(pout, "defop      %-15s  %s\n", upb->name, upb->fname);
    if (upb->type == 3)
      sprintf(pout, "gxdisplay  %-15s  %s\n", upb->name, upb->fname);
    if (upb->type == 4)
      sprintf(pout, "gxprint    %-15s  %s\n", upb->name, upb->fname);
    gaprnt(2, pout);
    upb = upb->upb;
  }
}
