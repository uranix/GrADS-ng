...
gaint gribmap(void) {
  ...
  if (update | upgrade | downgrade) {
    ...
    if (rc)
      printf("gribmap: ERROR! unable to parse the data descriptor file or the existing index file\n");
  } else {
    rc = gaddes(ifile, pfi, 0);
    if (rc)
      printf("gribmap: ERROR! unable to parse the data descriptor file\n");
  }
  if (rc)
    return (1);
  ...
  if (upgrade || downgrade) {
    ...
    if (pfi->idxflg == 1) {
      ...
      *(pindx->hipnt + 3) = pfi->grbgrd;
      ...
      for (i = 0; i < pindx->intnum; i++)
        *(pindx->intpnt + i) = *(pfi->pindx->intpnt + i);
      for (i = 0; i < pindx->fltnum; i++)
        *(pindx->fltpnt + i) = *(pfi->pindx->fltpnt + i);
      if (upgrade) {
        if (pfi->pindx->type == 4)
          for (i = 0; i < pindxb->bignum; i++)
            *(pindxb->bigpnt + i) = *(pfi->pindxb->bigpnt + i);
      }
    ...
    /* * * GRIB2 index file upgrade/downgrade * * */
      ...
      for (i = 0; i < g2indx->g2intnum; i++)
        *(g2indx->g2intpnt + i) = *(pfi->g2indx->g2intpnt + i);
      if (upgrade) {
        if (g2indx->bigflg) {
          ...
          for (i = 0; i < g2indx->g2intnum; i++)
            *(g2indx->g2bigpnt + i) = *(pfi->g2indx->g2bigpnt + i);
        }
      }
  ...
  if (pfi->idxflg == 1) {
    ...
    if (pfi->grbgrd < -900 || pfi->grbgrd == 255) {
      ...
      gtype[0] = 29;
      gtype[1] = 30;
      if (pfi->dnum[0] != 144 || pfi->dnum[1] != 73 || pfi->linear[0] != 1 || pfi->linear[1] != 1 || *(pfi->grvals[0]) != 2.5 ||
          *(pfi->grvals[0] + 1) != -2.5 || *(pfi->grvals[1]) != 2.5 || *(pfi->grvals[1] + 1) != -92.5) {
    ...
    *(pindx->hipnt + 5) = pfi->dnum[4];
      ...
      while (1) {
        if (pfi->tmplat) {
          ...
          if (tcur == 0) {
            told = 0;
            tcur = 1;
            while (pfi->fnums[(e - 1) * pfi->dnum[3] + tcur - 1] == -1)
              tcur++;
          } else { /* tcur!=0 */
          ...
          if ((told != -1) && (pfi->fnums[(e - 1) * pfi->dnum[3] + tcur - 1] == -1))
            break;
          ...
          if (fnum != -1) {
            while (fnum == pfi->fnums[(e - 1) * pfi->dnum[3] + tmax])
              tmax++;
        ...
        if (!quiet)
          printf("gribmap: opening GRIB file %s \n", ch);
        gfile = fopen(ch, "rb");
        if (gfile == NULL) {
          if (pfi->tmplat) {
            if (!quiet)
              printf("gribmap: Warning! could not open GRIB file %s\n", ch);
            fflush(stdout);
            continue;
          } else {
    ...
    if (!quiet)
      printf("gribmap: reached end of files\n");
   ...
   * * * * * * */
    ...
    if (pfi->dnum[4] > 1 && pfi->tmplat == 1) {
     ...
     These are the times that are contained in this particular file */
        ...
        /* Open this GRIB file and position to start of first record */
        if (!quiet)
          printf("gribmap: scanning GRIB2 file: %s \n", ch);
        fflush(stdout);
        gfile = fopen(ch, "rb");
        if (gfile == NULL) {
          if (!quiet)
  ...
  err:
    if (g2indx) {
      if (g2indx->g2intpnt)
        gree(g2indx->g2intpnt, "f314");
      if (g2indx->g2bigpnt)
        gree(g2indx->g2bigpnt, "f314");
      gree(g2indx, "f315");
    }
    if (cgrib)
      gree(cgrib, "f316");
...
gaint gribhdr(struct grhdr *ghdr) {
  ...
  if (ghdr->btim.hr > 23)
    ghdr->btim.hr = 0; /* Special for NCAR */
  if (len > 24) {
  ...
  if (ghdr->ftu == 0)
    ...
    atim.hr = ghdr->fcstt * 3; /* 3Hr incr */
  else if (ghdr->ftu == 11)
    atim.hr = ghdr->fcstt * 6; /* 6Hr incr */
  else if (ghdr->ftu == 12)
    atim.hr = ghdr->fcstt * 12; /* 12Hr incr */
  else if (ghdr->ftu == 13)
    atim.mn = ghdr->fcstt * 15; /* 15-minute incr */
  else if (ghdr->ftu == 14)
    atim.mn = ghdr->fcstt * 30; /* 30-minute incr */
  else
    ghdr->fcstt = -999;
  ...
  if (ghdr->vers == 0) {
    fpos = fpos + 8 + ghdr->pdslen + ghdr->gdslen + ghdr->bmslen + ghdr->bdslen;
  } else
    fpos = fpos + (off_t)ghdr->len;
...
gaint gribrec(struct grhdr *ghdr, struct gafile *pfi, struct gaindx *pindx, gaint tmin, gaint tmax, gaint e) {
  ...
  if (pfi->grbgrd == 255 || pfi->grbgrd < -900) {
    ...
    if (pfi->ppflag) {
      if (ghdr->gicnt != 65535 && ((ghdr->gicnt != pfi->ppisiz) || (ghdr->gjcnt != pfi->ppjsiz))) {
        if (verb) {
      ...
      if (ghdr->gicnt != 65535 && ((ghdr->gicnt != pfi->dnum[0]) || (ghdr->gjcnt != pfi->dnum[1]))) {
        if (verb) {
          printf("GRIB grid size does not match descriptor:");
          gribpr(ghdr);
        }
  ...
  if (tau0 && ((ghdr->btim.yr != btimdd.yr) || (ghdr->btim.mo != btimdd.mo) || (ghdr->btim.dy != btimdd.dy) ||
               (ghdr->btim.hr != btimdd.hr) || (ghdr->btim.mn != btimdd.mn))) {
    if (verb) {
  ...
  while (i < pfi->vnum) {
    if (pvar->levels > 0) { /* multi level data */
      ...
      if (dequal(pvar->units[0], ghdr->parm, 1e-8) == 0 && dequal(pvar->units[8], ghdr->ltyp, 1e-8) == 0) {
        if ((pvar->units[10] < -900 && dequal(pvar->units[9], ghdr->level, 1e-8) == 0) ||
            (pvar->units[10] > -900 && dequal(pvar->units[9], ghdr->l1, 1e-8) == 0 &&
             dequal(pvar->units[10], ghdr->l2, 1e-8) == 0) ||
...
void gribfill(gaint eoff, gaint ioff, gaint joff, gaint nsiz, struct grhdr *ghdr, struct gaindx *pindx) {
  ...
  if (bigflg) {
    ...
    *(pindxb->bigpnt + boff) = ghdr->ldpos;
    if (ghdr->bmsflg)
      *(pindxb->bigpnt + boff + 1) = ghdr->lbpos;
...
void gribpr(struct grhdr *ghdr) {
  if (bigflg) {
    printf("%6i % 10ld % 3i % 1i % 5i % 4i % 4i %-5i % 10ld % 10ld % 3i  ", irec, (long)fpos, ghdr->id, ghdr->gdsflg, ghdr->bmsflg,
           ghdr->parm, ghdr->ltyp, ghdr->level, (long)ghdr->ldpos, (long)ghdr->lbpos, ghdr->bnum);
  } else {
    printf("%6i % 10ld % 3i % 1i % 5i % 4i % 4i %-5i % 10i % 10i % 3i  ", irec, (long)fpos, ghdr->id, ghdr->gdsflg, ghdr->bmsflg,
           ghdr->parm, ghdr->ltyp, ghdr->level, ghdr->dpos, ghdr->bpos, ghdr->bnum);
  }
  printf("btim: %04i%02i%02i%02i:%02i ", ghdr->btim.yr, ghdr->btim.mo, ghdr->btim.dy, ghdr->btim.hr, ghdr->btim.mn);
  printf("tau: % 6i ", ghdr->fcstt);
  printf("dtim: %04i%02i%02i%02i:%02i ", ghdr->dtim.yr, ghdr->dtim.mo, ghdr->dtim.dy, ghdr->dtim.hr, ghdr->dtim.mn);
  printf("\n");
}
...
gaint wtgmap(void) {
  ...
  nb = 2 + (4 * 4) + /* version in byte 2, then 4 ints with number of each data type */
       pindx->hinum * sizeof(gaint) + pindx->hfnum * sizeof(gaint) + pindx->intnum * sizeof(gaint) +
  ...
  bcnt = 0;
  gapby(0, map, bcnt, 1);
  bcnt++; /* set the first byte to 0 */
  gapby(g1ver, map, bcnt, 1);
  bcnt++;                            /* set the second byte to the version number */
  ...
  if (g1ver == 2) {
    ...
    gapby(btimdd.yr, map, bcnt, 2);
    bcnt += 2; /* initial year */
    gapby(btimdd.mo, map, bcnt, 1);
    bcnt++; /* initial month */
    gapby(btimdd.dy, map, bcnt, 1);
    bcnt++; /* initial day */
    gapby(btimdd.hr, map, bcnt, 1);
    bcnt++; /* initial hour */
    gapby(btimdd.mn, map, bcnt, 1);
    bcnt++; /* initial minute */
    gapby(0, map, bcnt, 1);
    bcnt++; /* initial second */
  }
...
void g2fill(gaint eoff, gaint ioff, gaint ng2elems, off_t iseek, g2int fldnum, struct gag2indx *g2indx) {
  gaint joff;
  if (g2indx->bigflg) {
    joff = eoff + ioff;
...
gaint g2grid_check(gribfield *gfld, struct gafile *pfi, gaint r, gaint f) {
  ...
  if (pfi->grbgrd == 255 || pfi->grbgrd < -900) {
    if (((pfi->ppflag) && (gfld->ngrdpts != pfi->ppisiz * pfi->ppjsiz)) ||
      ...
      if (verb)
        printf("number of grid points does not match descriptor ");
      return (1);
    }
  }
  ...
  if ((gaint)gfld->igdtmpl[7] != -1) {
    if (gfld->igdtnum == 0 || gfld->igdtnum == 40 || gfld->igdtnum == 20 || gfld->igdtnum == 30) {
      if (gfld->igdtmpl[7] != xsize) {
        if (verb)
          printf("x dimensions are not equal: nx=%d xsize=%d", (gaint)gfld->igdtmpl[7], xsize);
        return (1);
      }
      if (gfld->igdtmpl[8] != ysize) {
        if (verb)
          printf("y dimensions are not equal: nx=%d xsize=%d", (gaint)gfld->igdtmpl[8], ysize);
        return (1);
      }
    }
  }
...
gaint g2time_check(gribfield *gfld, g2int *listsec1, struct gafile *pfi, gaint r, gaint f, gaint tmin, gaint tmax) {
  ...
  if (notau) {
    ...
    if (gfld->ipdtnum < 8 || gfld->ipdtnum == 15 || gfld->ipdtnum == 48 || gfld->ipdtnum == 60) {
      ...
      if (gfld->ipdtnum == 48)
        trui_idx = 18;
      if (gfld->ipdtmpl[trui_idx] == 0)
        tfld.mn = gfld->ipdtmpl[trui_idx + 1];
      else if (gfld->ipdtmpl[trui_idx] == 1)
        tfld.hr = gfld->ipdtmpl[trui_idx + 1];
      else if (gfld->ipdtmpl[trui_idx] == 2)
        tfld.dy = gfld->ipdtmpl[trui_idx + 1];
      else if (gfld->ipdtmpl[trui_idx] == 3)
        tfld.mo = gfld->ipdtmpl[trui_idx + 1];
      else if (gfld->ipdtmpl[trui_idx] == 4)
        tfld.yr = gfld->ipdtmpl[trui_idx + 1];
      else if (gfld->ipdtmpl[trui_idx] == 10)
        tfld.hr = gfld->ipdtmpl[trui_idx + 1] * 3; /* 3Hr incr */
      else if (gfld->ipdtmpl[trui_idx] == 11)
        tfld.hr = gfld->ipdtmpl[trui_idx + 1] * 6; /* 6Hr incr */
      else if (gfld->ipdtmpl[trui_idx] == 12)
        tfld.hr = gfld->ipdtmpl[trui_idx + 1] * 12; /* 2Hr incr */
      else
        tfield = -99;
      if (tfield == -99) {
        /* use reference time as valid time */
  ...
  if (t < (1.0 - delta) || t > ((gafloat)(pfi->dnum[3]) + delta)) {
    if (verb)
      printf("valid time %4d%02d%02d%02d:%02d (t=%g) is outside grid limits", tvalid.yr, tvalid.mo, tvalid.dy, tvalid.hr, tvalid.mn,
             t);
    return (-99);
  }
  /* Check if valid time is an integer (+/- the designated threshold) */
  ...
  if (fabs((gafloat)it - t) > delta) {
    if (verb)
      printf("valid time %4d%02d%02d%02d:%02d (t=%g) has non-integer grid index", tvalid.yr, tvalid.mo, tvalid.dy, tvalid.hr,
             tvalid.mn, t);
    return (-99);
  }
  /* Check if valid time matches range of times for this file  */
  if (it < tmin || it > tmax) {
    if (verb)
      printf("valid time %4d%02d%02d%02d:%02d (it=%d) is outside file limits (%d-%d)", tvalid.yr, tvalid.mo, tvalid.dy, tvalid.hr,
             tvalid.mn, it, tmin, tmax);
    return (-99);
  }
  if (verb)
    printf("valid at %4d-%02d-%02d-%02d:%02d (t=%d)", tvalid.yr, tvalid.mo, tvalid.dy, tvalid.hr, tvalid.mn, it);
  return (it);
}
  ...
  while (i < pfi->vnum) {
    if (pvar->levels > 0) {
      ...
      if (rc1 == 0 && rc2 == 0 && rc3 == 0 && rc4 == 0 && rc5 == 0 && rc6 == 0) { /* all the above match */
        ...
        if ((pvar->units[9] < -900)                                             /* LVAL not given */
...
gaint g2a_check(gribfield *gfld, struct gavar *pvar) {
  ...
  /* percentile forecasts */
    ...
    if ((gaint)pvar->units[16] == (gaint)gfld->ipdtmpl[15])
      match = 1; /* percentiles match */
  }
    ...
    if (ens->grbcode[0] > -900) {
      if (ens->grbcode[1] > -900) {
        ...
        if ((gfld->ipdtnum == 1 || gfld->ipdtnum == 11 || gfld->ipdtnum == 60 || gfld->ipdtnum == 61) &&
            ((ens->grbcode[0] == gfld->ipdtmpl[15]) && (ens->grbcode[1] == gfld->ipdtmpl[16]))) {
          return (e);
        }
      } else {
        /* PDT 2 or 12 */
        if ((gfld->ipdtnum == 2 || gfld->ipdtnum == 12) && (ens->grbcode[0] == gfld->ipdtmpl[15])) {
          return (e);
        }
      }
    }
