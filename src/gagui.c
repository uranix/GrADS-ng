...
void CB_Load(Widget w, void *data) {
  ...
  if (strstr(data, "open"))
    last_path = last_path_open;
  else if (strstr(data, "sdfopen"))
    last_path = last_path_sdfopen;
  else if (strstr(data, "exec"))
    last_path = last_path_exec;
  else if (strstr(data, "run"))
    last_path = last_path_run;
  else
    last_path = NULL;
  ...
  if (fname) {
    ...
    dname = dirname(fname);
    if (strstr(data, "open"))
      strcpy(last_path_open, dname);
    else if (strstr(data, "sdfopen"))
      strcpy(last_path_sdfopen, dname);
    else if (strstr(data, "exec"))
      strcpy(last_path_exec, dname);
    else if (strstr(data, "run"))
      strcpy(last_path_run, dname);
    if (dname)
      free(dname);
...
void CB_VarSel(Widget W, void *data) {
  ...
  for (i = 0; i < pfi->vnum; i++) {
    ...
    snprintf(item_list[i], 255, "%12.12s  %3i  %s", pvar->abbrv, pvar->levels, pvar->varnm);
    pvar++;
  }
...
void CB_CmdWin(Widget W, void *data) {
  ...
  /* Creates widgets, etc... */
  Cmd_window = MakeWindow("GrADS Command Window", SAME_DISPLAY, NONEXCLUSIVE_WINDOW);
  ...
  w[3] = MakeScrollList(CmdWinList, 500, 200, (void *)CB_CmdWinList, CmdWinList);
  CmdList_window = w[3];
...
void CB_Toggle(Widget w, void *data) {
  ...
  if (strstr(data, "hold")) {
    hold_on = 1 - hold_on;
    if (hold_on)
      printf("Hold  ON: no clear screen before display\n");
    else
      printf("Hold OFF: clear screen before display\n");
  }
}
