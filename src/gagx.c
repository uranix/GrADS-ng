...
void gaplot(struct gacmn *pcm) {
  ...
  proj = pcm->mproj;
  if (pcm->mproj > 1 && (pcm->xflip || pcm->yflip))
    pcm->mproj = 1;

  if (pcm->gout0 == 1)
    gastts(pcm); /* gxout stat     */
  else if (pcm->gout0 == 2)
    gadprnt(pcm); /* gxout print    */
  else if (pcm->gout0 == 3)
    gaoutgds(pcm); /* gxout writegds */
  else {
    ...
    if (pcm->type[0] == 1) {
      ...
      if (pgr->idim == -1) { /* 0-D */
        ...
        if (pcm->gout2a == 7)
          ...
          gaprnt(0, "Invalid dimension environment for Shapefile: \n");
          gaprnt(0, "  Longitude and Latitude must be varying \n");
        } else if (pcm->gout2b == 5 && pcm->numgrd > 1)
          gascat(pcm);
        else if (pcm->gout1 == 1)
          gagrph(pcm, 0);
        else if (pcm->gout1 == 2)
        ...
        if (pcm->numgrd == 1) {
          if (pcm->gout2a == 1)
            gacntr(pcm, 0, 0); /* contour */
          else if (pcm->gout2a == 2)
            gacntr(pcm, 1, 0); /* shaded */
            ...
            gafwrt(pcm); /* fwrite */
          else if (pcm->gout2a == 10)
            gacntr(pcm, 2, 0); /* grfill */
          else if (pcm->gout2a == 12)
            gagtif(pcm, 0); /* geotiff */
          else if (pcm->gout2a == 13 && pcm->kmlflg == 1)
            gagtif(pcm, 1); /* kml image output */
          else if (pcm->gout2a == 13 && pcm->kmlflg > 1)
            gakml(pcm); /* kml contours or polygons */
          else if (pcm->gout2a == 14)
            gacntr(pcm, 3, 0); /* imap */
          else if (pcm->gout2a == 15)
            gashpwrt(pcm); /* shapefile */
          else if (pcm->gout2a == 16)
            gacntr(pcm, 4, 0); /* gxshad2   */
          else if (pcm->gout2a == 17)
            gacntr(pcm, 5, 0); /* gxshad2b  */
          else {
      ...
      if (stn->idim == 0 && stn->jdim == 1) {
        if (pcm->goutstn == 1 || pcm->goutstn == 2 || pcm->goutstn == 6)
          gapstn(pcm);
        else if (pcm->goutstn == 3)
          gafstn(pcm);
        else if (pcm->goutstn == 4)
          gapmdl(pcm);
        else if (pcm->goutstn == 7)
          gasmrk(pcm);
        else if (pcm->goutstn == 8)
          gastnwrt(pcm);
        else if (pcm->goutstn == 9)
          gashpwrt(pcm); /* shapefile */
        else
          gawsym(pcm);
      } else if (stn->idim == 2 && stn->jdim == -1)
        gapprf(pcm);
      else if (stn->idim == 3 && stn->jdim == -1)
        gatser(pcm);
      else
...
void gaoutgds(struct gacmn *pcm) {
  ...
  while (currpt[0]) {
    ...
    for (i = 0; i < numvars; i++) {
      if (varlevels[i])
        continue;
      if (currpt[i] == NULL || currpt[i]->lat != ref->lat || currpt[i]->lon != ref->lon || currpt[i]->tim != ref->tim) {
        gaprnt(0, "error: bad structure in result\n");
        retval = 1;
        goto cleanup;
      }
      if (sendind) {
        snprintf(pout, 1255, "[%s: %f]  ", pcm->result[i].stn->pvar->abbrv, currpt[i]->val);
        gaprnt(0, pout);
  ...
  if (pcm->type[0] == 1) {
    ...
    while (rpt) {
      snprintf(pout, 1255, "%c%c%c%c%c%c%c%c %-9.4g %-9.4g %-9.4g \n", rpt->stid[0], rpt->stid[1], rpt->stid[2], rpt->stid[3],
               rpt->stid[4], rpt->stid[5], rpt->stid[6], rpt->stid[7], rpt->lon, rpt->lat, rpt->lev);
      gaprnt(2, pout);
      if (pcm->prstr) {
...
void gawsym(struct gacmn *pcm) {
  ...
  while (rpt != NULL) {
    if (rpt->umask != 0) {
      ...
      if (rlon > pcm->dmax[0])
        rlon -= 360.0;
      if (rlon > pcm->dmin[0] && rlon < pcm->dmax[0] && rpt->lat > pcm->dmin[1] && rpt->lat < pcm->dmax[1]) {
        i = (gaint)(rpt->val + 0.1);
        if (i > 0 && i < 42) {
          gxconv(rlon, rpt->lat, &x, &y, 2);
          scl = pcm->digsiz * 1.5;
  ...
  if (pcm->pass == 0 && pcm->grdsflg)
    gxchpl("GrADS/COLA", 10, 0.05, 0.05, 0.1, 0.09, 0.0);
  if (pcm->pass == 0 && pcm->timelabflg)
    gatmlb(pcm);
  gxstyl(1);
  gaaxpl(pcm, 0, 1);
  gagsav(12, pcm, NULL);
...
void gasmrk(struct gacmn *pcm) {
  ...
  if (dequal(stn->smin, stn->undef, 1e-12) == 0 || dequal(stn->smax, stn->undef, 1e-12) == 0)
    return;
  ...
  if (pcm->numgrd > 1 && (pcm->goutstn == 2 || pcm->goutstn == 6)) {
    ...
    while (rpt != NULL) {
      if (rpt->umask != 0) {
        rpt2 = stn2->rpt;
        while (rpt2 != NULL) {
          if (rpt2->umask != 0 && dequal(rpt->lat, rpt2->lat, 1e-12) == 0 && dequal(rpt->lon, rpt2->lon, 1e-12) == 0) {
            rlon = rpt->lon;
            if (rlon < pcm->dmin[0])
              rlon += 360.0;
            if (rlon > pcm->dmax[0])
            ...
            if (rlon > pcm->dmin[0] && rlon < pcm->dmax[0] && rpt->lat > pcm->dmin[1] && rpt->lat < pcm->dmax[1]) {
              ...
              if (pcm->goutstn == 2) {
                ...
                if (pcm->hemflg == 1)
                  hemflg = 1;
                else if (pcm->hemflg == 0)
                  hemflg = 0;
                else if (rpt->lat < 0.0)
                  hemflg = 1;
                gabarb(x, y, pcm->digsiz * 3.5, pcm->digsiz * 2.0, pcm->digsiz * 0.25, dir, spd, hemflg, pcm->barbolin);
...
void gabarb(gadouble x, gadouble y, gadouble plen, gadouble blen, gadouble rad, gadouble dir, gadouble spd, gaint hemflg,
            gaint fillflg) {
  ...
  if (var < 0.01) {
    rad *= 2.0;
    if (rad + blen * 0.3 > rad * 1.4)
      gxmark(2, x, y, rad + blen * 0.3);
    else
      gxmark(2, x, y, rad * 1.4);
    return;
  } else {
    if (var < 2.5)
      padd = bgap;
    ...
    if (rpt1->umask == 0) {
      ...
      for (i = 1; i < pcm->numgrd; i++) {
        if (pcm->type[i] != 1) {
          ...
          rpt = stn->rpt;
          while (rpt) {
            if (dequal(rpt->lon, rpt1->lon, 1e-12) == 0 && dequal(rpt->lat, rpt1->lat, 1e-12) == 0 &&
...
gaint gasmdl(struct gacmn *pcm, struct garpt *rpt, gadouble *vals, char *udefs) {
  ...
  if (*(udefs + 2)) {
    ...
    while (*(ch + len))
      len++;
    if (xlo[0] > -999.0)
      ylo[1] = yhi[0] + digsiz * 0.4;
    else if (xlo[5] > -999.0)
      ylo[1] = yhi[5] + digsiz * 0.4;
    else
      ylo[1] = y + digsiz * 0.3;
    t = ylo[1] - y;
    t = rad * rad - t * t;
  ...
  if (*(udefs + 3)) {
    ...
    while (*(ch + len))
      len++;
    if (xlo[0] > -999.0)
      yhi[2] = ylo[0] - digsiz * 0.4;
    else if (xlo[5] > -999.0)
      yhi[2] = ylo[5] - digsiz * 0.4;
    else
      yhi[2] = y - digsiz * 0.3;
    t = y - yhi[2];
    t = rad * rad - t * t;
  ...
  if (*(udefs + 5)) {
    if (*(vals + 5) > 0.0)
      snprintf(ch, 19, "+%.*f", pcm->dignum, (gafloat) * (vals + 5));
    else
      snprintf(ch, 19, "%.*f", pcm->dignum, (gafloat) * (vals + 5));
    len = 0;
    while (*(ch + len))
  ...
  if (pcm->stidflg) {
    len = 4;
    if (xlo[4] > -990.0)
      yhi[6] = ylo[4] - digsiz * 0.4;
    else
      yhi[6] = y - rad;
    if (xlo[2] > -990.0)
      xlo[6] = xhi[2] + 0.6 * digsiz;
    else
      xlo[6] = x - 1.2 * digsiz;
  ...
  if (orad < -990.0) {
    ...
    if (plen < pcm->digsiz * 3.5)
      plen = pcm->digsiz * 3.5;
    if (plen > pcm->digsiz * 6.0)
      plen = orad;
    wrad = orad;
  }
...
gadouble wndexit(gadouble uu, gadouble vv, gadouble x, gadouble y, gadouble rad, gadouble *xlo, gadouble *xhi, gadouble *ylo,
                 gadouble *yhi) {
  ...
  if (dequal(u, 0.0, 1e-12) == 0 && dequal(v, 0.0, 1e-12) == 0)
    return (orad);
  if (u < 0.0) {
  ...
  if (pcm->tser == 1) {
    ...
    while (rpt != NULL) {
      if (rpt->umask != 0 && rpt2->umask != 0) {
        ...
        if (pcm->hemflg == 1)
          ...
          hemflg = 1;
        gabarb(x, pcm->ysiz1, pcm->digsiz * 3.5, pcm->digsiz * 2.0, pcm->digsiz * 0.25, dir, hypot(rpt->val, rpt2->val), hemflg,
               pcm->barbolin);
      }
      rpt = rpt->rpt;
      rpt2 = rpt2->rpt;
    ...
    if (pcm->cstyle != 0) {
      ...
      while (rpt != NULL) {
        if (!cmpch(frpt->stid, rpt->stid, 8)) {
          if (rpt->umask != 0) {
            if (rpt->tim - tsav > 1.0 && !pcm->miconn)
              ipen = 3;
            if (pcm->rotate)
              gxconv(rpt->val, rpt->tim, &x, &y, 3);
            else
              gxconv(rpt->tim, rpt->val, &x, &y, 3);
            gxplot(x, y, ipen);
            ipen = 2;
        ...
        if (!cmpch(frpt->stid, rpt->stid, 8)) {
          if (rpt->umask != 0) {
            ...
            if (im == 1 || im == 2 || im == 4) {
              ...
              gxcolr(bcol);
              if (im == 1)
                gxmark(4, x, y, pcm->digsiz + 0.01);
              else
                gxmark(im + 1, x, y, pcm->digsiz + 0.01);
              gxcolr(pcm->ccolor);
            }
...
void gapprf(struct gacmn *pcm) {
  ...
  while (rpt != NULL) {
    if (rpt->umask == 0) {
      ...
      if (pcm->rotate)
        gxconv(rpt->lev, rpt->val, &x, &y, 2);
      else
        gxconv(rpt->val, rpt->lev, &x, &y, 2);
      if (cmpch(stid, rpt->stid, 8))
        ipen = 3;
      gxplot(x, y, ipen);
      ipen = 2;
    }
...
void gas1d(struct gacmn *pcm, gadouble cmin, gadouble cmax, gaint dim, gaint rotflg, struct gagrid *pgr, struct gastn *stn) {
  ...
  if (pcm->aflag == -1 || (rotflg && pcm->pass > 0)) {
    ...
    if ((cmin > (pcm->rmin - pcm->rint * 2.0)) && (cmax < (pcm->rmax + pcm->rint * 2.0))) {
      cmn = pcm->rmin;
      cmx = pcm->rmax;
      cint = pcm->rint;
      axmov = 0;
...
void gas2d(struct gacmn *pcm, struct gagrid *pgr, gaint imap) {
  ...
  if (idim == 0 && jdim == 1) {
    gamscl(pcm); /* Do map level scaling */
    if (imap)
      gawmap(pcm, 1); /* Draw map if requested */
  } else {
    if (pcm->paflg) {
      pcm->xsiz1 = pcm->pxmin;
      pcm->xsiz2 = pcm->pxmax;
...
void galfil(struct gacmn *pcm) {
  ...
  if (pgr1->idim != pgr2->idim || pgr1->jdim != -1 || pgr2->jdim != -1 || gagchk(pgr1, pgr2, pgr1->idim)) {
  ...
  while (1) {
    if (i >= pgr1->isiz)
      break;
    if (*gr1u != 0 && *gr2u != 0 && *(gr1u + 1) != 0 && *(gr2u + 1) != 0 && (*gr1 != *gr2 || *(gr1 + 1) != *(gr2 + 1)))
      break;
    i++;
    gr1++;
    gr2++;
...
void lfout(gadouble *u, gadouble *v1, gadouble *v2, gadouble *xyb, gaint cnt, gaint rotflg) {
  ...
  for (i = 0; i < cnt; i++) {
    if (rotflg)
      gxconv(*(v1 + i), *(u + i), xy, xy + 1, 3);
    else
      gxconv(*(u + i), *(v1 + i), xy, xy + 1, 3);
    j++;
    xy += 2;
  }
  i = cnt - 1;
  if (*(v1 + i) != *(v2 + i)) {
    if (rotflg)
      gxconv(*(v2 + i), *(u + i), xy, xy + 1, 3);
    else
      gxconv(*(u + i), *(v2 + i), xy, xy + 1, 3);
    j++;
    xy += 2;
  }
  for (i = 2; i < cnt; i++) {
    ii = cnt - i;
    if (rotflg)
      gxconv(*(v2 + ii), *(u + ii), xy, xy + 1, 3);
    else
      gxconv(*(u + ii), *(v2 + ii), xy, xy + 1, 3);
    j++;
    xy += 2;
  }
  if (*v1 != *v2) {
    if (rotflg)
      gxconv(*v2, *u, xy, xy + 1, 3);
    else
      gxconv(*u, *v2, xy, xy + 1, 3);
    j++;
    xy += 2;
  }
  gxfill(xyb, j);
...
void gagrph(struct gacmn *pcm, gaint barflg) {
  ...
  if (pgr->umin == 0) {
    ...
    gxcolr(1);
    if (pcm->dwrnflg)
      gxchpl("Entire Grid Undefined", 21, 3.0, 4.5, 0.3, 0.25, 0.0);
    return;
  }
  rmin = pgr->rmin;
  rmax = pgr->rmax;
  ...
  if (pcm->numgrd > 1) {
    flag = 1;
    pgr2 = pcm->result[1].pgr;
    if (pgr2->idim != pgr->idim || pgr2->jdim != pgr->jdim || gagchk(pgr2, pgr, pgr->idim) || gagchk(pgr2, pgr, pgr->jdim)) {
      flag = 0;
    ...
    if (pcm->numgrd > 2 && flag == 1) {
      ...
      if (pgr3->idim != pgr->idim || pgr3->jdim != pgr->jdim || gagchk(pgr3, pgr, pgr->idim) || gagchk(pgr3, pgr, pgr->jdim)) {
        flag = 1;
    ...
    if (pcm->cstyle != 0 && flag < 2) {
      ...
      for (i = 1; i <= pgr->isiz; i++) {
        if (*gru == 0) {
          ...
          if (rotflg)
            gxconv(*gr, (gadouble)i, &x, &y, 3);
          else
            gxconv((gadouble)i, *gr, &x, &y, 3);
          gxplot(x, y, ip);
          ip = 2;
        }
        gr++;
    ...
    if (im > 0 || flag == 2) {
      ...
      if (flag == 2) { /* if vectors/barbs, get ready */
        ...
        if (!pcm->arrflg) {
          ...
          gamnmx(pgr3);
          umax = pgr2->rmax;
          if (umax < fabs(pgr2->rmin))
            umax = fabs(pgr2->rmin);
          vmax = pgr3->rmax;
          if (vmax < fabs(pgr3->rmin))
            vmax = fabs(pgr3->rmin);
          vscal = hypot(umax, vmax);
          if (vscal > 0.0) {
            x = floor(log10(vscal));
      ...
      for (i = 1; i <= pgr->isiz; i++) {
        if (*gru != 0) {
          ...
          if (flag == 2) { /* handle vectors/barbs */
            ...
            if (*gr2u != 0 && *gr3u != 0) {
              if (rotflg)
                gxgrmp(*gr, (gadouble)i, &xx, &yy);
              else
                gxgrmp((gadouble)i, *gr, &yy, &xx);
              hemflg = 0;
              if (hflg == 0 && yy < 0.0)
                hemflg = 1;
              if (hflg == 3)
              ...
              if (*gr2 == 0.0 && *gr3 == 0.0)
                ...
                dir = atan2(*gr3, *gr2);
              if (pcm->gout1a == 2) {
                gabarb(x, y, pcm->digsiz * 3.5, pcm->digsiz * 2.0, pcm->digsiz * 0.25, dir, hypot(*gr2, *gr3), hemflg,
                       pcm->barbolin);
              } else {
                if (vscal > 0.0) {
            ...
            if (im == 1 || im == 2 || im == 4 || im == 7 || im == 8) {
              ...
              if (im == 1)
                ...
                gxmark(12, x, y, pcm->digsiz + 0.01); /* draw a closed diamond */
              else
                gxmark(im + 1, x, y, pcm->digsiz + 0.01);
              gxcolr(pcm->ccolor);
            }
...
void gastrm(struct gacmn *pcm) {
  ...
  if (pgru->idim != pgrv->idim || pgru->jdim != pgrv->jdim || gagchk(pgru, pgrv, pgru->idim) || gagchk(pgru, pgrv, pgru->jdim)) {
  ...
  if (pcm->numgrd > 2) {
    ...
    if (pgrc->idim != pgru->idim || pgrc->jdim != pgru->jdim || gagchk(pgrc, pgru, pgru->idim) || gagchk(pgrc, pgru, pgru->jdim)) {
      flag = 0;
  ...
  if ((pcm->rotate && (pgru->idim != 2 || pgru->jdim != 3)) || (!pcm->rotate && pgru->idim == 2 && pgru->jdim == 3)) {
    pgru = gaflip(pgru, pcm);
    pgrv = gaflip(pgrv, pcm);
    if (flag)
      pgrc = gaflip(pgrc, pcm);
  }
  ...
  if (flag) {
    ...
    if (pgrc->umin == 0) {
      ...
      gaselc(pcm, pgrc->rmin, pgrc->rmax);
  }
  ...
  if (flag) {
    gxstrm(u, v, c, pgru->isiz, pgru->jsiz, umask, vmask, cmask, flag, pcm->shdlvs, pcm->shdcls, pcm->shdcnt, pcm->strmden,
           pcm->strmarrd, pcm->strmarrsz, pcm->strmarrt);
  } else {
    gxstrm(u, v, NULL, pgru->isiz, pgru->jsiz, umask, vmask, 0, flag, pcm->shdlvs, pcm->shdcls, pcm->shdcnt, pcm->strmden,
           pcm->strmarrd, pcm->strmarrsz, pcm->strmarrt);
  }
...
void gavect(struct gacmn *pcm, gaint brbflg) {
  ...
  if (!pcm->arrflg) {
    umax = pgru->rmax;
    if (umax < fabs(pgru->rmin))
      umax = fabs(pgru->rmin);
    vmax = pgrv->rmax;
    if (vmax < fabs(pgrv->rmin))
      vmax = fabs(pgrv->rmin);
    vscal = hypot(umax, vmax);
    if (vscal > 0.0) {
      x = floor(log10(vscal));
      ...
      if (*umask != 0 && *vmask != 0) {
        ...
        if (lcol > -1) {
          if (*v == 0.0 && *u == 0.0)
            ...
            dir = atan2(*v, *u);
          if (brbflg) {
            gabarb(x, y, pcm->digsiz * 3.5, pcm->digsiz * 2.0, pcm->digsiz * 0.25, dir + adj, hypot(*u, *v), hemflg, pcm->barbolin);
          } else {
            if (vscal > 0.0) {
...
void gascat(struct gacmn *pcm) {
  ...
  pgr1 = pcm->result[0].pgr;
  pgr2 = pcm->result[1].pgr;
  if (pgr1->idim != pgr2->idim || pgr1->jdim != pgr2->jdim || gagchk(pgr1, pgr2, pgr1->idim) ||
  ...
  if (pcm->numgrd > 2) {
    flag = 1;
    pgrc = pcm->result[2].pgr;
    if (pgrc->idim != pgr1->idim || pgrc->jdim != pgr2->jdim || gagchk(pgrc, pgr1, pgr1->idim) || gagchk(pgrc, pgr2, pgr2->jdim)) {
      flag = 0;
  ...
  gaprnt(2, pout);
  gxscal(pcm->xsiz1, pcm->xsiz2, pcm->ysiz1, pcm->ysiz2, cmin1, cmax1, cmin2, cmax2);
  pass = pcm->gpass[3];
  if (pass > 5)
    pass = 5;
  gxwide(pcm->cthick);
...
void gaplvl(struct gacmn *pcm) {
  ...
  if (pcm->numgrd > 1) {
    ...
    if (pgrm->idim != pgr->idim || pgrm->jdim != pgr->jdim || gagchk(pgrm, pgr, pgr->idim) || gagchk(pgrm, pgr, pgr->jdim)) {
      flag = 0;
  ...
  if ((pcm->rotate && (pgr->idim != 2 || pgr->jdim != 3)) || (!pcm->rotate && pgr->idim == 2 && pgr->jdim == 3)) {
    pgr = gaflip(pgr, pcm);
    if (flag)
      pgrm = gaflip(pgrm, pcm);
  }
  ...
  for (j = 1; j <= pgr->jsiz; j++) {
    for (i = 1; i <= pgr->isiz; i++) {
      if (*rmask != 0) {
        ...
        gxchln(lab, len, pcm->digsiz, &cwid);
        gxchpl(lab, len, xlo - cwid * 0.5, ylo - pcm->digsiz * 0.5, pcm->digsiz, pcm->digsiz, 0.0);
      }
      r++;
      rmask++;
      if (flag) {
    ...
    if (grid) {
      ...
      while (rpt != NULL) {
        if (rpt->umask != 0) {
          ...
          if (lon > pcm->dmin[0] && lon < pcm->dmax[0] && lat > pcm->dmin[1] && lat < pcm->dmax[1]) {
            /* get the data value */
            val = rpt->val;
            /* create the shape, write it to the file, then release it */
            shp = SHPCreateObject(SHPT_POINTM, shpid, nParts, pstart, NULL, 1, &lon, &lat, NULL, &val);
...
*/
#if USESHP == 1
struct dbfld *newdbfld(char *fldname, DBFFieldType dbtype, gaint len, gaint prec, gaint flag, char *val) {
  gaint sz;
  struct dbfld *newfld;
  char *value;
...
cleanup:
  ...
  if (kmlfp)
    fclose(kmlfp); /* close the file */
  if (err)
    gaprnt(0, "Error from fwrite when writing KML file\n");
  return;
}
...
void ij2ll(struct gacmn *pcm, gadouble i, gadouble j, gadouble gx, gadouble gy, gadouble *tiepoints, gaint index) {
  gadouble (*conv)(gadouble *, gadouble);
  gadouble lon, lat;
...
void gafwrt(struct gacmn *pcm) {
  ...
  if (pcm->ffile == NULL) {
    ...
    if (pcm->fwname) {
      if (stat(pcm->fwname, &s) == 0)
        fexists = 1;
      else
        fexists = 0;
    } else {
      if (stat("grads.fwrite", &s) == 0)
        fexists = 1;
      else
        fexists = 0;
    }
    ...
    if (pcm->fwname)
      pcm->ffile = fopen(pcm->fwname, fmode);
    else
      pcm->ffile = fopen("grads.fwrite", fmode);
    if (pcm->ffile == NULL) {
  ...
  if (pcm->fwexflg && pgr->idim < 2 && pgr->jdim < 2) {
    if (pcm->xexflg) {
      if (pcm->x1ex != pgr->dimmin[0])
        diff = 1;
      if (pcm->x2ex != pgr->dimmax[0])
        diff = 1;
    }
    if (pcm->yexflg) {
      if (pcm->y1ex != pgr->dimmin[1])
        diff = 1;
      if (pcm->y2ex != pgr->dimmax[1])
        diff = 1;
    }
  }
  ...
  if (diff) {
    ...
    if (BYTEORDER != pcm->fwenflg)
      gabswp(&rdw, 1);
    if (pcm->fwsqflg)
      fwrite(&rdw, sizeof(gaint), 1, pcm->ffile);
    if (pgr->idim == 1) {
      goff = yoff;
    } else {
  ...
  if (pcm->ffile != stdout) {
    ...
    if (pcm->fwsqflg)
      gaprnt(2, " as Sequential");
    else
      gaprnt(2, " as Stream");
    if (pcm->fwenflg)
      gaprnt(2, " Big_Endian\n");
    else
      gaprnt(2, " Little_Endian\n");
  }
...
gaint gacntr(struct gacmn *pcm, gaint filflg, gaint shpflg) {
  ...
  if (filflg == 3 && (pcm->mproj < 1 || pcm->mproj > 2 || pgr->idim != 0 || pgr->jdim != 1)) {
  ...
  if (shpflg == 0) {
    ...
    gxwide(1);
    if (filflg)
      gas2d(pcm, pgr, 0); /* No map yet if shaded cntrs */
    else
      gas2d(pcm, pgr, 1); /* Scaling plus map */
  }
  ...
  if (filflg) {
    ...
    if (smooth) {
      if (filflg == 1) {
        ...
        gaimap(rrr, isz, jsz, pcm->shdlvs, pcm->shdcls, pcm->shdcnt, rrrmask, pcm->xsiz1, pcm->xsiz2, pcm->ysiz1, pcm->ysiz2);
      } else if (filflg == 4) {
      ...
      if (filflg == 1) {
        ...
        gaimap(pgr->grid, pgr->isiz, pgr->jsiz, pcm->shdlvs, pcm->shdcls, pcm->shdcnt, pgr->umask, pcm->xsiz1, pcm->xsiz2,
               pcm->ysiz1, pcm->ysiz2);
      } else if (filflg == 4) {
    ...
    if (pgr->idim == 0 && pgr->jdim == 1 && shpflg == 0)
      gawmap(pcm, 0);
  } else {
    gxwide(pcm->cthick);
    cntrcnt = 0;
    ...
    if (pcm->cflag) { /* user has specified contour levels */
      for (i = 0; i < pcm->cflag; i++) {
        rr = pcm->clevs[i];
        if (rr < 0.0 && pcm->cstyle == -9)
          gxstyl(3);
        else
          gxstyl(pcm->cstyle);
        if (pcm->ccolor < 0 && pcm->ccflg == 0) { /* user has not specified contour colors */
          if (pcm->cflag == 1)
            ii = irb / 2; /* only one specified level */
          else
            ii = (gaint)((gadouble)(i * irb) / ((gadouble)(pcm->cflag - 1)));
          if (ii > irb)
            ii = irb;
          if (ii < 0)
            ii = 0;
        ...
        if (smooth) {
          ...
          gxclev(pgr->grid, pgr->isiz, pgr->jsiz, 1, pgr->isiz, 1, pgr->jsiz, rr, pgr->umask, &scntr);
        }
      ...
      if (fabs(cmin / cint) < 1e6 || fabs(cmax / cint) < 1e6)
        clopt = 1;
      for (rl = cmin; rl <= cmax + (cint / 2.0); rl += cint) {
        if (dequal(rl, 0.0, 1e-15) == 0)
          rl = 0.0; /* a quick patch */
        if (rl < pcm->cmin || rl > pcm->cmax)
          continue;
        if (pcm->blkflg && rl >= pcm->blkmin && rl <= pcm->blkmax)
          continue;
        rr = rl;
        if (rr < 0.0 && pcm->cstyle == -9)
          gxstyl(3);
        ...
        if (clopt) {
          ...
          ii = (gaint)(tt + 0.009);
          ii2 = (gaint)(tt - 0.009);
          if (fabs(tt - (gadouble)ii) < 0.01 || fabs(tt - (gadouble)ii2) < 0.01)
            clflg = 1;
        } else {
          if (clcnt == pcm->clskip) {
            clflg = 1;
...
void gaconv(gadouble s, gadouble t, gadouble *x, gadouble *y) {
  ...
  if (iconv == NULL)
    ...
    *x = iconv(ivars, (s + ioffset));
  if (jconv == NULL)
    *y = t + joffset;
  else
    *y = jconv(jvars, (t + joffset));
}
...
void gaaxis(gaint axis, struct gacmn *pcm, gaint dim) {
  ...
  if (axis == 1) {
    ...
    thck = pcm->xlthck;
    if (pcm->xlside)
      pos = pcm->ysiz2 + pcm->xlpos;
    else
      pos = pcm->ysiz1 + pcm->xlpos;
    if (pcm->xlpos != 0.0) {
    ...
    thck = pcm->ylthck;
    if (pcm->ylside)
      pos = pcm->xsiz2 + pcm->ylpos;
    else
      pos = pcm->xsiz1 + pcm->ylpos;
    if (pcm->ylpos != 0.0) {
  ...
  if (dim != 3) {
    ...
    if (axis == 1 && pcm->xlflg > 0)
      cnt = pcm->xlflg;
    else if (axis == 0 && pcm->ylflg > 0)
      cnt = pcm->ylflg;
    else {
    ...
    while (i < cnt) {
      ...
      if (axis == 1 && pcm->xlstr)
        ...
        if (dim == 0 && pcm->mproj > 0)
          len = galnch(v, lab);
        else if (dim == 1 && pcm->mproj > 0)
          len = galtch(v, lab);
        else
          snprintf(lab, 29, "%g", v);
      }
      ...
      if (axis) {
        ...
        if (pcm->ylside) {
          ...
          gxplot(pos - (cs * 0.4), y, 2);
          x = pos - (cwid + cs) * 0.8;
          if (pcm->yllow < (cwid + cs) * 0.8)
            pcm->yllow = (cwid + cs) * 0.8;
        }
        /* X grid lines */
    ...
    while (timdif(&tstrt, &(pcm->tmax), 0) > -1L) {
      ...
      if (tincr.dy > 3 && tstrt.dy == 3 && (tstrt.dy == 2 || tstrt.dy == 3))
        tstrt.dy = 1;
    }
  }
}
...
void gamscl(struct gacmn *pcm) {
  ...
  if (pcm->mproj == 2 || flag)
    ...
    rc = gxscld(&mpj, pcm->xflip, pcm->yflip);
  if (rc) {
...
void gawmap(struct gacmn *pcm, gaint iflg) {
  ...
  if (pcm->mproj == 0 || (pcm->pass > 0 && iflg))
    return;
  if (pcm->mpdraw == 0)
    return;
  gxclip(pcm->xsiz1, pcm->xsiz2, pcm->ysiz1, pcm->ysiz2);
...
void gacsel(gadouble rmin, gadouble rmax, gadouble *cint, gadouble *cmin, gadouble *cmax) {
  ...
  if (*cint == 0.0) {   /* no precision check */
    ...
    if (norml >= 1.0 && norml <= 1.5)
      *cint = 1.0;
    else if (norml > 1.5 && norml <= 2.5)
      *cint = 2.0;
    else if (norml > 2.5 && norml <= 3.5)
      *cint = 3.0;
    else if (norml > 3.5 && norml <= 7.5)
      *cint = 5.0;
    else
      *cint = 10.0;
    *cint = *cint * w1;
  ...
  *cmax = *cint * floor(rmax / (*cint));
  ...
  t1 = rmin + *cint;
  t2 = rmax - *cint;
  if ((dequal(rmin, t1, 1.0e-16) == 0) || (dequal(rmax, t2, 1.0e-16) == 0)) {
    *cint = 0.0;
    *cmin = 0.0;
    *cmax = 0.0;
  }
...
void gaglin(gadouble *g1, gaint cols, gaint rows, gadouble *g2, gaint exp1, gaint exp2, char *g1u, char *g2u) {
  ...
  for (j = 0; j < jsiz; j++) {
    for (i = 0; i < isiz; i++) {
      ...
      if (y < 0.0)
        y = 0.0;
      if (x >= (gadouble)cols)
        x = (gadouble)cols - 0.001; /* fudge the edges  */
      if (y >= (gadouble)rows)
        y = (gadouble)rows - 0.001; /* check with bdoty */
...
void gagexp(gadouble *g1, gaint cols, gaint rows, gadouble *g2, gaint exp1, gaint exp2, char *g1u, char *g2u) {
...
gaint gatinc(struct gacmn *pcm, struct dt *tstrt, struct dt *tincr) {
  ...
  if (tdif > 1576800L)
    tincr->mo = 6L;
  else if (tdif > 788400L)
    tincr->mo = 3L;
  else if (tdif > 262800L)
    tincr->mo = 1L;
  else if (tdif > 129600L)
    tincr->dy = 15L;
  else if (tdif > 42000L)
    tincr->dy = 5L;
  else if (tdif > 14399L)
...
void gampax(struct gacmn *pcm) {
  ...
  if (pcm->mproj == 5) {
    ...
    /* these are labels when lon ranges from 0 to 360 */
      ...
      gxconv(180.0, 90.0, &x1, &y1, 2);
      gxchpl("180", 3, x1 - cs * 1.5, y1 + cs * 0.6, cs * 1.1, cs, 0.0);
      gxconv(270.0, 90.0, &x1, &y1, 2);
      gxchpl("90W", 3, x1 - cs * 1.5, y1 + cs * 0.6, cs * 1.1, cs, 0.0);
      gxconv(360.0, 90.0, &x1, &y1, 2);
      gxchpl("0", 1, x1 - cs * 0.5, y1 + cs * 0.6, cs * 1.1, cs, 0.0);
      ...
      gxconv(0.0, -60.0, &x1, &y1, 2);
      gxchpl("60S", 3, x1 - cs * 4.5, y1 - cs * 0.55, cs * 1.1, cs, 0.0);
      gxconv(0.0, -30.0, &x1, &y1, 2);
      gxchpl("30S", 3, x1 - cs * 4.0, y1 - cs * 0.55, cs * 1.1, cs, 0.0);
      gxconv(0.0, 0.0, &x1, &y1, 2);
      gxchpl("EQ", 2, x1 - cs * 2.7, y1 - cs * 0.55, cs * 1.1, cs, 0.0);
      gxconv(0.0, 30.0, &x1, &y1, 2);
      gxchpl("30N", 3, x1 - cs * 4.0, y1 - cs * 0.55, cs * 1.1, cs, 0.0);
      gxconv(0.0, 60.0, &x1, &y1, 2);
      ...
      gxconv(360.0, -60.0, &x1, &y1, 2);
      gxchpl("60S", 3, x1 + cs * 1.5, y1 - cs * 0.55, cs * 1.1, cs, 0.0);
      gxconv(360.0, -30.0, &x1, &y1, 2);
      gxchpl("30S", 3, x1 + cs * 1.0, y1 - cs * 0.55, cs * 1.1, cs, 0.0);
      gxconv(360.0, 0.0, &x1, &y1, 2);
      gxchpl("EQ", 2, x1 + cs * 0.7, y1 - cs * 0.55, cs * 1.1, cs, 0.0);
      gxconv(360.0, 30.0, &x1, &y1, 2);
      gxchpl("30N", 3, x1 + cs * 1.0, y1 - cs * 0.55, cs * 1.1, cs, 0.0);
      gxconv(360.0, 60.0, &x1, &y1, 2);
  ...
  if (pcm->xlint != 0.0)
    lnincr = pcm->xlint; /* set the lon increment from xlint */
  ...
  if (pcm->ylint != 0.0)
    ltincr = pcm->ylint; /* set the lat increment from ylint */
...
void wxsym(gaint type, gadouble xpos, gadouble ypos, gadouble scale, gaint colr, gaint *wxcols) {
  ...
  if (type == 40 || type == 42)
    gxmark(2, x, y, scale * 0.4);
  if (type == 41 || type == 43)
    gxmark(3, x, y, scale * 0.4);
}
...
void gagsav(gaint type, struct gacmn *pcm, struct gagrid *pgr) { pcm->lastgx = type; }
...
void gaimap(gadouble *r, gaint is, gaint js, gadouble *vs, gaint *clrs, gaint lvs, char *ru, gadouble xlo, gadouble xhi,
            gadouble ylo, gadouble yhi) {
  ...
  for (i = 0; i < iz; i++) {
    ...
    ii = 1;
    while (ii < is) {
      if ((*(xvals + ii) >= x && *(xvals + ii - 1) <= x) || (*(xvals + ii) <= x && *(xvals + ii - 1) >= x)) {
        if ((fabs(*(xvals + ii) - x)) > (fabs(*(xvals + ii - 1) - x)))
          ii = ii - 1;
        break;
      }
      ii++;
  ...
  for (j = 0; j < jz; j++) {
    ...
    jj = 1;
    while (jj < js) {
      if ((*(yvals + jj) >= y && *(yvals + jj - 1) <= y) || (*(yvals + jj) <= y && *(yvals + jj - 1) >= y)) {
        if ((fabs(*(yvals + jj) - y)) > (fabs(*(yvals + jj - 1) - y)))
          jj = jj - 1;
        break;
      }
      jj++;
...
void gaselc(struct gacmn *pcm, gadouble rmin, gadouble rmax) {
  ...
  if (!pcm->cflag) {
    if (pcm->cint == 0.0) { /* no precision check */
      if (pcm->rbflg)
        rdif = rdif / ((gadouble)(pcm->rbflg) - 0.5);
      else
        rdif = rdif / 12.5;
      w1 = floor(log10(rdif));
      w1 = pow(10.0, w1);
      ...
      if (norml < 2.0)
        fact = 10.0;
      else if (norml >= 2.0 && norml < 4.0)
        fact = 5.0;
      else if (norml >= 4.0 && norml < 7.0)
        fact = 2.0;
      else
        fact = 1.0;
      norml *= fact;
    ...
    for (rr = cmin - cint; rr <= cmax + (cint / 2.0); rr += cint) {
      ...
      if (pcm->shdcls[lcnt] == 1) {
        ...
        if (ii < 0)
          ii = 0;
        if (pcm->rbflg)
          pcm->shdcls[lcnt] = pcm->rbcols[ii];
        else
          pcm->shdcls[lcnt] = rcols[ii];
      }
      if (lcnt < 1 || pcm->shdcls[lcnt] != pcm->shdcls[lcnt - 1])
        lcnt++;
    }
    ...
    if (rmin < pcm->clevs[0])
      pcm->shdlvs[0] = rmin - 100.0;
    else
      pcm->shdlvs[0] = pcm->clevs[0] - 100.0;
    ccnt = rrb / (gadouble)(pcm->cflag + 1);
    ci = ccnt / 2.0;
    ii = (gaint)ci;
    if (pcm->ccflg > 0)
      pcm->shdcls[0] = pcm->ccols[0];
    else {
      if (pcm->rbflg > 0)
        pcm->shdcls[0] = pcm->rbcols[ii];
      else
        pcm->shdcls[0] = rcols[ii];
    }
    lcnt = 1;
    for (i = 0; i < pcm->cflag; i++) {
...
void gatmlb(struct gacmn *pcm) {
  ...
  if (vcnt <= 2) {
    sys_time();
    snprintf(dtgstr, 31, "%04ld-%02ld-%02ld-%02ld:%02ld", timeobj.year, timeobj.month, timeobj.date, timeobj.hour, timeobj.minute);
