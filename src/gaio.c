...
void gaiomg() { msgflg = 1; }

/* Routine to change the value of the cache scale factor */
void setcachesf(long sf) { cachesf = sf; }
...
gaint gaggrd(struct gagrid *pgrid) {
  ...
  if (pgr->idim < -1 || pgr->idim > 4 || pgr->jdim < -1 || pgr->jdim > 4 || (pgr->idim == -1 && pgr->jdim != -1)) {
  ...
  jd = pgr->jdim;
  if (id > -1)
    pgr->isiz = pgr->dimmax[id] - pgr->dimmin[id] + 1;
  else
    pgr->isiz = 1;
  if (jd > -1)
    pgr->jsiz = pgr->dimmax[jd] - pgr->dimmin[jd] + 1;
  else
    pgr->jsiz = 1;
...
nodatmsg:
  if (mfcmn.warnflg > 0) {
    ...
    snprintf(pout, 1255, "  Grid limits of file:     X = 1 %i  Y = 1 %i  Z = 1 %i  T = 1 %i  E = 1 %i \n", pfi->dnum[0],
             pfi->dnum[1], pfi->dnum[2], pfi->dnum[3], pfi->dnum[4]);
    gaprnt(2, pout);
    snprintf(pout, 1255, "  Grid limits of request:  X = %i %i  Y = %i %i  Z = %i %i  T = %i %i  E = %i %i \n", pgr->dimmin[0],
             pgr->dimmax[0], pgr->dimmin[1], pgr->dimmax[1], pgr->dimmin[2], pgr->dimmax[2], pgr->dimmin[3], pgr->dimmax[3],
...
gaint gagrow(gadouble *gr, char *gru, gaint *d) {
  ...
  if (!pfi->wrap) {
    ...
    if (pgr->dimmin[0] < 1 && pgr->dimmax[0] < 1)
      return (-1);
    if (pgr->dimmin[0] > pfi->dnum[0] && pgr->dimmax[0] > pfi->dnum[0])
      return (-1);
    i = 1 - pgr->dimmin[0];
    if (i > 0) {
      gr += i;
...
gaint garrow(gaint x, gaint y, gaint z, gaint t, gaint e, gaint len, gadouble *gr, char *gru, gaint toff) {
  struct gaens *ens;
...
off_t gafcorlf(gaint x, gaint y, gaint z, gaint t, gaint e) {
  ...
  if (pfi->tlpflg) {
    ...
    if (tl > (off_t)pfi->dnum[3])
      tl = tl - (off_t)pfi->dnum[3];
  }
  ...
  pos = ltmpe + ltmpt + pvr->offset + ltmpz + ltmpy + (xl - 1);

  if (pfi->xyhdr)
    pos = pos + (off_t)pfi->xyhdr;
  return (pos);
}
...
gaint gagstn(struct gastn *stn) {
  ...
  if (!scerr)
    scok = 1;
  if (scflg && stn->pfi->fseq != -999 && scseq == stn->pfi->fseq && scstn.pfi == stn->pfi && scstn.idim == stn->idim &&
      scstn.jdim == stn->jdim && scstn.tmin == stn->tmin && scstn.tmax == stn->tmax && scstn.rflag == stn->rflag &&
      scstn.sflag == stn->sflag) {
    rc = 1;
    if (stn->rflag && scstn.radius != stn->radius)
      rc = 0;
    if (stn->sflag) {
      for (i = 0; i < 8; i++)
        if (scstn.stid[i] != stn->stid[i])
          rc = 0;
    } else {
      if (scstn.dmin[0] != stn->dmin[0])
        rc = 0;
      if (scstn.dmin[1] != stn->dmin[1])
        rc = 0;
      if (scstn.dmax[0] != stn->dmax[0])
        rc = 0;
      if (scstn.dmax[1] != stn->dmax[1])
        rc = 0;
    }
    if (rc) {
      scuca = 1;
  ...
  for (tim = stn->tmin; tim <= stn->tmax; tim++) {
    ...
    while (1) {
      ...
      if (stn->sflag) {
        ...
        if (hlon > lnmax)
          hlon -= 360.0;
        if (hlon < lnmin || hlon > lnmax || hdr->lat < ltmin || hdr->lat > ltmax)
          selflg = 0;
        if (selflg && stn->rflag && hypot(hlon - stn->dmin[0], hdr->lat - stn->dmin[1]) > stn->radius) {
          selflg = 0;
        }
      }
   ...
   and set exact missing data values if specified.*/
      if (selflg) {
        ...
        for (i = 0; i < fnum; i++) {
          if (((*(pfi->sbuf + i) >= pfi->ulow) && (*(pfi->sbuf + i) <= pfi->uhi)) || (isnan(*(pfi->sbuf + i))) ||
              (isinf(*(pfi->sbuf + i)))) {
            *(pfi->ubuf + i) = 0;
          } else {
            *(pfi->ubuf + i) = 1;
        ...
        if (scok)
          gacstn((char *)hdr, (char *)pfi->sbuf, sizd, sizhdrd);
  ...
  if (pvr->levels == 0) {
    ...
    for (i = 0; i < mlev; i++) {
      ...
      if (dequal(stn->dmax[2], stn->dmin[2], 1e-08) == 0) {
        if (fabs(*pval - stn->dmin[2]) > 0.01)
          continue;
      } else {
        if (*pval < stn->dmax[2] || *pval > stn->dmin[2])
          continue;
      }
      rpt = gaarpt(stn);
      if (rpt == NULL)
...
gaint gagdef(void) {
  ...
  for (i = 0; i < 5; i++)
    d[i] = pgr->dimmin[i] - pfi->dimoff[i] - 1;
  for (i = 0; i < 5; i++)
    if (pfi->dnum[i] == 1)
      d[i] = 0;
  if (id > -1) {
  ...
  pgr->jsiz = 1;
  if (id > -1)
    pgr->isiz = 1 + d1max - d1min;
  if (jd > -1)
    pgr->jsiz = 1 + d2max - d2min;
  ...
  flag = 0;
  for (i = 0; i < 5; i++) {
    if (i != id && i != jd && (d[i] < 0 || d[i] >= pfi->dnum[i]))
      flag = 1;
  }
  if (flag) {
    for (i = 0; i < siz; i++) {
  ...
  for (yt = d2min; yt <= d2max; yt++) {
    ...
    for (d[id] = d1min; d[id] <= d1max; d[id]++) {
      if (d[jd] < 0 || d[jd] >= pfi->dnum[jd] || d[id] < 0 || d[id] >= pfi->dnum[id]) {
        *v = pfi->undef;
        *vmask = 0;
      } else {
        pos = (off_t)d[0] + (off_t)d[1] * (off_t)ys + (off_t)d[2] * (off_t)zs + (off_t)d[3] * (off_t)ts + (off_t)d[4] * (off_t)es;
...
gaint gairow(gaint x, gaint y, gaint z, gaint t, gaint e, gaint offset, gaint len, gadouble *gr, char *gru) {
  ...
  if (pfi->idxflg == 1) {
    ...
    if (bigflg)
      bpos = *(pindxb->bigpnt + brec * 2 + 1);
    else
      bpos = (off_t)(*(pindx->intpnt + irec + 1));
    ...
    clen = cend - cstrt + 2;
    if (bigflg)
      fpos = *(pindxb->bigpnt + brec * 2);
    else
      fpos = (off_t)(*(pindx->intpnt + irec));
  ...
  if (pfi->idxflg == 2) {
    ...
    if (buff == NULL)
      buff = g2read(seek, ifld, z, t, e); /* if not in cache, read the grid */
    if (buff == NULL)
      return (1); /* give up */
...
struct g2buff *g2check(gaint z, gaint t, gaint e) {
  ...
  if (anchor != NULL) {
    ...
    while (buff != NULL) {
      if ((buff->fseq == pfi->fseq) && (buff->z == z) && (buff->t == t) && (buff->e == e) && (cmpwrd(buff->abbrv, pvr->abbrv)) &&
          (buff->size == size)) {
...
gaint gaprow(gaint x, gaint y, gaint z, gaint t, gaint e, gaint tt, gaint len, gadouble *gr, char *gru) {
          ...
          if ((nc_inq_vartype(pfi->ncid, pvr->ncvid, &type)) == NC_NOERR) {
            if ((nc_inq_type(pfi->ncid, type, NULL, &size)) == NC_NOERR) {
              ...
              if (nelems * size > cachesf * pfi->cachesize) {
                ...
                gaprnt(1, pout);
                for (i = ndims - 1; i >= 0; i--) {
                  snprintf(pout, 1255, "%ld * ", (long)chsize[i]);
                  gaprnt(1, pout);
                }
...
gaint gancgrid(gadouble *gr, char *gru, gaint id, gaint jd) {
  ...
  if (pgr->dimmin[2] >= 1 && pgr->dimmax[2] <= pfi->dnum[2]) {
    ...
    if (pfi->zrflg) {
      if (pvr->levels == 0)
        ...
        zz = pvr->levels - pgr->dimmax[2]; /* use var nlevs instead of dnum[[2] */
    } else
      zz = pgr->dimmin[2] - 1;
  ...
  for (i = pvr->nvardims - 1; i >= 0; i--) {
    ...
    if ((id != -1 && pvr->units[i] == unitsvals[id]) || (jd != -1 && pvr->units[i] == unitsvals[jd])) {
  ...
  if (dequal(pvr->undef2, 0.0, 1e-08) == 0)
    ulow2 = 1e-5;
  else
    ulow2 = fabs(pvr->undef2 / EPSILON);
  ...
  for (i = 0; i < grsize; i++) {
    if ((*(grtmp + i) >= ulow && *(grtmp + i) <= uhi) || (*(grtmp + i) >= ulow2 && *(grtmp + i) <= uhi2) || (isnan(*(grtmp + i))) ||
        (isinf(*(grtmp + i)))) {
      *(grutmp + i) = 0;
    } else {
      /* Data value is good */
  ...
  if (!wflag) {
    ...
    while (j != jlimit) {
      ...
      for (x = pgr->dimmin[0]; x <= pgr->dimmax[0]; x++, i++) {
        ...
        itmp = x;
        while (itmp < 1)
          itmp = itmp + pfi->dnum[0];
        while (itmp > pfi->dnum[0])
          itmp = itmp - (pfi->dnum[0]);
...
gaint gancrow(gaint x, gaint y, gaint z, gaint t, gaint e, gaint len, gadouble *gr, char *gru) {
  ...
  for (i = 0; i < len; i++) {
    if ((*(gr + i) >= ulow && *(gr + i) <= uhi) || (*(gr + i) >= ulow2 && *(gr + i) <= uhi2) || (isnan(*(gr + i))) ||
        (isinf(*(gr + i)))) {
      *(gru + i) = 0;
    } else {
...
gaint gahrow(gaint x, gaint y, gaint z, gaint t, gaint e, gaint len, gadouble *gr, char *gru) {
  ...
  case (DFNT_INT8): /* definition value 20 */
    ...
    if (SDreaddata(sds_id, start, NULL, count, (VOIDP *)bval) != 0) {
      ...
      for (i = 0; i < len; i++)
        gr[i] = (gadouble)bval[i]; /* Cast int8 to gadouble */
    }
    gree(bval, "f126");
    break;
  ...
  case (DFNT_UINT8): /* definition value 21 */
    ...
    if (SDreaddata(sds_id, start, NULL, count, (VOIDP *)ubval) != 0) {
      ...
      for (i = 0; i < len; i++)
        gr[i] = (gadouble)ubval[i]; /* Cast uint8 to gadouble */
    }
    gree(ubval, "f127");
    break;
  ...
  case (DFNT_INT16): /* definition value 22 */
    ...
    if (SDreaddata(sds_id, start, NULL, count, (VOIDP *)sval) != 0) {
      ...
      for (i = 0; i < len; i++)
        gr[i] = (gadouble)sval[i]; /* Cast int16 to gadouble */
    }
    gree(sval, "f128");
    break;
  ...
  case (DFNT_UINT16): /* definition value 23 */
    ...
    if (SDreaddata(sds_id, start, NULL, count, (VOIDP *)usval) != 0) {
      ...
      for (i = 0; i < len; i++)
        gr[i] = (gadouble)usval[i]; /* Cast uint16 to gadouble */
    }
    gree(usval, "f129");
    break;
  ...
  case (DFNT_INT32): /* definition value 24 */
    ...
    if (SDreaddata(sds_id, start, NULL, count, (VOIDP *)ival) != 0) {
      ...
      for (i = 0; i < len; i++)
        gr[i] = (gadouble)ival[i]; /* Cast int32 to gadouble */
    }
    gree(ival, "f130");
    break;
  ...
  case (DFNT_UINT32): /* definition value 25 */
    ...
    if (SDreaddata(sds_id, start, NULL, count, (VOIDP *)uival) != 0) {
      ...
      for (i = 0; i < len; i++)
        gr[i] = (gadouble)uival[i]; /* Cast uint32 to gadouble */
    }
    gree(uival, "f131");
    break;
  ...
  case (DFNT_FLOAT32): /* definition value  5 */
    ...
    if (SDreaddata(sds_id, start, NULL, count, (VOIDP *)fval) != 0) {
      ...
      for (i = 0; i < len; i++)
        gr[i] = (gadouble)fval[i]; /* Cast uint32 to gadouble */
    }
    gree(fval, "f131");
    break;
...
gaint h5attr(hid_t varid, char *vname, char *aname, gadouble *value) {
  ...
  if ((aspace = H5Aget_space(aid)) < 0) {
    gaprnt(2, "H5Aget_space failed\n");
    return (1);
  }
  ...
  if ((atype = H5Aget_type(aid)) < 0) {
    gaprnt(2, "H5Aget_type failed\n");
    return (1);
  }
  if ((aclass = H5Tget_class(atype)) < 0) {
    gaprnt(2, "H5Tget_class failed\n");
    return (1);
  }
  if ((asize = H5Tget_size(atype)) == 0) {
    gaprnt(2, "H5Tget_size failed\n");
    return (1);
  }
...
gaint h5pattrs(hid_t h5id, char *vname, char *abbrv, gaint hdrflg, gaint fnum, char *ftit, long cachesize) {
  ...
  if (cmpwrd("global", abbrv)) {
    ...
    if (h5openvar(h5id, vname, &dsid, &vid, cachesize))
      err = 1;
  }
  if (!err) {
  ...
  for (aindex = 0; aindex < n_atts; aindex++) {
    ...
    err = 0;
    ai = (hsize_t)aindex;
    aid = H5Aopen_by_idx(vid, ".", H5_INDEX_CRT_ORDER, H5_ITER_INC, ai, H5P_DEFAULT, H5P_DEFAULT);
    if (aid < 0)
      err = 1;
    /* get the attribute rank, make sure it is not greater than 1. */
    if (!err)
    ...
    if (!err)
      stosize = H5Aget_storage_size(aid);
    if (stosize == 0)
      err = 1;
    if (!err)
      npts = H5Sget_simple_extent_npoints(aspace);
    if (npts < 0)
      err = 1;
    /* get the attribute name */
    if (!err)
      len = H5Aget_name(aid, 0, NULL);
    if (len < 0)
      err = 1;
    if (!err)
      aname = (char *)galloc((len + 1) * sizeof(char), "aname");
    if (aname == NULL)
      err = 1;
    if (!err)
      rc = H5Aget_name(aid, len + 1, aname);
    if (rc < 0)
      err = 1;
    if (err) {
        ...
        if (asize == 4) {
          if ((fval = (gafloat *)galloc(npts * sizeof(gafloat), "fval")) != NULL) {
            if ((rc = H5Aread(aid, H5T_NATIVE_FLOAT, fval)) >= 0) {
              snprintf(pout, 1255, "%s Float32 %s ", abbrv, aname);
              gaprnt(2, pout);
              for (i = 0; i < npts; i++) {
                snprintf(pout, 1255, "%g ", fval[i]);
                gaprnt(2, pout);
              }
              gaprnt(2, "\n");
            }
        ...
        /* double */
          ...
          if ((dval = (gadouble *)galloc(npts * sizeof(gadouble), "dval")) != NULL) {
            if ((rc = H5Aread(aid, H5T_NATIVE_DOUBLE, dval)) >= 0) {
              snprintf(pout, 1255, "%s Float64 %s ", abbrv, aname);
              gaprnt(2, pout);
              for (i = 0; i < npts; i++) {
                snprintf(pout, 1255, "%g ", dval[i]);
                gaprnt(2, pout);
              }
              gaprnt(2, "\n");
            }
          ...
          if (asize == 1) {
            if (asign == H5T_SGN_NONE) {
              if ((ucval = (unsigned char *)galloc(npts * sizeof(unsigned char), "ucval")) != NULL) {
                if ((rc = H5Aread(aid, H5T_NATIVE_UCHAR, (void *)ucval)) >= 0) {
                  snprintf(pout, 1255, "%s Byte %s ", abbrv, aname);
                  gaprnt(2, pout);
                  for (i = 0; i < npts; i++) {
          ...
          /* short */
            ...
            if (asign == H5T_SGN_NONE) {
              if ((usval = (unsigned short *)galloc(npts * sizeof(unsigned short), "usval")) != NULL) {
                if ((rc = H5Aread(aid, H5T_NATIVE_USHORT, (void *)usval)) >= 0) {
                  snprintf(pout, 1255, "%s UInt16 %s ", abbrv, aname);
                  gaprnt(2, pout);
                  for (i = 0; i < npts; i++) {
              ...
              if ((sval = (short *)galloc(npts * sizeof(short), "sval")) != NULL) {
                if ((rc = H5Aread(aid, H5T_NATIVE_SHORT, (void *)sval)) >= 0) {
                  snprintf(pout, 1255, "%s Int16 %s ", abbrv, aname);
                  gaprnt(2, pout);
                  for (i = 0; i < npts; i++) {
          ...
          /* int */
            ...
            if (asign == H5T_SGN_NONE) {
              if ((uival = (gauint *)galloc(npts * sizeof(gauint), "uival")) != NULL) {
                if ((rc = H5Aread(aid, H5T_NATIVE_UINT, (void *)uival)) >= 0) {
                  snprintf(pout, 1255, "%s UInt32 %s ", abbrv, aname);
                  gaprnt(2, pout);
                  for (i = 0; i < npts; i++) {
              ...
              if ((ival = (gaint *)galloc(npts * sizeof(gaint), "ival")) != NULL) {
                if ((rc = H5Aread(aid, H5T_NATIVE_INT, (void *)ival)) >= 0) {
                  snprintf(pout, 1255, "%s Int32 %s ", abbrv, aname);
                  gaprnt(2, pout);
                  for (i = 0; i < npts; i++) {
...
gaint gaopfn(gaint t, gaint e, gaint *ee, gaint *oflg, struct gafile *pfi) {
  ...
  *oflg = 0;
  ...
  need_new_file = 0;
  if (pfi->tmplat == 3 && ((i != pfi->fnumc) || (e != pfi->fnume)))
    need_new_file = 1;
  if (pfi->tmplat == 2 && (e != pfi->fnume))
    need_new_file = 2;
  if (pfi->tmplat == 1 && (i != pfi->fnumc))
    need_new_file = 3;
