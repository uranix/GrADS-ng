...
gaint gadsdf(struct gafile *pfi, GASDFPARMS parms) {
  ...
  if (parms.needundef) {
    ...
    if (attr) {
      ...
      if ((pfi->undefattr = (char *)galloc(len + 1, "undefattr1")) == NULL)
        goto err1;
      ...
      *(pfi->undefattr + len) = '\0';
      ...
      if (attr->len == 1) {
        if (attr->nctype == 3) {
          pfi->undef = *(short *)(attr->value);
          fwflg = 1;
        }
        if (attr->nctype == 4) {
          pfi->undef = *(gaint *)(attr->value);
          fwflg = 1;
        }
        if (attr->nctype == 5) {
    ...
    if (attr) {
      if (pfi->undefattrflg == 1) {
        ...
        pfi->undefattrflg = 2;
        len = strlen(attr->name);
        if ((pfi->undefattr2 = (char *)galloc(len + 1, "undefattr2")) == NULL)
          goto err1;
    ...
    if (!fwflg)
      pfi->undef = 9.96921e+36; /* use default for NC_FLOAT */
  }
  ...
  if (parms.needunpack) {
    ...
    if (attr) {
      len = strlen(attr->name);
      sz = len + 1;
      if ((pfi->scattr = (char *)galloc(sz, "scattr1")) == NULL)
        goto err1;
    ...
    if (attr) {
      len = strlen(attr->name);
      sz = len + 1;
      if ((pfi->ofattr = (char *)galloc(sz, "ofattr1")) == NULL)
        goto err1;
      ...
      if (pfi->dnum[ZINDEX] > 1) {
        ...
        if (attr != NULL) {
          if (!strncmp("down", (char *)attr->value, 4)) {
            if (lev2 > lev1)
              pfi->zrflg = 1; /* positive:down */
          } else {
            if (lev2 < lev1)
              pfi->zrflg = 1; /* positive:up */
          }
        } else if (ispress) {
          if (lev2 > lev1)
            pfi->zrflg = 1; /* pressure is always positive down */
        } else {
          if (lev2 < lev1)
            pfi->zrflg = 1; /* default is positive up */
        }
      }
      /* Read the axis values */
  ...
  if (parms.tsetup) {
    ...
    if (!Tcoord) {
      ...
      if (attr) {
        ...
        if ((!strncasecmp((char *)attr->value, "cal365", 6)) || (!strncasecmp((char *)attr->value, "altcal365", 9)) ||
            (!strncasecmp((char *)attr->value, "common_year", 11)) || (!strncasecmp((char *)attr->value, "365_day", 7)) ||
      ...
      if ((timeunits_attr->nctype == 1) && (timeunits_attr->len < 10) && (!strncmp((char *)timeunits_attr->value, "YYMMDDHH", 8))) {
      ...
      /* Handle all other udunits-compatible times */
        ...
        if (pfi->dnum[TINDEX] > 1) {
          ...
          if (compare_units("year", trunc_units) == Success) {
            ...
            tvals[5] = 0.0;
            if ((!strncmp(time_units, "month", 5)) || (!strncmp(time_units, "common_year/12", 14)) ||
              ...
              if (incrfactor < (28.0 * 24.0)) {
                ...
                if (incrfactor >= (360 * 24)) /* try years? */ {
                  tvals[5] = 12.0 * ((gadouble)((gaint)(((gaint)(incrfactor + 0.5)) / (360.0 * 24.0))));
                  tvals[6] = 0.0;
              ...
              if (incrfactor < 60.0) {
                snprintf(pout, 1255,
                         "Error: Time increment %g is too small for 'seconds since' time units attribute (must be >= 60)\n",
                         incrfactor);
                gaprnt(0, pout);
                goto err2;
              } else {
  ...
  if (parms.isxdf && (!parms.tsrch)) {
    if (parms.tdimname != NULL) {
      ...
      tdimid = -1; /* this is for the %nodim% option in TDEF */
  } else {
    if (Tcoord) {
  ...
  if ((pfi->rbuf = (gadouble *)galloc(sz, "rbuf")) == NULL)
    goto err1;
  sz = pfi->dnum[XINDEX] * sizeof(char);
  if ((pfi->ubuf = (char *)galloc(sz, "ubuf")) == NULL)
    goto err1;
...
gaint isdvar(struct gafile *pfi, struct gavar *var, gaint xdimid, gaint ydimid, gaint zdimid, gaint tdimid, gaint edimid) {
  ...
  for (i = 0; i < var->nvardims; i++) {
    if (var->vardimids[i] == xdimid) {
      var->units[i] = -100;
      hasX = 1;
    } else if (var->vardimids[i] == ydimid) {
      var->units[i] = -101;
      hasY = 1;
    } else if (var->vardimids[i] == zdimid) {
      var->units[i] = -102;
      hasZ = 1;
    } else if (var->vardimids[i] == tdimid) {
      var->units[i] = -103;
      hasT = 1;
    } else if (var->vardimids[i] == edimid) {
      var->units[i] = -104;
      hasE = 1;
    }
  }
  ...
  if (hasX || hasY || hasZ || hasT || hasE) {
    ...
    for (i = 0; i < var->nvardims; i++) {
      if (var->vardimids[i] != xdimid && var->vardimids[i] != ydimid && var->vardimids[i] != zdimid &&
          var->vardimids[i] != tdimid && var->vardimids[i] != edimid) {
        return Failure;
      }
    }
    ...
    if (attr == NULL)
      attr = find_att(var->longnm, pfi->attr, "standard_name");
    if (attr == NULL) {
...
gaint sdfdeflev(struct gafile *pfi, struct gavar *coord, gaint dim, gaint revflag) {
  ...
  if (pfi->ncflg == 2) {
    /* get the data type */
    if ((sds_id = SDselect(pfi->sdid, coord->sdvid)) == FAIL)
      return Failure;
...
gaint findX(struct gafile *pfi, struct gavar **Xcoordptr) {
  ...
  while (i < pfi->vnum) {
    ...
    if (iscoordvar) {
      ...
      if (attr) {
        match = 0;
        if (!strncmp(attr->value, "degrees_east", 12))
          match = 1;
        if (!strncmp(attr->value, "degree_east", 11))
          match = 1;
        if (!strncmp(attr->value, "degrees_E", 9))
          match = 1;
        if (!strncmp(attr->value, "degree_E", 8))
          match = 1;
        if (match) {
          *Xcoordptr = lclvar;
...
gaint findY(struct gafile *pfi, struct gavar **Ycoordptr) {
  ...
  while (i < pfi->vnum) {
    ...
    if (iscoordvar) {
      ...
      if (attr) {
        match = 0;
        if (!strncmp(attr->value, "degrees_north", 13))
          match = 1;
        if (!strncmp(attr->value, "degree_north", 12))
          match = 1;
        if (!strncmp(attr->value, "degrees_N", 9))
          match = 1;
        if (!strncmp(attr->value, "degree_N", 8))
          match = 1;
        if (match) {
          *Ycoordptr = lclvar;
          return Success; /* got a match on one of them */
...
gaint findZ(struct gafile *pfi, struct gavar **Zcoordptr, gaint *ispressptr) {
  ...
  while (i < pfi->vnum) {
    ...
    if (iscoordvar) {
      ...
      if (attr) {
        match = 0;
        if (!strncasecmp(attr->value, "hybrid_sigma_pressure", 21))
          match = 1;
        if (!strncasecmp(attr->value, "mb", 2))
          match = 1;
        if (!strncasecmp(attr->value, "millibar", 8))
          match = 1;
        if (!strncasecmp(attr->value, "hPa", 3))
          match = 1;
        if (match) {
          *Zcoordptr = lclvar;
          *ispressptr = 1;
        ...
        match = 0;
        if (!strncasecmp(attr->value, "sigma_level", 11))
          match = 1;
        if (!strncasecmp(attr->value, "degreesk", 8))
          match = 1;
        if (!strncasecmp(attr->value, "degrees_k", 9))
          match = 1;
        if (!strncasecmp(attr->value, "level", 5))
          match = 1;
        if (!strncasecmp(attr->value, "layer", 5))
          match = 1;
        if (!strncasecmp(attr->value, "layers", 6))
          match = 1;
        if (match) {
          *Zcoordptr = lclvar;
          *ispressptr = 0;
...
gaint findT(struct gafile *pfi, struct gavar **Tcoordptr) {
  ...
  while (i < pfi->vnum) {
    ...
    if (iscoordvar) {
      ...
      if (attr) {
        match = 0;
        if (!strncasecmp((char *)attr->value, "yyyymmddhhmmss", 14))
          match = 1;
        if (!strncasecmp((char *)attr->value, "yymmddhh", 8))
          match = 1;
        if (match) {
          *Tcoordptr = lclvar;
          rc = Success;
...
gaint decode_standard_time(gadouble time_val, gaint *year, gaint *month, gaint *day, gaint *hour, gaint *minn, gafloat *sec) {
  char str1[100];
...
static gaint parse_unit_to_months_mins(gadouble inc, char *unit, gadouble *dmon, gadouble *dmin) {
  ...
  if ((!strncmp(unit, "month", 5)) || (!strncmp(unit, "common_year/12", 14)) || (!strncmp(unit, "common_years/12", 15))) {
    *dmon = inc;
    *dmin = 0.0;
    if (!is_integer(*dmon)) {
      gaprnt(0, "Error: Fractional months are ill-defined and not supported by GrADS\n");
...
gaint advance_cal365_time(gadouble time, char *time_units, gaint *year, gaint *month, gaint *day, gaint *hour, gaint *min) {
...
gaint find_dim(struct gafile *pfi, char *name) {
  gaint i;
  for (i = 0; i < pfi->nsdfdims; i++) {
    if (!strcmp(pfi->sdfdimnam[i], name))
...
gaint read_one_dimension(struct gafile *pfi, struct gavar *coord, gaint start, gaint count, gadouble *data) {
  gaint rc;
  gadouble ddata;
  size_t st, cnt;
...
struct gavar *find_var(struct gafile *pfi, char *varname) {
  ...
  pvar = pfi->pvar1;
  while (i < pfi->vnum) {
    if (!strcmp(varname, pvar->longnm))
      return (struct gavar *)pvar;
    i++;
    pvar++;
  }
  return NULL;
  ...
  while (fgets(rec, 512, descr) != NULL) {
    ...
    /* Parse CHSUB records.  time1, time2, then a string,  multiple times */
      ...
      while (1) {
        ...
        len = wrdlen(ch);
        sz = len + 1;
        if ((pchsub->ch = (char *)galloc(sz, "chsub4")) == NULL)
          goto err8;
    ...
    /* Parse UNDEF */
      ...
      if ((ch = nxtwrd(ch)) != NULL) {
        ...
        while (*(ch + len) != ' ' && *(ch + len) != '\n' && *(ch + len) != '\t')
          len++;
        sz = len + 1;
        if ((pfi->undefattr = (char *)galloc(sz, "undefattr5")) == NULL)
          goto err8;
        for (i = 0; i < len; i++)
        ...
        *(pfi->undefattr + len) = '\0';
        ...
        if ((ch = nxtwrd(ch)) != NULL) {
          ...
          while (*(ch + len) != ' ' && *(ch + len) != '\n' && *(ch + len) != '\t')
            len++;
          sz = len + 1;
          if ((pfi->undefattr2 = (char *)galloc(sz, "undefattr6")) == NULL)
            goto err8;
          for (i = 0; i < len; i++)
    ...
    /* Parse XDEF */
      ...
      if (pfi->type == 2)
        continue;
      if ((ch = nxtwrd(mrec)) == NULL)
        goto err0; /* xdimname must be mixed case version*/
      parms->xsrch = 0;
      ...
      while (*(ch + len) != ' ' && *(ch + len) != '\n' && *(ch + len) != '\t')
        len++;
      sz = len + 1;
      if ((parms->xdimname = (char *)galloc(sz, "xdimname")) == NULL)
        goto err8;
      for (i = 0; i < len; i++)
        *(parms->xdimname + i) = *(ch + i);
    ...
    /* Parse YDEF */
      ...
      if (pfi->type == 2)
        continue;
      if ((ch = nxtwrd(mrec)) == NULL)
        goto err0; /* ydimname must be mixed case version*/
      parms->ysrch = 0;
      ...
      while (*(ch + len) != ' ' && *(ch + len) != '\n' && *(ch + len) != '\t')
        len++;
      sz = len + 1;
      if ((parms->ydimname = (char *)galloc(sz, "ydimname")) == NULL)
        goto err8;
      for (i = 0; i < len; i++)
        *(parms->ydimname + i) = *(ch + i);
    ...
    /* Parse ZDEF */
      ...
      if ((ch = nxtwrd(mrec)) == NULL)
        goto err0; /* get mixed case version */
      parms->zsrch = 0;
      ...
      while (*(ch + len) != ' ' && *(ch + len) != '\n' && *(ch + len) != '\t')
        len++;
      sz = len + 1;
      if ((parms->zdimname = (char *)galloc(sz, "zdimname")) == NULL)
        goto err8;
      for (i = 0; i < len; i++)
        *(parms->zdimname + i) = *(ch + i);
    ...
    /* Parse TDEF */
      ...
      if (!strncasecmp(ch, "%nodim%", 7)) {
        ...
        while (*(ch + len) != ' ' && *(ch + len) != '\n' && *(ch + len) != '\t')
          len++;
        sz = len + 1;
        if ((parms->tdimname = (char *)galloc(sz, "tdimname")) == NULL)
          goto err8;
        for (i = 0; i < len; i++)
          *(parms->tdimname + i) = *(ch + i);
      ...
      if ((ch = nxtwrd(ch)) == NULL) {
        if (parms->tdimname == NULL) {
          sz = sizeof(gadouble) * 8;
          if ((tvals = (gadouble *)galloc(sz, "tvals3")) == NULL)
            goto err8;
          tvals[0] = 1.0;
          tvals[1] = 1.0;
          tvals[2] = 1.0;
        ...
        if (cmpwrd("linear", ch)) {
          ...
          if (dequal(v1, 0.0, 1.0e-08) == 0 && dequal(v2, 0.0, 1.0e-08) == 0)
            goto err4a;
          sz = sizeof(gadouble) * 8;
          if ((tvals = (gadouble *)galloc(sz, "tvals4")) == NULL)
            goto err8;
          *(tvals) = dt1.yr;
          *(tvals + 1) = dt1.mo;
          *(tvals + 2) = dt1.dy;
    ...
    /* Parse EDEF */
      ...
      if ((ch = nxtwrd(mrec)) == NULL)
        goto err1;      /* get mixed case version */
      ...
      *(parms->edimname + len) = '\0';
      ...
      if ((ch = nxtwrd(ch)) != NULL) {
        if ((pos = intprs(ch, &(pfi->dnum[EINDEX]))) == NULL)
          goto err1;
    ...
    /* parse the variable declarations */
      ...
      if ((varnames = (struct sdfnames *)galloc(sz, "varnames")) == NULL)
        goto err8;
...
void freeparms(GASDFPARMS *parms) {
  if (parms->xdimname)
    gree(parms->xdimname, "f50");
  if (parms->ydimname)
    gree(parms->ydimname, "f51");
  if (parms->zdimname)
    gree(parms->zdimname, "f52");
  if (parms->tdimname)
    gree(parms->tdimname, "f53");
  if (parms->edimname)
    gree(parms->edimname, "f54");
  if (parms->names1)
    gree(parms->names1, "f55");
  return;
}
