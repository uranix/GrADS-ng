...
gaint gacmd(char *com, struct gacmn *pcm, gaint exflg) {
  ...
  if (dsubs == NULL)
    dsubs = getdsubs(); /* get ptrs to the graphics display functions */
  if (psubs == NULL)
    psubs = getpsubs(); /* get ptrs to the graphics printing functions */
  ...
  *(ccc + len) = '\0'; /* Maintain mixed case version */
  ...
  if (retcod >= 0)
    goto retrn; /* found a udc, all done */
  else
    retcod = 0; /* not found, keep going */
#endif
  ...
  if (cmpwrd("quit", cmd)) {
  ...
  } else if (cmpwrd("history", cmd) || cmpwrd("his", cmd) || cmpwrd("repeat", cmd) || cmpwrd("r", cmd)) {
...
#if USEGADAP
        if (pfi->dhandle > -999)
          dapclo(pfi); /* opendap station data */
#endif
        pfi2 = pfi->pforw;
        frepfi(pfi, 0);
        pfi = pfi2;
    ...
    if (pcm->fnum > 0 && pcm->pfi1) {
      ...
      if (pfi->type == 2) {
        ...
        if (pfi->type == 1 && pfi->dnum[2] > 1 && ((pfi->dnum[0] == 1) || (pfi->dnum[1] == 1))) {
          if (pfi->dnum[2] <= 1) {
    ...
    if (pcm->fnum == 0) { /* no files open, so ... */
      ...
      pfi = pcm->pfi1;
      for (i = 0; i < fnum - 2 && pfi; i++)
        pfi = pfi->pforw; /* move to end of chain */
    ...
    if ((cmd = nxtwrd(cmd)) != NULL) {
      rc = 99;
      if (cmpwrd("norset", cmd))
        rc = 1; /* clears without resetting user options */
      if (cmpwrd("events", cmd))
        rc = 2; /* flushes the Xevents buffer */
      if (cmpwrd("graphics", cmd))
        rc = 3; /* clears the graphics but not the widgets */
      if (cmpwrd("hbuff", cmd))
        rc = 4; /* clears the buffered display and turns off double buffer mode */
      if (cmpwrd("button", cmd))
        rc = 5; /* clears a button */
      if (cmpwrd("rband", cmd))
        rc = 6; /* clears a rubber band */
      if (cmpwrd("dropmenu", cmd))
        rc = 7; /* clears a dropmenu */
      if (cmpwrd("sdfwrite", cmd))
        rc = 8; /* clears sdfwrite filename and attributes only */
      if (cmpwrd("mask", cmd))
        rc = 9; /* clears contour label mask */
      if (cmpwrd("shp", cmd))
        rc = 10; /* clears shapefile output filename and attributs only */
    }
    if (rc == 99) {
    ...
    if (rc == 1)
      ...
      gacln(pcm, 2); /* clears sdfwrite file name and attributes */
    else if (rc == 10)
      gacln(pcm, 3); /* clears user-provided shapefile attributes */
    goto retrn;
    ...
    if ((cmd = nxtwrd(com)) == NULL) {
      ...
      if (cmpwrd("print", cmd))
        ...
        if (pcm->ffile) { /* don't close a file unless it's open  */
          if (pcm->ffile != stdout)
            fclose(pcm->ffile); /* don't close stdout */
        }
        pcm->ffile = NULL;
        if (pcm->fwname)
          gree(pcm->fwname, "f194"); /* reset fwrite file name on disable. */
        pcm->fwname = NULL;
        ...
        gaprnt(0, "DISABLE error: Invalid keyword\n");
    }
    goto retrn;
    ...
    while ((cmd = nxtwrd(cmd)) != NULL && (com = nxtwrd(com)) != NULL) {
      ...
      if (cmpwrd("black", cmd))
        bwin = 0;
      else if (cmpwrd("white", cmd))
        bwin = 1;
      /* explicit format types -- the union of all possibilities from all rendering engines */
      else if (cmpwrd("eps", cmd))
        fmtflg = 1;
      else if (cmpwrd("ps", cmd))
        fmtflg = 2;
      else if (cmpwrd("pdf", cmd))
        fmtflg = 3;
      else if (cmpwrd("svg", cmd))
        fmtflg = 4;
      /* ... image formats start here ... */
      else if (cmpwrd("png", cmd))
        fmtflg = 5;
      else if (cmpwrd("gif", cmd))
        fmtflg = 6;
      else if (cmpwrd("jpg", cmd))
        fmtflg = 7;
      else if (cmpwrd("jpeg", cmd))
        fmtflg = 7;
    ...
    if (fmtflg == 0) {
      ...
      if (pos > 0) {
        for (i = 0; i < len; i++)
          ext[i] = *(cc + pos + i);
        lowcas(ext);
        if (!strncmp(ext, ".eps", 4))
          fmtflg = 1;
        if (!strncmp(ext, ".ps", 3))
          fmtflg = 2;
        if (!strncmp(ext, ".pdf", 4))
          fmtflg = 3;
        if (!strncmp(ext, ".svg", 4))
          fmtflg = 4;
    ...
    if (fmtflg == 1 || fmtflg == 2 || fmtflg == 3 || fmtflg == 4) {
      if (*bgImage || *fgImage) {
        snprintf(pout, 1255,
                 "GXPRINT Warning: Background/Foreground images are incompatible with the %s format and will be ignored\n",
                 formats[fmtflg - 1]);
        gaprnt(1, pout);
        bgImage[0] = '\0';
        fgImage[0] = '\0';
    ...
    if (rc == 1)
      gaprnt(0, "GXPRINT error: something went wrong during rendering of the output file\n");
    if (rc == 2)
      gaprnt(0, "GXPRINT error: output error\n");
    if (rc == 3)
      gaprnt(0, "GXPRINT error: background image open error\n");
    if (rc == 4)
      gaprnt(0, "GXPRINT error: foreground image open error\n");
    if (rc == 5)
      gaprnt(0, "GXPRINT error: background image must be .png\n");
    if (rc == 6)
      gaprnt(0, "GXPRINT error: foreground image must be .png\n");
    if (rc == 7)
      gaprnt(0, "GXPRINT error: failed to import background image\n");
    if (rc == 8)
      gaprnt(0, "GXPRINT error: failed to import foreground image\n");
    if (rc == 9)
      gaprnt(0, "GXPRINT error: unsupported output format\n");
    if (rc == 10)
      gaprnt(0, "GXPRINT error: background image must be the same size as output\n");
    if (rc == 11)
      gaprnt(0, "GXPRINT error: foreground image must be the same size as output\n");
    if (rc)
      retcod = 1;
    else {
      retcod = 0;
    ...
    if (!retcod)
      mygreta(cc); /* (for IGES only) keep track of user's opened files */
    ...
    if (!retcod)
      mygreta(cmd); /* (for IGES only) keep track of user's opened files */
    goto retrn;
    ...
    if ((ivals = (gafloat *)galloc(size * sizeof(gafloat), "ppivals")) == NULL) {
      retcod = 1;
      goto retrn;
    }
    if ((jvals = (gafloat *)galloc(size * sizeof(gafloat), "ppjvals")) == NULL) {
      retcod = 1;
      goto retrn;
    }
    if ((rvals = (gafloat *)galloc(size * sizeof(gafloat), "pprvals")) == NULL) {
      retcod = 1;
      goto retrn;
    }
    ...
    rc = fwrite(ival, sizeof(gafloat), size, pdefid);
    if (rc != size) {
      retcod = 1;
      goto retrn;
    }
    rc = fwrite(jval, sizeof(gafloat), size, pdefid);
    if (rc != size) {
      retcod = 1;
      goto retrn;
    }
    ...
    if (pcm->pfid->ppwrot)
      for (i = 0; i < size; i++)
        *(rvals + i) = (gafloat)(*(pcm->pfid->ppw + i));
    else
      for (i = 0; i < size; i++)
        *(rvals + i) = -999;
    rc = fwrite(rvals, sizeof(gafloat), size, pdefid);
    if (rc != size) {
      retcod = 1;
      goto retrn;
    }
...
void gacln(struct gacmn *pcm, gaint flg) {
  ...
  if (flg == 2) {
    ...
    while (pcm->attr != NULL) {
      ...
      if (attr->value)
        gree(attr->value, "g85"); /* release memory from 1st block */
      gree(attr, "g86");
    }
  }
...
gaint gadraw(char *cmd, struct gacmn *pcm) {
  ...
  if (cmpwrd("button", oper)) {
    if ((cmd = nxtwrd(cmd)) == NULL)
      goto errbn;
    if (intprs(cmd, &(cnt)) == NULL)
      goto errbn;
    if ((cmd = nxtwrd(cmd)) == NULL)
      goto errbn;
    if (getdbl(cmd, &(btn.x)) == NULL)
      goto errbn;
    if ((cmd = nxtwrd(cmd)) == NULL)
      goto errbn;
    if (getdbl(cmd, &(btn.y)) == NULL)
      goto errbn;
    if ((cmd = nxtwrd(cmd)) == NULL)
      goto errbn;
    if (getdbl(cmd, &(btn.w)) == NULL)
      goto errbn;
    if ((cmd = nxtwrd(cmd)) == NULL)
      goto errbn;
    if (getdbl(cmd, &(btn.h)) == NULL)
      goto errbn;
    btn.bc = pcm->btnbc;
    btn.fc = pcm->btnfc;
    btn.oc1 = pcm->btnoc;
  ...
  if (cmpwrd("dropmenu", oper)) {
    ...
    if (cmpwrd("cascade", cmd)) {
      ...
      dmu.casc = 0;
      if (getdbl(cmd, &(dmu.x)) == NULL)
        goto errpm;
      if ((cmd = nxtwrd(cmd)) == NULL)
        goto errpm;
      if (getdbl(cmd, &(dmu.y)) == NULL)
        goto errpm;
      if ((cmd = nxtwrd(cmd)) == NULL)
        goto errpm;
      if (getdbl(cmd, &(dmu.w)) == NULL)
        goto errpm;
      if ((cmd = nxtwrd(cmd)) == NULL)
        goto errpm;
      if (getdbl(cmd, &(dmu.h)) == NULL)
        goto errpm;
    }
    dmu.fc = pcm->drvals[0];
    dmu.bc = pcm->drvals[1];
    dmu.oc1 = pcm->drvals[2];
    dmu.oc2 = pcm->drvals[3];
    dmu.tfc = pcm->drvals[4];
    dmu.tbc = pcm->drvals[5];
    dmu.toc1 = pcm->drvals[6];
    dmu.toc2 = pcm->drvals[7];
    dmu.bfc = pcm->drvals[8];
    dmu.bbc = pcm->drvals[9];
    dmu.boc1 = pcm->drvals[10];
    dmu.boc2 = pcm->drvals[11];
    dmu.soc1 = pcm->drvals[12];
    dmu.soc2 = pcm->drvals[13];
  ...
  if (cmpwrd("wxsym", oper)) {
    if ((cmd = nxtwrd(cmd)) == NULL)
      goto errwx;
    if (intprs(cmd, &wx) == NULL)
      goto errwx;
    if ((cmd = nxtwrd(cmd)) == NULL)
      goto errwx;
    if (getdbl(cmd, &x) == NULL)
      goto errwx;
    if ((cmd = nxtwrd(cmd)) == NULL)
      goto errwx;
    if (getdbl(cmd, &y) == NULL)
      goto errwx;
    if ((cmd = nxtwrd(cmd)) == NULL)
      goto errwx;
    if (getdbl(cmd, &cs) == NULL)
      goto errwx;
    thk = 3;
    col = -1;
    if ((cmd = nxtwrd(cmd)) != NULL) {
      if (intprs(cmd, &col) == NULL)
        goto errwx;
      if ((cmd = nxtwrd(cmd)) != NULL) {
        if (intprs(cmd, &thk) == NULL)
          goto errwx;
      }
    }
    if (wx < 1 || wx > 43)
  ...
  if (cmpwrd("rec", oper)) {
    if ((cmd = nxtwrd(cmd)) == NULL)
      goto errrc;
    if (getdbl(cmd, &xlo) == NULL)
      goto errrc;
    if ((cmd = nxtwrd(cmd)) == NULL)
      goto errrc;
    if (getdbl(cmd, &ylo) == NULL)
      goto errrc;
    if ((cmd = nxtwrd(cmd)) == NULL)
      goto errrc;
    if (getdbl(cmd, &xhi) == NULL)
      goto errrc;
    if ((cmd = nxtwrd(cmd)) == NULL)
      goto errrc;
    if (getdbl(cmd, &yhi) == NULL)
      goto errrc;
    if (xlo >= xhi || ylo >= yhi)
      goto errrc;
    gxwide(pcm->linthk);
  ...
  if (cmpwrd("shp", oper)) {
    ...
    for (i = begshp; i <= endshp; i++) {
      ...
      shape = 0;
      if ((shp->nSHPType == SHPT_POINT) || (shp->nSHPType == SHPT_MULTIPOINT) || (shp->nSHPType == SHPT_POINTZ) ||
          (shp->nSHPType == SHPT_MULTIPOINTZ) || (shp->nSHPType == SHPT_POINTM) || (shp->nSHPType == SHPT_MULTIPOINTM))
        shape = 1; /* points */
      if ((shp->nSHPType == SHPT_ARC) || (shp->nSHPType == SHPT_ARCZ) || (shp->nSHPType == SHPT_ARCM))
        shape = 2; /* lines */
      if ((shp->nSHPType == SHPT_POLYGON) || (shp->nSHPType == SHPT_POLYGONZ) || (shp->nSHPType == SHPT_POLYGONM))
        shape = 3; /* polygons */
      ...
      if (shape > 0) {
        ...
        for (p = 0; p < numparts; p++) {
          ...
          lnfact = 0.0;
          while (shp->dfXMax + lnfact > pcm->dmin[0])
            lnfact -= 360.0;
          lnfact += 360.0;
  ...
  if (cmpwrd("polyf", oper) || cmpwrd("mappoly", oper)) {
    ...
    gxcolr(pcm->lincol);
    if (pcm->ptflg)
      gxptrn(pcm->ptopt, pcm->ptden, pcm->ptang);
    gxfill(xy, cnt);
    if (pcm->ptflg)
      gxptrn(1, 1, 0);
    gree(xy, "f205");
  ...
  if (cmpwrd("title", oper) || cmpwrd("xlab", oper) || cmpwrd("ylab", oper)) {
...
void gapysavpcm(struct gacmn *pcm) { savpcm = pcm; }
...
gaint gaqdef(char *cmd, struct gacmn *pcm, gaint flag) {
  ...
  if ((cmd = nxtwrd(cmd)) == NULL)
    goto err;
  if (intprs(cmd, &gri) == NULL)
    goto err;
  if ((cmd = nxtwrd(cmd)) == NULL)
    goto err;
  if (intprs(cmd, &grj) == NULL)
    goto err;
  if (flag) {
    if ((cmd = nxtwrd(cmd)) == NULL)
      goto err;
    if ((strncmp(cmd, "missing", 7)) == 0)
      flag = 2;
    else if (getdbl(cmd, &grval) == NULL)
      goto err;
  }
...
gaint gamodf(char *cmd, struct gacmn *pcm) {
  ...
  pdf = pcm->pdf1;
  while (pdf != NULL && !cmpwrd(name, pdf->abbrv))
    pdf = pdf->pforw;
  if (pdf == NULL) {
...
gaint gadef(char *cmd, struct gacmn *pcm, gaint impf) {
  ...
  if ((pfiv->grvals[2] = cpscal(pfi->grvals[2], pfi->linear[2], 0, 2)) == NULL)
    goto etrn;
  if ((pfiv->abvals[2] = cpscal(pfi->abvals[2], pfi->linear[2], 1, 2)) == NULL)
    goto etrn;

  if ((pfiv->grvals[3] = cpscal(pfi->grvals[3], pfi->linear[3], 0, 3)) == NULL)
    goto etrn;
  if ((pfiv->abvals[3] = cpscal(pfi->abvals[3], pfi->linear[3], 1, 3)) == NULL)
    goto etrn;

  if ((pfiv->grvals[4] = cpscal(pfi->grvals[4], pfi->linear[4], 0, 4)) == NULL)
    goto etrn;
  if ((pfiv->abvals[4] = cpscal(pfi->abvals[4], pfi->linear[4], 1, 4)) == NULL)
    goto etrn;
  ...
  if (pgr1->idim > -1 && pgr1->jdim > -1) { /* I and J are varying */
    ...
    if ((pfiv->grvals[0] = cpscal(pgr1->ivals, pgr1->ilinr, 0, pgr1->idim)) == NULL)
      goto etrn;
    if ((pfiv->abvals[0] = cpscal(pgr1->iavals, pgr1->ilinr, 1, pgr1->idim)) == NULL)
      goto etrn;
    ...
    if ((pfiv->grvals[1] = cpscal(pgr1->jvals, pgr1->jlinr, 0, pgr1->jdim)) == NULL)
      goto etrn;
    if ((pfiv->abvals[1] = cpscal(pgr1->javals, pgr1->jlinr, 1, pgr1->jdim)) == NULL)
      goto etrn;
    ...
    if (pgr1->idim == 0) {                          /* I is X */
      ...
      if ((pfiv->grvals[1] = cpscal(pfi->grvals[1], pfi->linear[1], 0, 1)) == NULL)
        goto etrn;
      if ((pfiv->abvals[1] = cpscal(pfi->abvals[1], pfi->linear[1], 1, 1)) == NULL)
        goto etrn;
      ...
      if ((pfiv->grvals[1] = cpscal(pgr1->ivals, pgr1->ilinr, 0, pgr1->idim)) == NULL)
        goto etrn;
      if ((pfiv->abvals[1] = cpscal(pgr1->iavals, pgr1->ilinr, 1, pgr1->idim)) == NULL)
        goto etrn;
      ...
      if ((pfiv->grvals[0] = cpscal(pfi->grvals[0], pfi->linear[0], 0, 0)) == NULL)
        goto etrn;
      if ((pfiv->abvals[0] = cpscal(pfi->abvals[0], pfi->linear[0], 1, 0)) == NULL)
        goto etrn;
      ...
      for (it = itmin; it <= itmax; it++) {
        for (iz = izmin; iz <= izmax; iz++) {
          ...
          if (ie == iemin && it == itmin && iz == izmin) {
            ...
            pgr = pst->result.pgr;
            if (pgr->idim != pgr1->idim || pgr->jdim != pgr1->jdim || gagchk(pgr, pgr1, pgr1->idim) ||
...
retrn:
  ...
  if (pfiv != NULL) {
    if (pfiv->rbuf != NULL)
      gree(pfiv->rbuf, "f210a");
    if (pfiv->ubuf != NULL)
      gree(pfiv->ubuf, "f210b");
    gree(pfiv, "f100");
  }
...
char *gxnms[29] = {"Clear",  "Contour", "Shaded", "Vector",  "Grid",    "Fgrid",     "Line",     "Scatter",  "Bar",     "Stream",
                   "Value",  "Model",   "Wxsym",  "Line",    "StnMark", "Barb",      "GrFill",   "LineFill", "",        "",
                   "Fwrite", "Findstn", "Stat",   "GeoTIFF", "KML",     "Shapefile", "ImageMap", "Shaded2",  "Shaded2b"};
...
static char *gxout2Da[20] = {"0",      "Contour", "Shaded",  "Grid", "4",        "5",         "Fgrid", "Fwrite", "8",  "9",
                             "Grfill", "Pgrid",   "GeoTIFF", "KML",  "ImageMap", "Shapefile", "16",    "17",     "18", "19"};
static char *gxout2Db[10] = {"0", "1", "2", "Grid", "Vector", "Scatter", "6", "7", "Stream", "Barb"};
static char *gxoutStn[10] = {"0", "Value", "Barb", "Findstn", "Model", "Wxsym", "6", "Stnmark", "Stnwrite", "Shapefile"};
...
gaint gaqury(char *cmd, char *ccc, struct gacmn *pcm) {
  ...
  if (arg == NULL) {
    ...
    if (pcm->dbflg)
      snprintf(pout, 1255, "double buffering is on\n");
    else
      snprintf(pout, 1255, "double buffering is off\n");
    gaprnt(2, pout);
  } else if (cmpwrd(arg, "calendar")) {
    if (mfcmn.cal365 == -999)
      snprintf(pout, 1255, "calendar mode not yet set\n");
    else if (mfcmn.cal365 == 1)
      snprintf(pout, 1255, "365-day calendar in effect\n");
    else
      snprintf(pout, 1255, "standard calendar in effect\n");
    gaprnt(2, pout);
    ...
    if (fn <= 5) {
      if (fn == 3) {
        ...
        snprintf(pout, 1255, "%s ", dbq.fname);
        gaprnt(2, pout);
        if (dbq.fbold)
          gaprnt(2, "bold ");
        if (dbq.fitalic)
    ...
    if ((arg = nxtwrd(arg)) == NULL)
      goto errdl;
    if (getdbl(arg, &(dlg.x)) == NULL)
      goto dialog;
    if ((arg = nxtwrd(arg)) == NULL)
      goto errdl;
    if (getdbl(arg, &(dlg.y)) == NULL)
      goto errdl;
    if ((arg = nxtwrd(arg)) == NULL)
      goto errdl;
    if (getdbl(arg, &(dlg.w)) == NULL)
      goto errdl;
    if ((arg = nxtwrd(arg)) == NULL)
      goto errdl;
    if (getdbl(arg, &(dlg.h)) == NULL)
      goto errdl;
    dlg.pc = pcm->dlgpc;
    dlg.fc = pcm->dlgfc;
    dlg.bc = pcm->dlgbc;
  ...
  dialog:
    ...
    *(dlg.ch + dlg.len) = '\0';
    ...
    while (i < pfi->dnum[4]) {
      ...
      snprintf(pout, 1255, "Ensemble %d named %s has %d timesteps and begins at %s (t=%d) ", i + 1, ens->name, ens->length, lab,
               ens->gt);
      gaprnt(2, pout);
  ...
  /* this command is used by the GDS -- formatted specifically */
    ...
    while (j < pfi->vnum) {
      ...
      printf(" levels: %d ", pvar->levels);
      printf("\n");
      printf(" units: ");
      for (i = 0; i < 16; i++)
        printf("%-4g ", pvar->units[i]);
      printf("\n");
      printf(" vecpair=%d \n", pvar->vecpair);
      printf(" isu=%d \n", pvar->isu);
      printf(" offset=%lld \n", pvar->offset);
      ...
      if (pfi->ncflg) {
        ...
        for (i = 0; i < pvar->nvardims; i++)
          printf("%-4d ", pvar->vardimids[i]);
        printf("\n");
        if (pfi->ncflg == 1)
          printf(" ncvid=%d \n", pvar->ncvid);
        if (pfi->ncflg == 2)
    ...
    gaprnt(2, pout);
    if (pcm->shptype == 1)
      gaprnt(2, " output type: point\n");
    if (pcm->shptype == 2)
      gaprnt(2, " output type: line\n");
    ...
    if ((arg = nxtwrd(arg)) == NULL) {
      ...
      if (shpid) {
        ...
        snprintf(pout, 1255, "Shapefile Type=%s #Shapes=%d XBounds=%g:%g YBounds=%g:%g\n", SHPTypeName(shptype), shpcnt, minvals[0],
                 maxvals[0], minvals[1], maxvals[1]);
        gaprnt(2, pout);
        for (i = 0; i < shpcnt; i++) {
          shp = NULL;
          if ((shp = SHPReadObject(shpid, i)) != NULL) {
            snprintf(pout, 1255, "%d:  %s  parts=%d  vertices=%d  ", shp->nShapeId, SHPTypeName(shp->nSHPType), shp->nParts,
                     shp->nVertices);
            gaprnt(2, pout);
            snprintf(pout, 1255, "XBounds=%g:%g  ", shp->dfXMin, shp->dfXMax);
            gaprnt(2, pout);
            snprintf(pout, 1255, "YBounds=%g:%g  ", shp->dfYMin, shp->dfYMax);
            gaprnt(2, pout);
            snprintf(pout, 1255, "ZBounds=%g:%g  ", shp->dfZMin, shp->dfZMax);
            gaprnt(2, pout);
            snprintf(pout, 1255, "MBounds=%g:%g \n", shp->dfMMin, shp->dfMMax);
            gaprnt(2, pout);
            SHPDestroyObject(shp);
          }
        }
    ...
    if (pcm->ffile)
      gaprnt(2, "FWrite file is open\n");
    else
      gaprnt(2, "FWrite file is closed\n");
    if (pcm->fwname) {
      gaprnt(2, "FWrite file name is: ");
      gaprnt(2, pcm->fwname);
      gaprnt(2, "\n");
    } else
      gaprnt(2, "FWrite file name is: grads.fwrite\n");
    if (pcm->fwenflg == 0) {
    ...
    if (pcm->sdfwname) {
      ...
      gaprnt(2, "SDFwrite file name is: grads.sdfwrite.nc\n");
    gaprnt(2, "SDFwrite format is ");
    if (pcm->sdfzip == 1)
      gaprnt(2, "compressed ");
    if (pcm->sdfwtype == 1)
      gaprnt(2, "NetCDF ");
    else if (pcm->sdfwtype == 2)
      gaprnt(2, "NetCDF-4 ");
    if (pcm->sdfprec == 8)
      gaprnt(2, "double precision ");
    else if (pcm->sdfprec == 4)
      gaprnt(2, "floating point precision ");
    gaprnt(2, "\n");
    if (pcm->sdfchunk || pcm->sdfzip) {
      gaprnt(2, "SDFWrite output chunk dimensions: ");
      if (pcm->xchunk)
        snprintf(pout, 1255, "%d ", pcm->xchunk);
      else
        snprintf(pout, 1255, "Xsize ");
      gaprnt(2, pout);
      if (pcm->ychunk)
        snprintf(pout, 1255, "%d ", pcm->ychunk);
      else
        snprintf(pout, 1255, "Ysize ");
      gaprnt(2, pout);
    ...
    if (pcm->attr) {
      ...
      while (attr) {
        ...
        if (attr->nctype <= 2) {
          ...
          gaprnt(2, pout);
          if (attr->nctype == 3)
            sptr = (short *)attr->value;
          else if (attr->nctype == 4)
            lptr = (long *)attr->value;
          else if (attr->nctype == 5)
            fptr = (gafloat *)attr->value;
          else if (attr->nctype == 6)
            dptr = (gadouble *)attr->value;
  ...
  /* Configuration options */
    ...
    if (pcm->tmin.mn == 0)
      gat2ch(&(pcm->tmin), 4, lab, 20);
    else
      gat2ch(&(pcm->tmin), 5, lab, 20);
    if (pcm->tmax.mn == 0)
      gat2ch(&(pcm->tmax), 4, lab2, 20);
    else
      gat2ch(&(pcm->tmax), 5, lab2, 20);
    if (v1 == v2) {
    ...
    if ((arg = nxtwrd(arg)) == NULL)
      goto errw;
    if (pcm->xdim == 3) {
      if (adtprs(arg, &(pcm->tmin), &dtim) == NULL)
        goto errw;
      lon = t2gr(pcm->xgrval, &dtim);
    } else {
      if (getdbl(arg, &lon) == NULL)
        goto errw;
    }
    if ((arg = nxtwrd(arg)) == NULL)
      goto errw;
    ...
    if (flag) {
      ...
      x = lon;
      if (conv && pcm->xdim != 3)
        x = conv(pcm->xabval, lon);
      conv = pcm->yab2gr;
      y = lat;
      if (conv && pcm->ydim != 3)
        y = conv(pcm->yabval, lat);
  ...
  errw:
    if (flag)
      snprintf(pout, 1255, "Query Error: Syntax is QUERY W2GR %s %s\n", cdims[pcm->xdim + 1], cdims[pcm->ydim + 1]);
    else
      snprintf(pout, 1255, "Query Error: Syntax is QUERY W2XY %s %s\n", cdims[pcm->xdim + 1], cdims[pcm->ydim + 1]);
    gaprnt(0, pout);
    return (1);
  }
    ...
    if ((arg = nxtwrd(arg)) == NULL)
      goto errgr;
    if (getdbl(arg, &x) == NULL)
      goto errgr;
    if ((arg = nxtwrd(arg)) == NULL)
      goto errgr;
    if (getdbl(arg, &y) == NULL)
      goto errgr;
    conv = pcm->xgr2ab;
    lon = x;
    if (conv && pcm->xdim != 3)
      lon = conv(pcm->xgrval, x);
    conv = pcm->ygr2ab;
    lat = y;
    if (conv && pcm->ydim != 3)
      lat = conv(pcm->ygrval, y);
    if (flag) {
      if (pcm->xdim == 3) {
  ...
  errgr:
    if (flag)
      snprintf(pout, 1255, "Query Error: Syntax is QUERY GR2W %s %s\n", ccdims[pcm->xdim], ccdims[pcm->ydim]);
    else
      snprintf(pout, 1255, "Query Error: Syntax is QUERY GR2XY %s %s\n", ccdims[pcm->xdim], ccdims[pcm->ydim]);
    gaprnt(0, pout);
    return (1);
  }
  ...
  errxy:
    if (flag)
      gaprnt(0, "Query Error: Syntax is QUERY XY2GR x y\n");
    else
      gaprnt(0, "QUERY error: Syntax is QUERY XY2W x y\n");
    return (1);
  }
  ...
  errll:
    ...
    if (pfi->ncflg == 1)
      gaprnt(2, "dtype netcdf\n");
    if (pfi->ncflg == 2)
      gaprnt(2, "dtype hdfsds\n");
    if (pfi->type == 2) {
      ...
      for (i = 0; i < pfi->vnum; i++) {
        ...
        for (j = 0; j < 5; j++) {
          if (pvar->units[j] != -999) {
            if (pvar->units[j] == -100)
              gaprnt(2, "x");
            else if (pvar->units[j] == -101)
              gaprnt(2, "y");
            else if (pvar->units[j] == -102)
              gaprnt(2, "z");
            else if (pvar->units[j] == -103)
              gaprnt(2, "t");
            else if (pvar->units[j] == -104)
              gaprnt(2, "e");
            else {
              snprintf(pout, 1255, "%g", pvar->units[j]);
              gaprnt(2, pout);
            }
            if (pvar->units[j + 1] != -999)
              gaprnt(2, ","); /* add a comma if we've got more */
          }
        }
    ...
    if ((arg = nxtwrd(arg)) == NULL) {
      ...
      while (pdf) {
        if (!strcmp(pdf->abbrv, lab)) {
          ...
          if (pfi->calendar == 1)
            gaprnt(2, " Calendar: 365-Day \n");
          else
            gaprnt(2, " Calendar: Gregorian \n");

          /* Has it been modified? */
          if (pfi->climo == 1)
            gaprnt(2, " Climatology: Seasonal \n");
          if (pfi->climo == 2)
            gaprnt(2, " Climatology: Diurnal \n");
        }
        pdf = pdf->pforw;
      }
    }
...
gaint gacoll(char *cmd, struct gacmn *pcm) {
  ...
  if (pcm->vdim[0] != 0 || pcm->vdim[1] != 0 || (pcm->vdim[2] == 1 && pcm->vdim[3] == 1) ||
      (pcm->vdim[2] == 0 && pcm->vdim[3] == 0)) {
...
gaint gaset(char *cmd, char *com, struct gacmn *pcm) {
  ...
  size_t sz;
  char *tileimg = NULL;
  static char *kwds[130] = {
      "X",       "Y",        "Z",       "T",        "LON",      "LAT",     "LEV",     "TIME",     "CINT",       "CSTYLE",
      "CCOLOR",  "LOOPDIM",  "LOOPING", "LOOPINCR", "DFILE",    "VRANGE",  "CSMOOTH", "GRID",     "CMARK",      "XAXIS",
      "YAXIS",   "GXOUT",    "BLACK",   "DIGNUM",   "DIGSIZ",   "CMIN",    "CMAX",    "ARRSCL",   "ARRSIZ",     "CLEVS",
      "STID",    "GRADS",    "CLAB",    "MPROJ",    "CTERP",    "XSIZE",   "CCOLS",   "MPVALS",   "MPDSET",     "VPAGE",
      "PAREA",   "LINE",     "STRING",  "STRSIZ",   "RGB",      "FGVALS",  "MAP",     "BARBASE",  "BARGAP",     "CTHICK",
      "MPDRAW",  "POLI",     "DBUFF",   "XYREV",    "XFLIP",    "YFLIP",   "ANNOT",   "DISPLAY",  "BACKGROUND", "RBCOLS",
      "RBRANGE", "MISSCONN", "IMPRUN",  "ZLOG",     "STRMDEN",  "FRAME",   "CLIP",    "VRANGE2",  "ARROWHEAD",  "MDLOPTS",
      "XLOPTS",  "YLOPTS",   "CLOPTS",  "XLAB",     "YLAB",     "XLEVS",   "YLEVS",   "XLINT",    "YLINT",      "MISSWARN",
      "BUTTON",  "DEFVAL",   "BAROPTS", "LFCOLS",   "WXCOLS",   "FONT",    "FWRITE",  "XLPOS",    "YLPOS",      "CLSKIP",
      "RBAND",   "ARRLAB",   "MPT",     "WXOPT",    "XLABS",    "YLABS",   "FILL",    "DROPMENU", "LATS",       "TIMELAB",
      "WARN",    "STNPRINT", "STAT",    "TLSUPP",   "GRIDLN",   "HEMPREF", "PRNOPTS", "DATAWARN", "DIALOG",     "WRITEGDS",
      "COSLAT",  "E",        "ENS",     "SDFWRITE", "SDFATTR",  "GEOTIFF", "KML",     "UNDEF",    "CHUNKSIZE",  "CACHESF",
      "SHPOPTS", "SHP",      "SHPATTR", "LOG1D",    "STRMOPTS", "TILE",    "HERSHEY", "LWID",     "ANTIALIAS",  "BARBOPTS"};
  ...
  if ((cmd = nxtwrd(cmd)) == NULL) {
    ...
    if (cmpwrd("auto", cmd))
      pcm->hemflg = -1;
    else if (cmpwrd("shem", cmd))
      pcm->hemflg = 1;
    else if (cmpwrd("nhem", cmd))
      pcm->hemflg = 0;
    else
      goto err;
    ...
    if ((cmd = nxtwrd(cmd)) == NULL)
      goto err;
    if (cmpwrd("outline", cmd))
      pcm->barolin = 1;
    if (cmpwrd("filled", cmd))
      pcm->barolin = 0;
    return (0);
    ...
    if (getdbl(cmd, &pcm->barbase) == NULL)
      goto err;
    pcm->barflg = 1;
    ...
    if ((cmd = nxtwrd(cmd)) == NULL)
      goto err;
    if (cmpwrd("on", cmd))
      gxdbsethersh(0); /* 0, use hershey fonts */
      ...
      if (dsubs->gxdckfont() == 0 || psubs->gxpckfont() == 0) {
        ...
        gxdbsethersh(1); /* 1, use emulation */
    } else
      goto err;
    ...
    if (xlo < 0.0 || ylo < 0.0)
      goto err;
    if (xhi > pcm->pxsize || yhi > pcm->pysize)
      goto err;
    if (yhi <= ylo || xhi <= xlo)
      goto err;
    ...
    if (cmpwrd("off", cmd))
      ...
      if (intprs(cmd, &red) == NULL)
        goto err;
      if (red < -1)
        red = -1;
      if ((cmd = nxtwrd(cmd)) != NULL) {
        if (intprs(cmd, &green) == NULL)
          goto err;
        if (green < 1)
          green = 1;
        if ((cmd = nxtwrd(cmd)) != NULL) {
          if (intprs(cmd, &blue) == NULL)
            goto err;
          if (blue < 1)
            blue = 1;
        }
    ...
    if ((cmd = nxtwrd(cmd)) == NULL)
      goto err;
    if (intprs(cmd, &xx) == NULL)
      goto err;
    if ((cmd = nxtwrd(cmd)) == NULL)
      goto err;
    if (intprs(cmd, &yy) == NULL)
      goto err;
    dsubs->gxdxsz(xx, yy);
    gxfrme(9);
    ...
    if (cmpwrd("off", ch)) {
      ...
      while ((cmd = nxtwrd(cmd)) != NULL) {
        if (getdbl(cmd, &(pcm->mpvals[i1])) == NULL)
          goto err;
        i1++;
        if (i1 > 9)
          goto err;
    ...
    i1 = 0;
    while ((cmd = nxtwrd(cmd)) != NULL) {
      if (intprs(cmd, &(pcm->fgvals[i1])) == NULL)
        goto err;
      if ((cmd = nxtwrd(cmd)) == NULL)
        goto err;
      if (intprs(cmd, &(pcm->fgcols[i1])) == NULL)
        goto err;
      i1++;
      if (i1 > 48)
        goto err;
    ...
    i1 = 0;
    while ((cmd = nxtwrd(cmd)) != NULL) {
      if (getdbl(cmd, &(pcm->clevs[i1])) == NULL)
        goto err;
      i1++;
      if (i1 > 254)
        goto err;
    ...
    i1 = 0;
    while ((cmd = nxtwrd(cmd)) != NULL) {
      if (getdbl(cmd, &(pcm->xlevs[i1])) == NULL)
        goto err;
      i1++;
      if (i1 > 49)
        goto err;
    ...
    i1 = 0;
    while ((cmd = nxtwrd(cmd)) != NULL) {
      if (getdbl(cmd, &(pcm->ylevs[i1])) == NULL)
        goto err;
      i1++;
      if (i1 > 49)
        goto err;
    ...
    i1 = 0;
    while ((cmd = nxtwrd(cmd)) != NULL) {
      if (i1 == 0 && cmpwrd("auto", cmd))
        break;
      if (intprs(cmd, &(pcm->rbcols[i1])) == NULL)
        goto err;
      i1++;
      if (i1 > 255)
        goto err;
    ...
    pcm->rbflg = i1;
    if (i1 == 0)
      gaprnt(2, "Rainbow colors set to auto\n");
    else {
    ...
    while ((cmd = nxtwrd(cmd)) != NULL) {
      if (i1 > 14)
        goto drerr;
      if (intprs(cmd, &(pcm->drvals[i1])) == NULL)
        goto drerr;
      i1++;
    }
    if (i1 == 0)
    ...
    i1 = 0;
    while ((cmd = nxtwrd(cmd)) != NULL) {
      if (intprs(cmd, &(pcm->ccols[i1])) == NULL)
        goto err;
      i1++;
      if (i1 > 255)
        goto err;
    ...
    if (cmpwrd("off", cmd))
      pcm->mproj = 0;
    else if (cmpwrd("scaled", cmd))
      pcm->mproj = 1;
    else if (cmpwrd("latlon", cmd))
      pcm->mproj = 2;
    else if (cmpwrd("nps", cmd))
      pcm->mproj = 3;
    else if (cmpwrd("sps", cmd))
      pcm->mproj = 4;
    else if (cmpwrd("robinson", cmd))
      pcm->mproj = 5;
    else if (cmpwrd("mollweide", cmd))
      pcm->mproj = 6;
    else if (cmpwrd("orthogr", cmd))
      pcm->mproj = 7;
    else if (cmpwrd("orthographic", cmd))
      pcm->mproj = 7;
    else if (cmpwrd("ortho", cmd))
      pcm->mproj = 7;
    else if (cmpwrd("lambert", cmd))
      pcm->mproj = 13;
    else
      goto err;
    ...
    *(ch + i1) = '\0';
    ...
    while (cmpwrd("-pt", cmd) || cmpwrd("-point", cmd) || cmpwrd("-ln", cmd) || cmpwrd("-line", cmd) || cmpwrd("-poly", cmd) ||
           cmpwrd("-fmt", cmd)) {
      if (cmpwrd("-pt", cmd) || cmpwrd("-point", cmd))
        pcm->shptype = 1;
      if (cmpwrd("-ln", cmd) || cmpwrd("-line", cmd))
        pcm->shptype = 2;
      if (cmpwrd("-poly", cmd))
        pcm->shptype = 3;
      if (cmpwrd("-fmt", cmd)) {
    ...
    *(ch + itt) = '\0';
    ...
    if (pcm->shpfname)
      gree(pcm->shpfname, "f225e");
    pcm->shpfname = ch;

    snprintf(pout, 1255, "Shapefile output file name root: %s\n", pcm->shpfname);
    gaprnt(2, pout);
    if (pcm->shptype == 1)
      gaprnt(2, "Shapefile output type: point\n");
    if (pcm->shptype == 2)
      gaprnt(2, "Shapefile output type: line\n");
    if (pcm->shptype == 3)
      gaprnt(2, "Shapefile output type: polygon\n");
    ...
    while (cmpwrd("-img", cmd) || cmpwrd("-image", cmd) || cmpwrd("-ln", cmd) || cmpwrd("-line", cmd) || cmpwrd("-poly", cmd)) {
      if (cmpwrd("-img", cmd) || cmpwrd("-image", cmd))
        pcm->kmlflg = 1; /* image output */
      if (cmpwrd("-ln", cmd) || cmpwrd("-line", cmd))
        pcm->kmlflg = 2; /* contour output */
      if (cmpwrd("-poly", cmd))
        pcm->kmlflg = 3; /* polygon output */
      if ((cmd = nxtwrd(cmd)) == NULL)
        goto err;
    }
    ...
    if (pcm->kmlname)
      gree(pcm->kmlname, "f225d");
    pcm->kmlname = ch;
    if (pcm->kmlflg == 1) {
      if (pcm->tifname)
        gree(pcm->tifname, "f225c");
      pcm->tifname = ch2;
    ...
    while (cmpwrd("-dbl", cmd) || cmpwrd("-flt", cmd)) {
      if (cmpwrd("-flt", cmd))
        pcm->gtifflg = 1; /* 4-byte depth in GeoTIFF output file */
      if (cmpwrd("-dbl", cmd))
        pcm->gtifflg = 2; /* 8-byte depth in GeoTIFF output file */
      if ((cmd = nxtwrd(cmd)) == NULL)
        goto err;
    }
    ...
    if (pcm->gtifname)
      gree(pcm->gtifname, "f225b");
    pcm->gtifname = ch;
    snprintf(pout, 1255, "GeoTIFF file name = %s\n", pcm->gtifname);
    gaprnt(2, pout);
    if (pcm->gtifflg == 1)
      gaprnt(2, "GeoTIFF format is float  \n");
    if (pcm->gtifflg == 2)
      gaprnt(2, "GeoTIFF format is double \n");
#else
  ...
  /* Following is for the so-called 'exact fwrite' to
    ...
    if (pcm->ffile) {
      ...
      if ((cmd = nxtwrd(cmd)) == NULL)
        goto err;
      while (cmpwrd("-be", cmd) || cmpwrd("-le", cmd) || cmpwrd("-ap", cmd) || cmpwrd("-cl", cmd) || cmpwrd("-sq", cmd) ||
             cmpwrd("-st", cmd) || cmpwrd("-ex", cmd)) {
        if (cmpwrd("-be", cmd))
          pcm->fwenflg = 1;
        if (cmpwrd("-le", cmd))
          pcm->fwenflg = 0;
    ...
    while (cmpwrd("-4d", cmd) || cmpwrd("-4de", cmd) || cmpwrd("-5d", cmd) || cmpwrd("-3dz", cmd) || cmpwrd("-3dt", cmd) ||
           cmpwrd("-flt", cmd) || cmpwrd("-dbl", cmd) || cmpwrd("-nc3", cmd) || cmpwrd("-nc4", cmd) || cmpwrd("-rt", cmd) ||
           cmpwrd("-re", cmd) || cmpwrd("-zip", cmd) || cmpwrd("-chunk", cmd)) {
      if (cmpwrd("-3dz", cmd))
        pcm->sdfwpad = 1;
      if (cmpwrd("-3dt", cmd))
        pcm->sdfwpad = 2;
    ...
    *(ch + i1) = '\0';
    ...
    if (pcm->attr) {
      attr = pcm->attr;
      while (attr->next)
        attr = attr->next;  /* advance to end of chain */
    ...
    if (pcm->dbfld) {
      fld = pcm->dbfld;
      while (fld->next)
        fld = fld->next;  /* advance to end of chain */
    ...
    if ((cmd = nxtwrd(cmd)) == NULL)
      goto err;
    if (cmpwrd("loglog", cmd))
      pcm->log1d = 3;
    else if (cmpwrd("logh", cmd))
      pcm->log1d = 1;
    else if (cmpwrd("logv", cmd))
      pcm->log1d = 2;
    else if (cmpwrd("off", cmd))
      pcm->log1d = 0;
    else
    ...
    if (cmpwrd("off", cmd))
      ...
      if (getdbl(cmd, &(pcm->blkmin)) == NULL)
        goto err;
      if ((cmd = nxtwrd(cmd)) == NULL)
        goto err;
      if (getdbl(cmd, &(pcm->blkmax)) == NULL)
        goto err;
      pcm->blkflg = 1;
    }
    ...
    if ((cmd = nxtwrd(cmd)) == NULL)
      goto err;
    if (cmpwrd("color", cmd) || cmpwrd("grey", cmd) || cmpwrd("greyscale", cmd))
      itt = 2; /* this will be a no-op */
    else if (cmpwrd("white", cmd))
      itt = 1;
    else if (cmpwrd("black", cmd))
      itt = 0;
    ...
    if ((cmd = nxtwrd(cmd)) == NULL)
      goto err;
    pcm->gout0 = 9;
    if (cmpwrd("contour", cmd))
      pcm->gout2a = 1;
    else if (cmpwrd("shaded", cmd))
      pcm->gout2a = 16;
    else if (cmpwrd("shade1", cmd))
      pcm->gout2a = 2;
    else if (cmpwrd("shade2", cmd))
      pcm->gout2a = 16;
    else if (cmpwrd("shade2b", cmd))
      pcm->gout2a = 17;
      ...
      pcm->goutstn = 6;
      pcm->gout1a = 1;
    } else if (cmpwrd("scatter", cmd))
      pcm->gout2b = 5;
    else if (cmpwrd("fgrid", cmd))
      pcm->gout2a = 6;
    else if (cmpwrd("fwrite", cmd))
      pcm->gout2a = 7;
    else if (cmpwrd("stream", cmd))
      pcm->gout2b = 8;
    else if (cmpwrd("grfill", cmd))
      pcm->gout2a = 10;
    else if (cmpwrd("geotiff", cmd)) {
#if GEOTIFF == 1
      pcm->gout2a = 12;
...
#endif
    else if (cmpwrd("value", cmd))
      pcm->goutstn = 1;
    else if (cmpwrd("barb", cmd)) {
      pcm->goutstn = 2;
      pcm->gout2b = 9;
      pcm->gout1a = 2;
    } else if (cmpwrd("findstn", cmd))
      pcm->goutstn = 3;
    else if (cmpwrd("model", cmd))
      pcm->goutstn = 4;
    else if (cmpwrd("wxsym", cmd))
      pcm->goutstn = 5;
    else if (cmpwrd("stnmark", cmd))
      pcm->goutstn = 7;
    else if (cmpwrd("stnwrt", cmd))
      pcm->goutstn = 8; /* undocumented */
    else if (cmpwrd("line", cmd)) {
      pcm->gout1 = 1;
      pcm->tser = 0;
    } else if (cmpwrd("bar", cmd))
      pcm->gout1 = 2;
    else if (cmpwrd("errbar", cmd))
      pcm->gout1 = 3;
    else if (cmpwrd("linefill", cmd))
      pcm->gout1 = 4;
    else if (cmpwrd("stat", cmd))
      pcm->gout0 = 1;
    else if (cmpwrd("print", cmd))
      ...
      pcm->gout0 = 3; /* only for internal use */
    else if (cmpwrd("tserwx", cmd))
      pcm->tser = 1;
    else if (cmpwrd("tserbarb", cmd))
      pcm->tser = 2;
    else
      goto err;
    if (pcm->gout0 == 9)
    ...
    if ((cmd = nxtwrd(cmd)) == NULL)
      goto err;
    if (getdbl(cmd, &(pcm->arrsiz)) == NULL)
      goto err;
    if ((cmd = nxtwrd(cmd)) == NULL) {
      pcm->arrmag = -999.0;
    } else {
      if (getdbl(cmd, &(pcm->arrmag)) == NULL)
        goto err;
    }
    pcm->arrflg = 1;
    ...
    if (cmpwrd("on", cmd))
      ...
      i1 = 0;
    else if (cmpwrd("forced", cmd))
      i1 = 2;
    else if (cmpwrd("masked", cmd))
      i1 = 3;
    ...
    *(strng + num) = '\0';
    ...
    if (cmpwrd("on", cmd))
      ...
      pcm->grflag = 0;
    else if (cmpwrd("horizontal", cmd))
      pcm->grflag = 2;
    else if (cmpwrd("vertical", cmd))
      pcm->grflag = 3;
    else
      goto err;
    if ((cmd = nxtwrd(cmd)) != NULL) {
    ...
    if ((cmd = nxtwrd(cmd)) == NULL)
      ...
      pcm->wxopt = 1;
    else if (cmpwrd("mark", cmd))
      pcm->wxopt = 2;
    else if (cmpwrd("char", cmd))
      pcm->wxopt = 3;
    else
      goto err;
    ...
    if ((cmd = nxtwrd(cmd)) == NULL)
      goto err;
    if (intprs(cmd, itmp) == NULL)
      goto err;
    if ((cmd = nxtwrd(cmd)) == NULL)
      goto err;
    if (intprs(cmd, itmp + 1) == NULL)
      goto err;
    if ((cmd = nxtwrd(cmd)) == NULL)
      goto err;
    if (intprs(cmd, itmp + 2) == NULL)
      goto err;
    if ((cmd = nxtwrd(cmd)) == NULL)
      goto err;
    if (intprs(cmd, itmp + 3) == NULL)
      goto err;
    if ((cmd = nxtwrd(cmd)) == NULL)
      goto err;
    if (intprs(cmd, itmp + 4) == NULL)
      goto err;
    for (i1 = 0; i1 < 5; i1++)
      pcm->wxcols[i1] = itmp[i1];
    ...
    snprintf(pout, 1255, "SET BUTTON values:  Fc, Bc, Oc, Oc2 = %i %i %i %i ", pcm->btnfc, pcm->btnbc, pcm->btnoc, pcm->btnoc2);
    gaprnt(2, pout);
    snprintf(pout, 1255, "Toggle Fc, Bc, Oc, Oc2 = %i %i %i %i ", pcm->btnftc, pcm->btnbtc, pcm->btnotc, pcm->btnotc2);
      ...
      if ((cmd = nxtwrd(cmd)) != NULL) {
        ...
        if ((cmd = nxtwrd(cmd)) != NULL) {
          ...
          if ((cmd = nxtwrd(cmd)) != NULL) {
            ...
            pcm->dlgth = itt;
            if ((cmd = nxtwrd(cmd)) != NULL) {
              if (cmpwrd("n", cmd) || cmpwrd("numeric", cmd))
                pcm->dlgnu = 1;
            } else
              pcm->dlgnu = 0;
          }
    ...
    snprintf(pout, 1255, "SET DIALOG values:  Pc, Fc, Bc, Oc = %i %i %i %i ", pcm->dlgpc, pcm->dlgfc, pcm->dlgbc, pcm->dlgoc);
    gaprnt(2, pout);
    if (pcm->dlgnu) {
    ...
    pcm->xlpos = val1;
    if ((cmd = nxtwrd(cmd)) != NULL) {
      if (cmpwrd("b", cmd) || cmpwrd("bottom", cmd))
        pcm->xlside = 0;
      if (cmpwrd("t", cmd) || cmpwrd("top", cmd))
        pcm->xlside = 1;
    }
    if (pcm->xlside)
    ...
    pcm->ylpflg = 1;
    if ((cmd = nxtwrd(cmd)) != NULL) {
      if (cmpwrd("r", cmd) || cmpwrd("right", cmd))
        pcm->ylside = 1;
      if (cmpwrd("l", cmd) || cmpwrd("left", cmd))
        pcm->ylside = 0;
    }
    ...
    snprintf(pout, 1255, "SET STRING values:  color = %i  just = %s", pcm->strcol, justs[pcm->strjst]);
    gaprnt(2, pout);
    ...
    if ((cmd = nxtwrd(cmd)) == NULL)
      goto err;
    if (getdbl(cmd, &(pcm->axmin)) == NULL)
      goto err;
    if ((cmd = nxtwrd(cmd)) == NULL)
      goto err;
    if (getdbl(cmd, &(pcm->axmax)) == NULL)
      goto err;
    pcm->axflg = 1;
    pcm->axint = 0.0;
    if ((cmd = nxtwrd(cmd)) != NULL) {
    ...
    if ((cmd = nxtwrd(cmd)) == NULL)
      goto err;
    if (getdbl(cmd, &(pcm->aymin)) == NULL)
      goto err;
    if ((cmd = nxtwrd(cmd)) == NULL)
      goto err;
    if (getdbl(cmd, &(pcm->aymax)) == NULL)
      goto err;
    pcm->ayflg = 1;
    pcm->ayint = 0.0;
    if ((cmd = nxtwrd(cmd)) != NULL) {
    ...
    if (cmpwrd("auto", cmd))
      ...
      if (getdbl(cmd, &(pcm->rmin)) == NULL)
        goto err;
      pcm->aflag = 0;
      if ((cmd = nxtwrd(cmd)) == NULL)
        goto err;
      if (getdbl(cmd, &(pcm->rmax)) == NULL)
        goto err;
      pcm->aflag = -1;
    ...
    if ((cmd = nxtwrd(cmd)) == NULL)
      goto err;
    if (getdbl(cmd, &(pcm->rmin2)) == NULL)
      goto err;
    pcm->aflag2 = 0;
    if ((cmd = nxtwrd(cmd)) == NULL)
      goto err;
    if (getdbl(cmd, &(pcm->rmax2)) == NULL)
      goto err;
    pcm->aflag2 = -1;
    ...
    if ((cmd = nxtwrd(cmd)) == NULL)
      goto err;
    if (intprs(cmd, &(pcm->strmden)) == NULL)
      goto err;
    if ((cmd = nxtwrd(cmd)) != NULL) {
      if (getdbl(cmd, &tt) == NULL)
        goto err;
      if (tt > 0.001)
        pcm->strmarrd = tt;
      if ((cmd = nxtwrd(cmd)) != NULL) {
    ...
    if (cmpwrd("x", cmd))
      pcm->loopdim = 0;
    else if (cmpwrd("y", cmd))
      pcm->loopdim = 1;
    else if (cmpwrd("z", cmd))
      pcm->loopdim = 2;
    else if (cmpwrd("t", cmd))
      pcm->loopdim = 3;
    else if (cmpwrd("e", cmd))
    ...
    snprintf(pout, 1255, "E set to %g %g \n", pcm->dmin[4], pcm->dmax[4]);
    gaprnt(2, pout);
  } else if (cmpwrd("x", cmd) || cmpwrd("y", cmd) || cmpwrd("z", cmd) || cmpwrd("t", cmd)) {
    if (*cmd == 'x')
      kwrd = 0;
    if (*cmd == 'y')
      kwrd = 1;
    ...
    if (kwrd == 3) {
      ...
      if (num == 1)
        ...
        gr2t(vals, v2, &(pcm->tmax));
      gaprnt(2, "Time values set: ");
      snprintf(pout, 1255, "%i:%i:%i:%i ", pcm->tmin.yr, pcm->tmin.mo, pcm->tmin.dy, pcm->tmin.hr);
      gaprnt(2, pout);
      snprintf(pout, 1255, "%i:%i:%i:%i \n", pcm->tmax.yr, pcm->tmax.mo, pcm->tmax.dy, pcm->tmax.hr);
      gaprnt(2, pout);
    } else {
      if (pfi->type == 1) {
        conv = pfi->gr2ab[kwrd];
    ...
    if (pcm->pfid == NULL)
      goto errf;
    num = 1;
    if (getdbl(cmd, &(pcm->dmin[id])) == NULL)
      goto err;
    pcm->dmax[id] = pcm->dmin[id];
    if ((cmd = nxtwrd(cmd)) != NULL) {
      num = 2;
      if (getdbl(cmd, &(pcm->dmax[id])) == NULL)
        goto err;
      if (dequal(pcm->dmin[id], pcm->dmax[id], 1.0e-8) == 0)
        num = 1;
    }
    pcm->vdim[id] = num - 1;
    pfi = pcm->pfid;
    ...
    num = 1;
    tdef = pcm->tmin;
    if (adtprs(cmd, &tdef, &(pcm->tmin)) == NULL)
      goto err;
    pcm->tmax = pcm->tmin;
    if ((cmd = nxtwrd(cmd)) != NULL) {
      num = 2;
      if (adtprs(cmd, &tdef, &(pcm->tmax)) == NULL)
        goto err;
    }
    pcm->vdim[3] = 1;
    if (num == 1) {
    ...
    if (cmpwrd("on", cmd)) {
      ...
      if (intprs(cmd, &num) == NULL)
        goto err;
      if (num != -90 && num != 90 && num != -60 && num != 60 && num != -30 && num != 30 && num != -45 && num != 45 && num != 0) {
...
gaint gaopen(char *name, struct gacmn *pcm) {
  ...
  if (*name == 'h' && *(name + 1) == 't' && *(name + 2) == 't' && *(name + 3) == 'p' && *(name + 4) == ':' && *(name + 5) == '/' &&
      *(name + 6) == '/') {
    rc = dappfi(name, pfi);
  } else {
...
#include <readline/history.h>
  ...
  if (ie > history_length)
    ie = history_length;
  for (i = 1; i <= in; i++)
    com = nxtwrd(com);
  if (com) {
    if (verbose)
      printf("Writing command history to file '%s'.\n", com);
    fileout = 1;
    logfile = strtok(com, " ");
    ...
    if (strcmp(".gs", com + strlen(com) - 3) == 0) {
      pformat = 1;
    } else if (strcmp(".eps", com + strlen(com) - 4) == 0 || strcmp(".EPS", com + strlen(com) - 4) == 0) {
      pformat = 3;
  ...
  if (cmpwrd("history", cmd) || cmpwrd("his", cmd)) {
    ...
    for (i = is; i <= ie; i++) {
      if (cmpwrdl(his_cmd[i - 1]->line, "his") || cmpwrdl(his_cmd[i - 1]->line, "history") || cmpwrdl(his_cmd[i - 1]->line, "r") ||
  ...
  if (attname != NULL) {
    ...
    while (attr) {
      if (!strcmp(attr->varname, varname)) { /* varname must match */
        ...
        if (attr->nctype <= 2) {
          rc = nc_put_att_text(pcm->ncwid, varid, attr->name, attr->len, (char *)attr->value);
          if (rc) {
          ...
          rc = nc_put_att_short(pcm->ncwid, varid, attr->name, attr->nctype, attr->len, (short *)attr->value);
          if (rc) {
          ...
          rc = nc_put_att_long(pcm->ncwid, varid, attr->name, attr->nctype, attr->len, (long *)attr->value);
          if (rc) {
          ...
          rc = nc_put_att_float(pcm->ncwid, varid, attr->name, attr->nctype, attr->len, (gafloat *)attr->value);
          if (rc) {
          ...
          rc = nc_put_att_double(pcm->ncwid, varid, attr->name, attr->nctype, attr->len, (gadouble *)attr->value);
          if (rc) {
...
gaint sdfwdim(struct gafile *pfi, struct gacmn *pcm, gaint dim, gaint varid) {
  ...
  if (dim == 3) {
    ...
    flag = 0;
    if (*(pfi->grvals[3] + 5) > 0)
      flag = 1; /* change time unit to months instead of minutes */
  ...
  if ((dim == 4 && (pcm->sdfrecdim == 2 || pcm->sdfrecdim == 3)) || (dim == 3 && (pcm->sdfrecdim == 1 || pcm->sdfrecdim == 3))) {
...
gaint sdfdefdim(gaint fileid, char *dimname, gaint dimsize, gaint *dimid, gaint *varid, gaint recflg, gaint sdfwtype) {
#if USENETCDF == 1
  gaint rc;
  size_t chunksize;
...
gaint ncwrite(char *cmd, struct gacmn *pcm) {
  ...
  /* Create the file. */
  if (pcm->sdfwname)
    filename = pcm->sdfwname; /* override default filename */
  if (pcm->sdfwtype == 2)
#if HAVENETCDF4 == 1
  ...
  if (pcm->sdfwpad) {
    padX = 1;
    padY = 1;
    if (pcm->sdfwpad == 1 || pcm->sdfwpad == 3 || pcm->sdfwpad == 5)
      padZ = 1;
    if (pcm->sdfwpad >= 2)
      padT = 1;
    if (pcm->sdfwpad >= 4)
  ...
  if (pfi->dnum[0] > 1 || padX) {
    /* define the dimension */
    if (sdfdefdim(pcm->ncwid, "lon", pfi->dnum[0], &xdimid, &xvarid, 0, pcm->sdfwtype))
      goto err;
    /* assign default and user-defined attributes */
    if (sdfwatt(pcm, xvarid, "lon", "units", "degrees_east"))
      goto err;
    if (sdfwatt(pcm, xvarid, "lon", "long_name", "Longitude"))
      goto err;
    if (sdfwatt(pcm, xvarid, "lon", NULL, NULL))
      goto err;
  ...
  if (pfi->dnum[1] > 1 || padY) {
    /* define the dimension */
    if (sdfdefdim(pcm->ncwid, "lat", pfi->dnum[1], &ydimid, &yvarid, 0, pcm->sdfwtype))
      goto err;
    /* assign default and user-defined attributes */
    if (sdfwatt(pcm, yvarid, "lat", "units", "degrees_north"))
      goto err;
    if (sdfwatt(pcm, yvarid, "lat", "long_name", "Latitude"))
      goto err;
    if (sdfwatt(pcm, yvarid, "lat", NULL, NULL))
      goto err;
    /* increment the number of varying dimensions */
    vdims[nvdims] = 1;
    nvdims++;
  ...
  if (pfi->dnum[2] > 1 || padZ) {
    /* define the dimension */
    if (sdfdefdim(pcm->ncwid, "lev", pfi->dnum[2], &zdimid, &zvarid, 0, pcm->sdfwtype))
      goto err;
    /* assign default and user-defined attributes */
    if (sdfwatt(pcm, zvarid, "lev", "units", "millibar"))
      goto err;
    if (sdfwatt(pcm, zvarid, "lev", "long_name", "Level"))
      goto err;
    if (sdfwatt(pcm, zvarid, "lev", NULL, NULL))
      goto err;
    /* increment the number of varying dimensions */
    vdims[nvdims] = 2;
    nvdims++;
  ...
  if (pfi->dnum[3] > 1 || padT) {
    /* define the dimension */
    recflg = 0;
    if ((pcm->sdfrecdim == 1) || (pcm->sdfrecdim == 3))
      recflg = 1;
    if (sdfdefdim(pcm->ncwid, "time", pfi->dnum[3], &tdimid, &tvarid, recflg, pcm->sdfwtype))
      goto err;
    /* assign default and user-defined attributes */
    if (sdfwatt(pcm, tvarid, "time", "long_name", "Time"))
      goto err;
    if (sdfwatt(pcm, tvarid, "time", NULL, NULL))
      goto err;
    ...
    if ((*(pfi->grvals[3] + 6) != 0))
      snprintf(tunit, 255, "minutes since %04d-%02d-%02d %02d:%02d", tinit.yr, tinit.mo, tinit.dy, tinit.hr, tinit.mn);
    else
      snprintf(tunit, 255, "months since %04d-%02d-%02d %02d:%02d", tinit.yr, tinit.mo, tinit.dy, tinit.hr, tinit.mn);
    if (sdfwatt(pcm, tvarid, "time", "units", tunit))
      goto err;
    /* if necessary, assign 365_day_calendar attribute */
    if ((*(pfi->grvals[3] + 6) != 0) && (pfi->calendar == 1)) {
      if (sdfwatt(pcm, tvarid, "time", "calendar", "noleap"))
        goto err;
    }
    /* increment the number of varying dimensions */
    vdims[nvdims] = 3;
  ...
  if (pfi->dnum[4] > 1 || padE) {
    ...
    if (pcm->sdfrecdim >= 2)
      recflg = 1;
    if (sdfdefdim(pcm->ncwid, "ens", pfi->dnum[4], &edimid, &evarid, recflg, pcm->sdfwtype))
      goto err;
    /* assign default and user-defined attributes */
    if (sdfwatt(pcm, evarid, "ens", "grads_dim", "e"))
      goto err;
    if (sdfwatt(pcm, evarid, "ens", "long_name", "Ensemble member"))
      goto err;
    if (sdfwatt(pcm, evarid, "ens", NULL, NULL))
      goto err;
    /* increment the number of varying dimensions */
    vdims[nvdims] = 4;
    nvdims++;
  ...
  */
  for (i = 0; i < nvdims; i++) {
    if (vdims[nvdims - 1 - i] == 4)
      dimids[i] = edimid;
    else if (vdims[nvdims - 1 - i] == 3)
      dimids[i] = tdimid;
    else if (vdims[nvdims - 1 - i] == 2)
      dimids[i] = zdimid;
    else if (vdims[nvdims - 1 - i] == 1)
      dimids[i] = ydimid;
    else if (vdims[nvdims - 1 - i] == 0)
      dimids[i] = xdimid;
  }
  ...
  if (pcm->sdfchunk || pcm->sdfzip) {
    ...
    for (i = 0; i < nvdims; i++) {
      if (vdims[nvdims - 1 - i] == 4)
        chunksize[i] = pcm->echunk ? pcm->echunk : 1;
      else if (vdims[nvdims - 1 - i] == 3)
        chunksize[i] = pcm->tchunk ? pcm->tchunk : 1;
      else if (vdims[nvdims - 1 - i] == 2)
        chunksize[i] = pcm->zchunk ? pcm->zchunk : 1;
      else if (vdims[nvdims - 1 - i] == 1)
        chunksize[i] = pcm->ychunk ? pcm->ychunk : pfi->dnum[1];
      else if (vdims[nvdims - 1 - i] == 0)
        chunksize[i] = pcm->xchunk ? pcm->xchunk : pfi->dnum[0];
    }
  ...
  if (sdfwatt(pcm, varid, name, NULL, NULL))
    goto err;
  if (sdfwatt(pcm, NC_GLOBAL, "global", NULL, NULL))
    goto err;
  ...
  if (pfi->dnum[0] > 1 || padX)
    if (sdfwdim(pfi, pcm, 0, xvarid))
      goto err;
  if (pfi->dnum[1] > 1 || padY)
    if (sdfwdim(pfi, pcm, 1, yvarid))
      goto err;
  if (pfi->dnum[2] > 1 || padZ)
    if (sdfwdim(pfi, pcm, 2, zvarid))
      goto err;
  if (pfi->dnum[3] > 1 || padT)
    if (sdfwdim(pfi, pcm, 3, tvarid))
      goto err;
  if (pfi->dnum[4] > 1 || padE)
    if (sdfwdim(pfi, pcm, 4, evarid))
      goto err;
...
SHPHandle gaopshp(char *shparg) {
  ...
  while (sdir != NULL) {
    ...
    while (*(sdir + i) != ' ' && *(sdir + i) != ';' && *(sdir + i) != ',' && *(sdir + i) != ':' && *(sdir + i) != '\0') {
      i++;
      ch1++;
    }
    ...
    for (j = 0; j < ch1; j++)
      *(fname + j) = *(sdir + j); /* copy dir harvested from sdir to fname */
    len = ch1;
    if (*(fname + ch1 - 1) != '/') {
      *(fname + ch1) = '/';
      len++;
    } /* add slash to fname if needed */
    for (j = 0; j < ch2; j++)
      *(fname + len + j) = *(shparg + j); /* copy shparg to fname */
    len += ch2;
    *(fname + len) = '\0';
    ...
    if (*(sdir) == '\0')
      ...
      sdir++; /* advance past delimiter */
    while (*(sdir) == ' ' && *(sdir) != '\0')
      sdir++; /* advance past any extra spaces */
  }

  /* final option: try prepending GADDIR */
  if ((id = SHPOpen(gxgnam(shparg), "rb")) != NULL)
    return (id);
...
DBFHandle gaopdbf(char *shparg) {
  ...
  /* final option: try prepending GADDIR */
  if ((id = DBFOpen(gxgnam(shparg), "rb")) != NULL)
    return (id);
...
struct dbfld *parsedbfld(char *ch) {
  ...
  if (!(strncasecmp(fldtype, "string", 6)) || !(strncasecmp(fldtype, "char", 4))) {
    dbtype = FTString;
...
int gadoexpr(char *expr, struct pygagrid *pypgr) {
  ...
  pst = getpst(pcm);
  if (pst == NULL) {
    printf("Error in gadoexpr: getpst failed\n");
    goto reterr;
  }
  ...
  rc = gaexpr(expr, pst);
  if (rc) {
    printf("Error in gadoexpr: gaexpr failed\n");
    goto reterr;
  }
...
int gadoget(char *vnm, struct pygagrid *pypgr) {
  ...
  pypgr->xvals = (double *)malloc(nvals * sizeof(double));
  if (pypgr->xvals == NULL) {
    printf("Error in gadoget: memory allocation failed for xvals\n");
    goto reterr;
  }
  s = pypgr->xvals;
  /* write world coordinate values, use NaN if dimension isn't varying */
  ...
  pypgr->yvals = (double *)malloc(nvals * sizeof(double));
  if (pypgr->yvals == NULL) {
    printf("Error in gadoget: memory allocation failed for yvals\n");
    goto reterr;
  }
  s = pypgr->yvals;
  /* write world coordinate values, use NaN if dimension isn't varying  */
  ...
  pypgr->zvals = (double *)malloc(pypgr->zsz * sizeof(double));
  if (pypgr->zvals == NULL) {
    printf("Error in gadoget: memory allocation failed for zvals\n");
    goto reterr;
  }
  s = pypgr->zvals;
  /* write world coordinate values, use NaN if dimension isn't varying  */
...
int gasetup(char *ch, struct pygagrid *pypgr) {
  ...
  pypfi = getpfi();
  if (pypfi == NULL) {
    printf("Put Error: getpfi failed\n");
    goto reterr;
  }
