...
gadouble t2gr(gadouble *vals, struct dt *dtim) {
  ...
  if (*mnincr > 0.1) {
    ...
    if (stim.yr > dtim->yr)
      eyear = dtim->yr;
    rdiff = (((dtim->yr - eyear) * 12) + dtim->mo) - (((stim.yr - eyear) * 12) + stim.mo);
    stim.yr = dtim->yr;
    stim.mo = dtim->mo;
    mins = timdif(&stim, dtim, 0);
    if (mins > 0) {
...
void gr2t(gadouble *vals, gadouble gr, struct dt *dtim) {
  ...
  if (*mnincr > 0.1) {
    ...
    if (dtim->mo < 0) {
      ...
      timadd(&stim, dtim);
    if (v < 0.0001)
      return; /* if fraction small, return       */
...
static char *mons[12] = {"jan", "feb", "mar", "apr", "may", "jun", "jul", "aug", "sep", "oct", "nov", "dec"};
...
gadouble liconv(gadouble *vals, gadouble v) { return ((*vals * v) + *(vals + 1)); }
...
gadouble gr2lev(gadouble *vals, gadouble gr) {
  gaint i;
  if (gr < 1.0)
    return (*(vals + 1) + (1.0 - gr) * (*(vals + 1) - *(vals + 2)));
  if (gr > *vals) {
    i = (gaint)(*vals + 0.1);
...
char *dimprs(char *ch, struct gastat *pst, struct gafile *pfi, gaint *dim, gadouble *d, gaint type, gaint *wflag) {
  ...
  if (cmpwrd("x", name))
    *dim = 0;
  else if (cmpwrd("y", name))
    *dim = 1;
  else if (cmpwrd("z", name))
    *dim = 2;
  else if (cmpwrd("t", name))
    *dim = 3;
  else if (cmpwrd("offt", name))
    *dim = 3;
  else if (cmpwrd("e", name))
    *dim = 4;
  else if (cmpwrd("lon", name))
    *dim = 5;
  else if (cmpwrd("lat", name))
    *dim = 6;
  else if (cmpwrd("lev", name))
    *dim = 7;
  else if (cmpwrd("time", name))
    *dim = 8;
  else if (cmpwrd("ens", name))
    *dim = 9;
  else if (type == 0 && cmpwrd("r", name))
    *dim = 10;
  else {
    gaprnt(0, "Syntax Error:  Invalid dimension expression\n");
    ...
    *wflag = 1;
    ...
    if (*dim == 4) {
      ...
      while (i < pfi->dnum[*dim]) {
        if (strcmp(ename, ens->name) == 0)
          enum1 = i; /* grid coordinate of matching name */
        i++;
        ens++;
      }
      if (enum1 < 0) {
...
static gadouble glts15[40] = {-86.60, -82.19, -77.76, -73.32, -68.88, -64.43, -59.99, -55.55, -51.11, -46.66,
                              -42.22, -37.77, -33.33, -28.89, -24.44, -20.00, -15.55, -11.11, -6.67,  -2.22,
                              2.22,   6.67,   11.11,  15.55,  20.00,  24.44,  28.89,  33.33,  37.77,  42.22,
                              46.66,  51.11,  55.55,  59.99,  64.43,  68.88,  73.32,  77.76,  82.19,  86.60};
static gadouble glts20[52] = {-87.38, -83.98, -80.56, -77.13, -73.71, -70.28, -66.85, -63.42, -59.99, -56.57, -53.14,
                              -49.71, -46.28, -42.85, -39.43, -36.00, -32.57, -29.14, -25.71, -22.28, -18.86, -15.43,
                              -12.00, -8.57,  -5.14,  -1.71,  1.71,   5.14,   8.57,   12.00,  15.43,  18.86,  22.28,
                              25.71,  29.14,  32.57,  36.00,  39.43,  42.85,  46.28,  49.71,  53.14,  56.57,  59.99,
    2.8571,   4.76184,  6.66657,  8.57131,  10.47604, 12.3808,   14.2855,  16.1902,  18.095,   19.9997,  21.9044,   23.8092,
    25.7139,  27.6186,  29.5234,  31.4281,  33.3328,  35.2375,   37.1422,  39.047,   40.9517,  42.8564,  44.7611,   46.6658,
    48.5705,  50.4752,  52.3799,  54.2846,  56.1893,  58.0939,   59.9986,  61.9033,  63.8079,  65.7125,  67.6171,   69.5217,
    71.4262,  73.3307,  75.2351,  77.1394,  79.0435,  80.9473,   82.8508,  84.7532,  86.6531,  88.542};
...
gadouble *gagst62(gaint istrt, gaint num) {
  ...
  *vals = (gadouble)num;
  for (size = 0; size < num; size++)
    *(vals + size + 1) = gltst62[size + istrt];
  *(vals + num + 1) = -999.9;
  return (vals);
}

char *monc[12] = {"JAN", "FEB", "MAR", "APR", "MAY", "JUN", "JUL", "AUG", "SEP", "OCT", "NOV", "DEC"};
...
char *gafndt(char *fn, struct dt *dtim, struct dt *dtimi, gadouble *vals, struct gachsub *pch1st, struct gaens *ens1st, gaint t,
             gaint e, gaint *flag) {
  ...
  olen = 0;
  while (*(fn + olen))
    olen++; /* get length of original DSET entry */
  olen += 5;
  sz = olen;
  ...
  *out = '\0';
  ...
  while (*in) {
    ...
    /* forecast hour */
      ...
      if (tdif < 99)
        snprintf(out, sz, "%02i", tdif);
      else
        snprintf(out, sz, "%i", tdif);
      while (*out)
        out++;
      in += 3;
    ...
    /* forecast time in hours/minutes (hhnn) */
      ...
      mns = tdif - (hrs * 60);
      if (hrs < 99)
        snprintf(out, sz, "%02i%02i", hrs, mns);
      else
        snprintf(out, sz, "%i%02i", hrs, mns);
      while (*out)
        out++;
      in += 4;
    }
    /* forecast time in days/hours/minutes (ddhhnn) */
      ...
      if (dys < 99)
        snprintf(out, sz, "%02i%02i%02i", dys, hrs, mns);
      else
        snprintf(out, sz, "%i%02i%02i", dys, hrs, mns);
      while (*out)
        out++;
      in += 5;
    }
...
void gahswp(struct rpthdr *hdr) { gabswp((gafloat *)(&(hdr->lat)), 5); }
...
gaint dayweek(struct dt *dtime) {
  struct dt anch;
  gaint i, j;
  if (dtime->yr < 1950 || dtime->yr > 3000)
    return (7);
  anch.yr = 1950;
  anch.mo = 1;
  anch.dy = 1;
...
gafloat ieee2flt(unsigned char *ieee) {
  ...
  fmant = (gadouble)((gaint)ieee[3] + (gaint)(ieee[2] << 8) + (gaint)((ieee[1] | 128) << 16));
  if (ieee[0] & 128)
    fmant = -fmant;
...
gadouble *cpscal(gadouble *vals, gaint lin, gaint dir, gaint dim) {
  ...
  if (dim == 3) {
    ...
    if (lin == 1)
      ...
      num = (gaint)(*vals + 0.5) + 5;
  }
  sz = sizeof(gadouble) * num;
  if (dir)
    sprintf(name, "cpscala%d", dim);
  else
    sprintf(name, "cpscalg%d", dim);
...
gaint fseeko(FILE *stream, off_t offset, gaint whence) { fseek(stream, (long)offset, whence); }
...
gaint nxrdln(char *cmd, char *prompt) {
  ...
  if (ch == NULL) {
    ...
    ch2 = ch;
    while (*ch == ' ')
      ch++; /* Skip leading blanks */
    strcpy(cmd, ch);
    if (*ch)
      add_history(ch); /* Skip blank lines */
  }
  return (strlen(cmd) + 1);
}
