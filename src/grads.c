...
int main(int argc, char *argv[]) {
  ...
  if (argc > 1) {
    for (i = 1; i < argc; i++) {
      /* user needs help */
      if (*(argv[i] + 0) == '-' && *(argv[i] + 1) == 'h' && *(argv[i] + 2) == 'e' && *(argv[i] + 3) == 'l' &&
          *(argv[i] + 4) == 'p') {
        command_line_help();
        return (0);
        ...
        while (*(argv[i] + j)) {
          if (*(argv[i] + j) == 'a')
            ratioflg = 1; /* aspect ratio to follow */
          else if (*(argv[i] + j) == 'b')
            gcmn.batflg = 1; /* batch mode */
          else if (*(argv[i] + j) == 'c')
            cmdflg = 1; /* command to follow */
          else if (*(argv[i] + j) == 'C')
            txtcs = -1; /* text color scheme */
          else if (*(argv[i] + j) == 'd')
            gxd = 1; /* graphics display back end choice  */
          else if (*(argv[i] + j) == 'E')
            hstflg = 0; /* disable command line editing */
          else if (*(argv[i] + j) == 'g')
            geomflg = 1; /* geometry specification to follow */
          else if (*(argv[i] + j) == 'h')
            gxp = 1; /* graphics hardcopy printing back end choice */
          else if (*(argv[i] + j) == 'H')
            wrhstflg = 1; /* write history to log file */
          else if (*(argv[i] + j) == 'l')
            land = 1; /* landscape mode */
          else if (*(argv[i] + j) == 'm')
            metabuff = 1; /* metafile buffer size to follow */
          else if (*(argv[i] + j) == 'p')
            port = 1;                       /* portrait mode */
            ...
            killflg = 1; /* quit after finishing (usually used with -c) */
          else
            printf("Unknown command line option: %c\n", *(argv[i] + j));
          j++;
        }
      } else
        printf("Unknown command line keyword: %s\n", argv[i]);
    }
  }

  if (txtcs > -2)
    gatxti(1, txtcs); /* turn on text colorizing */

  if (ratioflg == 1)
    printf("Note: -a option was specified, but no aspect ratio was provided\n");
  if (cmdflg == 1)
    printf("Note: -c option was specified, but no command was provided\n");
  if (geomflg == 1)
    printf("Note: -g option was specified, but no geometry specification was provided\n");
  if (metabuff == 1)
    printf("Note: -m option was specified, but no metafile buffer size was provided\n");
  ...
  if (icmd)
    rc = gacmd(icmd, &gcmn, 0); /* execute command given on startup */
  else
    rc = 0; /* set up for entering the main command line loop */
...
gaint gaqsig(void) { return (gcmn.sig); }
...
void gaudpdef(void) {
  ...
  while (pass < 2) {
    ...
    while (1) {
      ...
      err = 0;
      if (*ch == '*' || *ch == '#')
        continue; /* Check if this is a comment field */
      /* allocate memory */
      upb = (struct gaupb *)malloc(sizeof(struct gaupb));
      if (upb == NULL)
        goto memerr;
      while (*ch == ' ')
        ch++; /* move past leading blanks */
      ...
      lowcas(ptype);
      upb->type = 0;
      if (!strncmp(ptype, "function", 8))
        upb->type = 1;
      /* if (!strncmp(ptype,"defop",5))     upb->type=2; */
      if (!strncmp(ptype, "gxdisplay", 9))
        upb->type = 3;
      if (!strncmp(ptype, "gxprint", 7))
        upb->type = 4;
      if (upb->type == 0) {
        err = 1;
        goto fmterr;
      ...
      while (*ch == ' ')
        ch++; /* move past any in-between blanks */
      ...
      if (upb->type <= 2)
        lowcas(upb->name); /* gaexpr needs functions to be lower case */
      while (*ch == ' ')
        ch++; /* move past any in-between blanks */
    ...
    fmterr:
      if (err) {
        ...
        if (err == 1)
          printf("  Plug-in type must be either 'function', 'gxdisplay', or 'gxprint'\n");
        if (err == 2)
          printf("  Plug-in name and shared object filename are missing\n");
        if (err == 3)
          printf("  Shared object filename is missing\n");
      ...
      for (i = 0; i < len; i++)
        upb->fname[i] = newname[i];
      upb->fname[len] = '\0';

      for (i = 0; i < sz; i++)
        ch++; /* move past the shared object file name */
      while (*ch == ' ')
        ch++; /* move past any in-between blanks */
...
char *gaqupb(char *name, gaint type) {
  ...
  upb = upba;
  while (upb) {
    if (cmpwrd(upb->name, name) && upb->type == type)
      return (upb->fname);
    upb = upb->upb;
  }
  return (NULL);
