...
gaint main(gaint argc, char *argv[]) {
  ...
  while (recnum) {
    ...
    while (1) {
      ...
      for (i = 0; i < 257; i++) {
        if (*(work + i + 0) == 'G' && *(work + i + 1) == 'R' && *(work + i + 2) == 'I' && *(work + i + 3) == 'B') {
          fpos = fpos + (off_t)i;
          flg = 1;
          i = 999;
        }
    ...
    printf("\nRecord %d ", recnum);
    if (verb)
      printf("starts at %i of length %i \n", i, rlen);
    else
      printf("\n");
    if (verb) {
    ...
    while (1) {
      ...
      s7len = gagby(s7, 0, 4);
      if (verb > 1)
        printf("  Lengths of Sections 1-7: %i %i %i %i %i %i %i\n", s1len, s2len, s3len, s4len, s5len, s6len, s7len);
      roff += s7len;

      if (ctl)
        printf(" ctl:  var%d.%d   0,%s  %s  %s   description\n", recnum, field, levs, extra, units);
...
gaint sect1(unsigned char *s1, struct gag2 *pg2) {
  ...
  printf(" Reference Time = %4i-%02i-%02i %02i:%02i:%02i  ", pg2->yr, pg2->mo, pg2->dy, pg2->hr, pg2->mn, pg2->sc);
...
gaint sect4(unsigned char *s4, struct gag2 *pg2) {
  ...
  if (sp < 0 || sp == 255) {
    ...
    if (ctl)
      sprintf(units, "%d,%d,%d", pg2->discipline, pg2->parcat, pg2->parnum);
  } else {
    if (sp2 < 0) {
      printf("   Parameter: disc,cat,num,sp = %d,%d,%d,%d\n", pg2->discipline, pg2->parcat, pg2->parnum, sp);
      if (ctl)
        sprintf(units, "%d,%d,%d,%d", pg2->discipline, pg2->parcat, pg2->parnum, sp);
    } else {
      printf("   Parameter: disc,cat,num,sp,sp2 = %d,%d,%d,%d,%d\n", pg2->discipline, pg2->parcat, pg2->parnum, sp, sp2);
      if (ctl)
        sprintf(units, "%d,%d,%d,%d,%d", pg2->discipline, pg2->parcat, pg2->parnum, sp, sp2);
    }
  }

  /* Get level information */
  if (pg2->lev1 != -1)
    lev1 = scaled2dbl(pg2->lev1sf, pg2->lev1);
  if (pg2->lev2type != 255) { /* we have two level types */
    if (pg2->lev2 != -1) {
      ...
      if (pg2->lev2type == pg2->lev1type) {
        ...
        if (ctl)
          sprintf(levs, "%d,%g,%g", pg2->lev1type, lev1, lev2);
      } else {
        printf("   Levels: ltype,lval,lval2,ltype2 = %d,%g,%g,%d ", pg2->lev1type, lev1, lev2, pg2->lev2type);
        if (ctl)
          sprintf(levs, "%d,%g,%g,%d", pg2->lev1type, lev1, lev2, pg2->lev2type);
      }
      if (verb)
      ...
      if (pg2->lev2type == pg2->lev1type) {
        ...
        if (ctl)
          sprintf(levs, "%d", pg2->lev1type);
      } else {
        printf("   Levels: ltype,,,ltype2 = %d,,,%d \n", pg2->lev1type, pg2->lev2type);
        if (ctl)
          sprintf(levs, "%d,,,%d", pg2->lev1type, pg2->lev2type);
      }
    }
    ...
    if (pg2->lev1 != -1 && pg2->lev1type > 10) { /* some level types don't have an associated level value */
      ...
      if (ctl)
        sprintf(levs, "%d,%g", pg2->lev1type, lev1);
      if (verb)
        printf("  (sf,sval = %d %d)", pg2->lev1sf, pg2->lev1);
      printf("\n");
    } else {
  ...
  if (pg2->pdt == 9 || pg2->pdt == 5) {
    ...
    if (ptyp <= 4) {
      if (ptyp == 2) {
        ...
        if (ctl)
          sprintf(extra, "a%d,%g,%g", ptyp, ll, ul);
      } else if (ptyp == 0 || ptyp == 3) {
        printf("   Prob: type,lim = %d,%g", ptyp, ll);
        if (ctl)
          sprintf(extra, "a%d,%g", ptyp, ll);
      } else {
        printf("   Prob: type,lim = %d,%g", ptyp, ul);
        if (ctl)
          sprintf(extra, "a%d,%g", ptyp, ul);
      }
    } else if (ptyp == 255)
      printf("   Probability Type is missing");
    else
      printf("   Probability Type = %d", ptyp);

    /* decoding code table 4p9 */
    if (verb) {
      if (ptyp == 0)
        printf("  (Probability of event below %g)", ll);
      else if (ptyp == 1)
        printf("  (Probability of event above %g)", ul);
      else if (ptyp == 2)
        printf("  (Probability of event between %g and %g)", ll, ul);
      else if (ptyp == 3)
        printf("  (Probability of event above %g)", ll);
      else if (ptyp == 4)
        printf("  (Probability of event below %g)", ul);
    }
    if (verb > 1)
      printf("   (llsf,llval=%d,%d  ulsf,ulval=%d,%d)", llsf, llval, ulsf, ulval);
    printf("\n");
  }
    ...
    if (atyp != 65535) {
      if (styp != 255) {
        if (wtyp != 255) {
          ...
          if (ctl)
            sprintf(extra, "a%d,%d,%g,%g,%d,%g,%g", atyp, styp, s1, s2, wtyp, w1, w2);
        } else {
          printf("   Aerosol: atyp,styp,s1,s2 = %d,%d,%g,%g", atyp, styp, s1, s2);
          if (ctl)
            sprintf(extra, "a%d,%d,%g,%g", atyp, styp, s1, s2);
        }
      } else {
        if (wtyp != 255) {
          printf("   Aerosol: atyp,wtyp,w1,w1 = %d,%d,%g,%g", atyp, wtyp, w1, w2);
          if (ctl)
            sprintf(extra, "a%d,%d,%g,%g", atyp, wtyp, w1, w2);
        } else {
          printf("   Aerosol: atyp = %d", atyp);
          if (ctl)
            sprintf(extra, "a%d", atyp);
        }
      }
    }
    if (verb) {
      ...
      if (wtyp <= 11) {
        /* code table 4p91 */
        if (wtyp == 0)
          printf("  (wavelength is < %g)", w1);
        else if (wtyp == 1)
          printf("  (wavelength is > %g)", w2);
        else if (wtyp == 2)
          printf("  (wavelength is >= %g and < %g)", w1, w2);
        else if (wtyp == 3)
          printf("  (wavelength is > %g)", w1);
        else if (wtyp == 4)
          printf("  (wavelength is < %g)", w2);
      ...
      if (verb > 1)
        printf("  (wavelength: sf1,val1=%d,%d  sf2,val2=%d,%d)", w1sf, w1sval, w2sf, w2sval);
    }
    printf(" \n");
  }
  return (0);
...
void CodeTable1p2(gaint i) {
  if (i == 0)
    ...
    printf("(Start of Forecast)");
  else if (i == 2)
    printf("(Verifying Time of Forecast)");
  else if (i == 3)
...
void CodeTable3p1(gaint i) {
  if (i == 0)
    ...
    printf("(Rotated Lat/Lon)");
  else if (i == 2)
    printf("(Stretched Lat/Lon)");
  else if (i == 3)
    printf("(Rotated and Stretched Lat/Lon)");
  else if (i == 10)
    printf("(Mercator)");
  else if (i == 20)
    ...
    printf("(Gaussian Lat/Lon)");
  else if (i == 41)
    printf("(Rotated Gaussian Lat/Lon)");
  else if (i == 42)
    printf("(Stretched Gaussian Lat/Lon)");
  else if (i == 43)
    printf("(Rotated and Stretched Gaussian Lat/Lon)");
  else if (i == 50)
    printf("(Spherical Harmonic Coefficients)");
  else if (i == 51)
    printf("(Rotated Spherical Harmonic Coefficients)");
  else if (i == 52)
    printf("(Stretched Spherical Harmonic Coefficients)");
  else if (i == 53)
    printf("(Rotated and Stretched Spherical Harmonic Coefficients)");
  else if (i == 90)
    printf("(Orthoraphic)");
  else if (i == 100)
    printf("(Triangular Grid Based on an Icosahedron)");
  else if (i == 110)
    printf("(Equatorial Azimuthal Equidistant Projection)");
  else if (i == 120)
    printf("(Azimuth-Range Projection)");
}
...
void CodeTable4p7(gaint i) {
  if (i == 0)
    printf("Unweighted Mean All Members ");
  else if (i == 1)
    printf("Weighted Mean All Members ");
  else if (i == 2)
    printf("StdDev (Cluster Mean) ");
  else if (i == 3)
    printf("StdDev (Cluster Mean, normalized) ");
  else if (i == 4)
    printf("Spread of All Members ");
  else if (i == 5)
    printf("Large Anomaly Index All Members ");
  else if (i == 6)
    printf("Unweighted Mean Cluster Members ");
}
...
void CodeTable4p15(gaint i) {
  if (i == 0)
    ...
    printf("Budget interpolation");
  else if (i == 5)
    printf("Spectral interpolation");
  else if (i == 6)
    printf("Neighbor budget interpolation");
  else
    printf("sp2=%d", i);
}
...
void CodeTable5p0(gaint i) {
  if (i == 0)
    printf("(Grid Point Data - Simple Packing)");
  else if (i == 1)
    printf("(Matrix Value at Grid Point - Simple Packing)");
  else if (i == 2)
    printf("(Grid Point Data - Complex Packing)");
  else if (i == 3)
    printf("(Grid Point Data - Complex Packing and Spatial Differencing)");
  else if (i == 40)
    printf("(Grid Point Data - JPEG2000 Compression)");
  else if (i == 41)
    printf("(Grid Point Data - PNG Compression )");
  else if (i == 50)
    printf("(Spectral Data - Simple Packing)");
  else if (i == 51)
    printf("(Spectral Data - Simple Packing)");
}

void gaprnt(gaint i, char *ch) { printf("%s", ch); }
char *gxgsym(char *ch) { return (getenv(ch)); }
