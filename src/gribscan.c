...
gaint main(gaint argc, char *argv[]) {
  ...
  if (skip == -999) {
    ...
    fpos = 0;
    while (fpos < flen && !(*(uch) == 'G' && *(uch + 1) == 'R' && *(uch + 2) == 'I' && *(uch + 3) == 'B')) {
      fpos++;
    ...
    if (fpos == flen) {
      ...
      if (verb)
        printf("the header is %lld bytes long\n", fpos);
    }
...
gaint gribhdr(struct grhdr *ghdr) {
  ...
  if (*rec != 'G' || *(rec + 1) != 'R' || *(rec + 2) != 'I' || *(rec + 3) != 'B') {
    ...
    while ((fpos < flen - 4) && (i < scanlim) && !(*ch == 'G' && *(ch + 1) == 'R' && *(ch + 2) == 'I' && *(ch + 3) == 'B')) {
      fpos++;
      i++;
      rc = fseeko(gfile, fpos, 0);
      if (rc)
  ...
  */
  ...
  if (len > 25) {
    ...
    */
    ...
    if (ghdr->dsf == 0.0)
      ghdr->dsf = 1.0; /* mf make sure dsf != 0 */
  } else
    ghdr->dsf = 1.0;
  ...
  if (!silent) {
    if (qout) {
      ...
      if (ghdr->ltyp == 100) {
        printf("%d, F ,%d,%d,%d,%d,%5g, T ,%d,%d,%d,%d,%d,%d,%d,%d", irec, ghdr->parm, ghdr->ltyp, ghdr->level, ghdr->tri,
               ghdr->dsf, ghdr->dtim.yr, ghdr->dtim.mo, ghdr->dtim.dy, ghdr->dtim.hr, ghdr->dtim.mn, ghdr->ftu, ghdr->p1, ghdr->p2);

        if (gvout)
          printf(", V , %s, %s, %s", vt[ghdr->parm].name, vt[ghdr->parm].desc, vt[ghdr->parm].units);

        printf(", G ,%d, BDTG, %02d%02d%02d%02d ", ghdr->gridid, yy, ghdr->dtim.mo, ghdr->dtim.dy, ghdr->dtim.hr);

      } else {
        printf("%d, F ,%d,%d,%d,%d,%d,%d,%5g, T ,%d,%d,%d,%d,%d,%d,%d,%d", irec, ghdr->parm, ghdr->ltyp, ghdr->level, ghdr->l1,
               ghdr->l2, ghdr->tri, ghdr->dsf, ghdr->dtim.yr, ghdr->dtim.mo, ghdr->dtim.dy, ghdr->dtim.hr, ghdr->dtim.mn, ghdr->ftu,
               ghdr->p1, ghdr->p2);
        if (gvout)
          printf(", V ,%s ,%s ,%s", vt[ghdr->parm].name, vt[ghdr->parm].desc, vt[ghdr->parm].units);
        printf(", G ,%d, BDTG, %02d%02d%02d%02d ", ghdr->gridid, yy, ghdr->dtim.mo, ghdr->dtim.dy, ghdr->dtim.hr);
      ...
      if (ghdr->ltyp == 100) {
        printf("%40s, F ,%d,%d,%d,%d,%d,%d,%5g, T ,%d,%d,%d,%d,%d,%d,%d,%d", ifname, ghdr->parm, ghdr->ltyp, ghdr->level, ghdr->l1,
               ghdr->l2, ghdr->tri, ghdr->dsf, ghdr->dtim.yr, ghdr->dtim.mo, ghdr->dtim.dy, ghdr->dtim.hr, ghdr->dtim.mn, ghdr->ftu,
               ghdr->p1, ghdr->p2);
        printf(", G ,%d, BDTG, %02d%02d%02d%02d ", ghdr->gridid, yy, ghdr->dtim.mo, ghdr->dtim.dy, ghdr->dtim.hr);
      } else {
      ...
      if (delim) {
        printf("PDS,%d,%d,%d,%d,%d,%d,%d,", irec, ghdr->gridid, ghdr->parm, ghdr->ltyp, ghdr->level, ghdr->l1, ghdr->l2);
        printf("%d,%d,%d,%d,%d,%d,%d,%d,%d,%g", ghdr->dtim.yr, ghdr->dtim.mo, ghdr->dtim.dy, ghdr->dtim.hr, ghdr->dtim.mn,
               ghdr->ftu, ghdr->p1, ghdr->p2, ghdr->tri, ghdr->dsf);
        if (gvout)
          printf(",%s,%s,%s", vt[ghdr->parm].name, vt[ghdr->parm].desc, vt[ghdr->parm].units);
      } else {
        printf("PDS #%-4d %2d %3d %3d %4d %3d %3d BMSFLG: %1d ", irec, ghdr->gridid, ghdr->parm, ghdr->ltyp, ghdr->level, ghdr->l1,
               ghdr->l2, ghdr->bmsflg);
        printf("%4d %02d %02d %02d %02d % 3d % 3d % 3d % 3d %8g", ghdr->dtim.yr, ghdr->dtim.mo, ghdr->dtim.dy, ghdr->dtim.hr,
               ghdr->dtim.mn, ghdr->ftu, ghdr->p1, ghdr->p2, ghdr->tri, ghdr->dsf);
        if (gvout)
          printf(" %6s %38s %16s", vt[ghdr->parm].name, vt[ghdr->parm].desc, vt[ghdr->parm].units);
      }
  ...
  if (ghdr->gdsflg) {
    ...
    */
    ...
    if (!silent) {
      if (gdsout && qout == 0) {
        if (delim) {
          printf(",GDS,%d,%d,%d,%-6.3f,%-6.3f,%-6.3f,%-6.3f,%-6.3f,%-6.3f,", ghdr->gtyp, ghdr->gicnt, ghdr->gjcnt, lo1, lo2, lodi,
                 la1, la2, ladi);
          printf("%d,%d,%1d", ghdr->gsf1, ghdr->gsf2, ghdr->gsf3);
        } else {
          printf(" GDS % 3d % 3d % 3d %6.3f %6.3f %6.3f %6.3f %6.3f %6.3f", ghdr->gtyp, ghdr->gicnt, ghdr->gjcnt, lo1, lo2, lodi,
                 la1, la2, ladi);
  ...
  if (ggout || gaout || gfout) {
    ...
    */
    ...
    if ((iok[0] != 0) || (iok[1] != 0) || (iok[2] != 0)) {
      ...
      if (iok[0] == 1 && iok[1] == 0 && iok[2] == 0)
        iokwrite = 1;
      if (iok[0] == 0 && iok[1] == 1 && iok[2] == 0)
        iokwrite = 1;
      if (iok[0] == 0 && iok[1] == 0 && iok[2] == 1)
        iokwrite = 1;
      if (iok[0] == 1 && iok[1] == 1 && iok[2] == 0)
        iokwrite = 1;
      if (iok[0] == 0 && iok[1] == 1 && iok[2] == 1)
        iokwrite = 1;
      if (iok[0] == 1 && iok[1] == 0 && iok[2] == 1)
        iokwrite = 1;
      if (iok[0] == 1 && iok[1] == 1 && iok[2] == 1)
        iokwrite = 1;

    } else {
      /*	dump all fields */
      if (gaout || gfout || ggout)
        iokwrite = 1;
    }
    ...
    if (iokwrite) {
      ...
      if (gaout) {
        ...
        fprintf(ofile, "Base DTG= %02d%02d%02d%02d  ", yy, ghdr->dtim.mo, ghdr->dtim.dy, ghdr->dtim.hr);
        ...
        rc = fwrite(gd.pds, 1, glen.pds, ofile);
        if (gd.gds != NULL)
          rc = fwrite(gd.gds, 1, glen.gds, ofile);
        if (gd.bms != NULL)
          rc = fwrite(gd.bms, 1, glen.bms, ofile);
        rc = fwrite(gd.bds, 1, glen.bds, ofile);
        strncpy((char *)gd.es, "7777", 4);
        rc = fwrite(gd.es, 1, 4, ofile);
      }
