...
struct gsrecd *gsrtyp(char **ppos, gaint *reccnt, gaint *rc) {
  ...
  if ((*pos >= 'a' && *pos <= 'z') || (*pos >= 'A' && *pos <= 'Z') || *pos == '_') {
    while ((*pos >= 'a' && *pos <= 'z') || (*pos >= 'A' && *pos <= 'Z') || (*pos == '.') || (*pos == '_') ||
           (*pos >= '0' && *pos <= '9'))
      pos++;
    while (*pos == ' ')
      pos++;
  ...
  while (recd) {
    ...
    if (recd != NULL && recd->type == 11) {
      ...
      if ((*fch >= 'a' && *fch <= 'z') || (*fch >= 'A' && *fch <= 'Z')) {
        i = 0;
        while ((*fch >= 'a' && *fch <= 'z') || (*fch >= 'A' && *fch <= 'Z') || (*fch >= '0' && *fch <= '9') || *fch == '_') {
          if (i > 15) {
...
struct gsrecd *gsbkst(struct gsrecd *recd, struct gsrecd *ifblk, struct gsrecd *doblk, gaint *rc) {
  gaint ret;
...
struct gsrecd *gsbkdo(struct gsrecd *recd, struct gsrecd *ifblk, struct gsrecd *doblk, gaint *rc) {
  gaint ret;
...
struct gsrecd *gsbkif(struct gsrecd *recd, struct gsrecd *ifblk, struct gsrecd *doblk, gaint *rc) {
  gaint ret, eflg;
...
char *gsexpr(char *expr, struct gscmn *pcmn) {
  ...
  while (1) {
    ...
    if (state) { /* Expect oprnd, unary op, '(' */
      ...
      else if (*pos == '\"' || *pos == '\'' || (*pos >= '0' && *pos <= '9')) {
        curr->type = 0;
        curr->obj.strng = gscnst(&pos);
        if (curr->obj.strng == NULL)
          goto err3;
        state = 0;
        uflag = 0;
      }
      ...
      else if (*pos == '\"' || *pos == '\'' || *pos == '_' || (*pos >= '0' && *pos <= '9') || (*pos >= 'a' && *pos <= 'z') ||
               (*pos >= 'A' && *pos <= 'Z')) {
        curr->type = 1;
        curr->obj.op = 9;
        state = 1;
      ...
      /*  Handle operator   */
        ...
        for (i = 0; i < 13; i++) {
          if (*pos != *(opchars[i]))
            continue;
          if (*(opchars[i] + 1) && (*(pos + 1) != *(opchars[i] + 1)))
            continue;
          flag = opvals[i];
          break;
        }
...
struct stck *gseval(struct stck *curr) {
  ...
  for (i = 0; i < 7; i++) {
    ...
    */
    srch = sbeg;
    while (srch != curr) {
      if (srch->type == 1 && srch->obj.op >= opmins[i] && srch->obj.op <= opmaxs[i]) {
        srch = gsoper(srch);
        if (srch == NULL)
          return (NULL);
      }
...
struct stck *gsoper(struct stck *soper) {
  ...
  if (optyps[op - 1]) {
    ...
    if (op < 14)
      gsnum(sop1->obj.strng, &ntyp1, &iv1, &v1);
    else
      ntyp1 = ntyp2;
    if (ntyp1 == 1 && ntyp2 == 1)
      ntype = 1;
    else if (ntyp1 == 0 || ntyp2 == 0)
      ntype = 0;
    else
      ntype = 2;
  }
...
char *gsfunc(char *pos, char *name, struct gscmn *pcmn) {
  ...
  pcmn->farg = avar;
  if (cmpwrd(name, "substr"))
    rc = gsfsub(pcmn);
  else if (cmpwrd(name, "subwrd"))
    rc = gsfwrd(pcmn);
  else if (cmpwrd(name, "sublin"))
    rc = gsflin(pcmn);
  else if (cmpwrd(name, "wrdpos"))
    rc = gsfpwd(pcmn);
  else if (cmpwrd(name, "strlen"))
    rc = gsfsln(pcmn);
  else if (cmpwrd(name, "valnum"))
    rc = gsfval(pcmn);
  else if (cmpwrd(name, "read"))
    rc = gsfrd(pcmn);
  else if (cmpwrd(name, "write"))
    rc = gsfwt(pcmn);
  else if (cmpwrd(name, "close"))
    rc = gsfcl(pcmn);
  else if (cmpwrd(name, "sys"))
    rc = gsfsys(pcmn);
  else if (cmpwrd(name, "gsfallow"))
    rc = gsfallw(pcmn);
  else if (cmpwrd(name, "gsfpath"))
    rc = gsfpath(pcmn);
  else if (cmpwrd(name, "math_log"))
    rc = gsfmath(pcmn, 1);
  else if (cmpwrd(name, "math_log10"))
    rc = gsfmath(pcmn, 2);
  else if (cmpwrd(name, "math_cos"))
    rc = gsfmath(pcmn, 3);
  else if (cmpwrd(name, "math_sin"))
    rc = gsfmath(pcmn, 4);
  else if (cmpwrd(name, "math_tan"))
    rc = gsfmath(pcmn, 5);
  else if (cmpwrd(name, "math_atan"))
    rc = gsfmath(pcmn, 6);
  else if (cmpwrd(name, "math_atan2"))
    rc = gsfmath(pcmn, 7);
  else if (cmpwrd(name, "math_sqrt"))
    rc = gsfmath(pcmn, 8);
  else if (cmpwrd(name, "math_abs"))
    rc = gsfmath(pcmn, 9);
  else if (cmpwrd(name, "math_acosh"))
    rc = gsfmath(pcmn, 10);
  else if (cmpwrd(name, "math_asinh"))
    rc = gsfmath(pcmn, 11);
  else if (cmpwrd(name, "math_atanh"))
    rc = gsfmath(pcmn, 12);
  else if (cmpwrd(name, "math_cosh"))
    rc = gsfmath(pcmn, 13);
  else if (cmpwrd(name, "math_sinh"))
    rc = gsfmath(pcmn, 14);
  else if (cmpwrd(name, "math_exp"))
    rc = gsfmath(pcmn, 15);
  else if (cmpwrd(name, "math_fmod"))
    rc = gsfmath(pcmn, 16);
  else if (cmpwrd(name, "math_pow"))
    rc = gsfmath(pcmn, 17);
  else if (cmpwrd(name, "math_sinh"))
    rc = gsfmath(pcmn, 18);
  else if (cmpwrd(name, "math_tanh"))
    rc = gsfmath(pcmn, 19);
  else if (cmpwrd(name, "math_acos"))
    rc = gsfmath(pcmn, 20);
  else if (cmpwrd(name, "math_asin"))
    rc = gsfmath(pcmn, 21);
  else if (cmpwrd(name, "math_format"))
    rc = gsfmath(pcmn, 22);
  else if (cmpwrd(name, "math_nint"))
    rc = gsfmath(pcmn, 23);
  else if (cmpwrd(name, "math_int"))
    rc = gsfmath(pcmn, 24);
  else if (cmpwrd(name, "math_mod"))
    rc = gsfmath(pcmn, 25);
  else if (cmpwrd(name, "math_strlen"))
    rc = gsfmath(pcmn, 26);
...
gaint gsfsys(struct gscmn *pcmn) {
  ...
  while (1) {
    ...
    if (len < incr)
      break;
    if (siz > 10000 && incr < 5000)
      incr = 5000;
    if (siz > 100000 && incr < 10000)
      incr = 10000;
    if (siz > 1000000 && incr < 100000)
      incr = 100000;
    siz += incr;
  }
  *(res + pos) = '\0';
...
gaint gsfwrd(struct gscmn *pcmn) {
  ...
  len = 0;
  while (*(ch + len) != '\0' && *(ch + len) != ' ' && *(ch + len) != '\t' && *(ch + len) != '\n')
    len++;
...
gaint gsfallw(struct gscmn *pcmn) {
  ...
  i = 999;
  if (cmpwrd(pvar->strng, "on"))
    i = 1;
  if (cmpwrd(pvar->strng, "On"))
    i = 1;
  if (cmpwrd(pvar->strng, "ON"))
    i = 1;
  if (cmpwrd(pvar->strng, "off"))
    i = 0;
  if (cmpwrd(pvar->strng, "Off"))
    i = 0;
  if (cmpwrd(pvar->strng, "OFF"))
...
FILE *gsonam(struct gscmn *pcmn, struct gsfdef *pfdf) {
  ...
  if (ifile == NULL) {
    ...
    while (*(uname + len))
      len++;
    if (*(uname + len - 1) != 's' || *(uname + len - 2) != 'g' || *(uname + len - 3) != '.') {
  ...
  if (uname)
    free(uname); /* Hopefully set  */
  if (xname)
    free(xname); /*   to null        */
  if (dname)
    free(dname); /*     if assigned    */
  if (lname)
    free(lname); /*       to oname       */
