...
static struct Func_Table Widgets[] = {{"OpenDisplay", OPENDISPLAY},
...
static struct CB_Table CallBacks[] = {{"NULL", NULL},
                                      {"Exit", CB_Exit},
                                      {"Open", CB_Open},
                                      {"Load", CB_Load},
                                      {"Cmd", CB_Cmd},
...
static struct Macro_Table Macros[] = {
    {"NO_CARE", NO_CARE}, {"PLACE_RIGHT", PLACE_RIGHT}, {"PLACE_UNDER", PLACE_UNDER}, {"FALSE", FALSE}, {"TRUE", TRUE}, {NULL, 0}};
 ...
 *
 */
int GetCallBackIndex(char *name) {
  int i, index = -1;

  for (i = 0;; i++) {
    if (CallBacks[i].name == NULL)
      break;
...
int LoadUserFont(char *name,      /* a short name used by the script */
 ...
 *                LoadUserFont()
 */
int GetFontIndex(char *fontname) /* fontname is a short name
                                    used by the script */
{
  int i, index = -1;
  for (i = 0; i <= ifonts; i++) {
 ...
 *                 LoadUserColor()
 */
int GetColorIndex(char *colorname) /* colorname is a short name
                                     used by the script */
{
  int i, color = -1;
...
int Custom_GUI(char *fname) {
  ...
  do {
    ...
    case MAKEBUTTON:
      ...
      if ((i = SetWidgetIndex(argv[1])) < 0)
        break;
      if ((j = GetCallBackIndex(argv[3])) < 0)
        break;
      strcpy(UserWidgets[i].data, argv[4]);
      UserWidgets[i].w = MakeButton(argv[2], CallBacks[j].func, UserWidgets[i].data);
      break;
    ...
    case MAKEMENUITEM:
      ...
      if ((i = SetWidgetIndex(argv[1])) < 0)
        break;
      if ((j = GetWidgetIndex(argv[2])) < 0)
        break;
      if ((k = GetCallBackIndex(argv[4])) < 0)
        break;
      strcpy(UserWidgets[i].data, argv[5]);
      UserWidgets[i].w = MakeMenuItem(UserWidgets[j].w, argv[3], CallBacks[k].func, UserWidgets[i].data);
      break;
    ...
    case MAKETOGGLE:
      ...
      if ((i = SetWidgetIndex(argv[1])) < 0)
        break;
      if ((m1 = GetMacroValue(argv[3])) < 0)
        break;
      if ((j = GetWidgetIndex(argv[4])) < 0)
        break;
      if ((k = GetCallBackIndex(argv[5])) < 0)
        break;
      strcpy(UserWidgets[i].data, argv[6]);
      UserWidgets[i].w = MakeToggle(argv[2], m1, UserWidgets[j].w, CallBacks[k].func, UserWidgets[i].data);
      break;

    case SETWIDGETPOS:
      if (argc != 5)
        break;
      if ((i = GetWidgetIndex(argv[1])) < 0)
        break;
      if ((m1 = GetMacroValue(argv[2])) < 0)
        break;
      if ((j = GetWidgetIndex(argv[3])) < 0)
        break;
      if ((m2 = GetMacroValue(argv[4])) < 0)
        break;
      if ((k = GetWidgetIndex(argv[5])) < 0)
        break;
      SetWidgetPos(UserWidgets[i].w, m1, UserWidgets[j].w, m2, UserWidgets[k].w);
      break;
    ...
    case MAKEWINDOW:
      ...
      if ((i = SetWidgetIndex(argv[1])) < 0)
        break;
      UserWidgets[i].w = MakeWindow(argv[2], SAME_DISPLAY, NONEXCLUSIVE_WINDOW);
      break;
...
main(int argc, char **argv) { Custom_GUI("sample.gui"); }

/*
 *   fake callbacks for prototyping
 */

void CB_Load(Widget w, void *data) { printf("*** Load callback: %s ***\n", data); }

/*
 * CB_Cmd() - Callback funtion for a generic grads command  button.
 */
void CB_Cmd(Widget w, void *data) { printf("*** Cmd callback: %s ***\n", data); }

/*
 *  CB_CmdLine() - Callback funtion for the command line button.
 */
void CB_CmdLine(Widget w, void *data) { printf("*** CmdLine callback ***\n"); }

/*
 * CB_VarSel() - Callback for selecting a variable.
 *
 */
void CB_VarSel(Widget W, void *data) { printf("*** VarSel callback: %s ***\n", data); }
...
void CB_Display(Widget w, void *data) { printf("*** Display callback: %s ***\n", data); }

/*
 * CB_Toggle() - Callback funtion for a generic toggle button.
 */
void CB_Toggle(Widget w, void *data) { printf("*** Toggle callback: %s ***\n", data); }

/*
 * CB_FileSel() - Callback for selecting an already open file.
 *
 */
void CB_FileSel(Widget W, void *data) { printf("*** FileSel callback: ***\n"); }

/*
 * CB_CmdStr() - Callback funtion for a generic grads command with user input.
 */
void CB_CmdStr(Widget w, void *data) { printf("*** CmdStr callback: %s ***\n", data); }
