...
gaint gxChinit(gadouble xsz, gadouble ysz, gaint xin, gaint yin, gaint bwin, char *fname, gaint fmtflg, char *bgImage,
               gadouble border) {
  gaint status;
...
void gxCflush(gaint opt) {
  ...
  if (maskflag) {
    ...
    if (opt) {
      ...
      cairo_set_source_rgba(cr, (double)(rsav) / 255.0, (double)(gsav) / 255.0, (double)(bsav) / 255.0,
                            (double)(-1 * psav) / 255.0);
  ...
  if (surftype == 1 && opt)
    gxdXflush(); /* force completed graphics to display */
}
...
void gxCcol(gaint clr) {
  ...
  if (bcol == 1) { /* if background is white ...  */
    if (clr == 0)
      clr = 1; /*  ...change color 0 to white (1) */
    else if (clr == 1)
      clr = 0; /*  ...change color 1 to black (0) */
  }
    ...
    if (dbq.tile > -900) {
      ...
      if (dbq.alpha == 255) {
        ...
        if (dbq.alpha < 0) {
          ...
          cairo_set_source_rgba(cr, (double)(dbq.red) / 255.0, (double)(dbq.green) / 255.0, (double)(dbq.blue) / 255.0,
                                (double)(dbq.alpha) / 255.0);
        }
      }
    }
...
void gxCselfont(gaint fn) {
  ...
  if (fn < 6) {
    ...
    if (dbq.fbold == 1)
      cbold = CAIRO_FONT_WEIGHT_BOLD;
    citalic = CAIRO_FONT_SLANT_NORMAL;
    if (dbq.fitalic == 1)
      citalic = CAIRO_FONT_SLANT_ITALIC;
    if (dbq.fitalic == 2)
      citalic = CAIRO_FONT_SLANT_OBLIQUE;
    ...
    dflt = 0;
    if (library == NULL)
      dflt = 1; /* use default fonts */
    if (dbq.fname == NULL)
      dflt = 1; /* make sure we have a font filename */
    ...
    if (dbq.ptype == 1) {
      ...
      if (dbq.pfcol < 0) {
        ...
        if (dbq.red < 0) {
          ...
          if (alph < 0)
            alph = -1 * alph;
          cairo_set_source_rgba(crpatt, (double)(dbq.red) / 255.0, (double)(dbq.green) / 255.0, (double)(dbq.blue) / 255.0,
                                (double)(alph) / 255.0);
        }
      }
    } else {
...
/* close the FreeType library */
void gxCftend(void) { FT_Done_FreeType(library); }

void gxCpush(void) { cairo_push_group(cr); }
...
void gxCcfg(void) { printf("cairo-%d.%d.%d ", CAIRO_VERSION_MAJOR, CAIRO_VERSION_MINOR, CAIRO_VERSION_MICRO); }
