...
void gxdeve(gaint flag) {
  ...
  while (1) {
    if (wflg && !rdrflg) {
      XMaskEvent(display, ButtonReleaseMask | ButtonPressMask | ExposureMask | StructureNotifyMask, &report);
      rc = 1;
    } else {
      rc = XCheckMaskEvent(display, ButtonReleaseMask | ButtonPressMask | ExposureMask | StructureNotifyMask, &report);
    }
    if (!rc && rdrflg) {
      gxdrdw();
      rdrflg = 0;
...
void gxdcol(gaint clr) { /* set color to new value */ gxCcol(clr); }

void gxdwid(gaint wid) { /* Set line width */ gxCwid(wid); }

void gxdmov(gadouble x, gadouble y) { /* Move to x,y   */ gxCmov(x, y); }
...
void gxdssv(int frame) { printf("Warning: The Cairo graphics display interface does not support the screen command\n"); }
void gxdssh(int cnt) { printf("Warning: The Cairo graphics display interface does not support the screen command\n"); }
void gxdsfr(int frame) { printf("Warning: The Cairo graphics display interface does not support the screen command\n"); }
...
gadouble gxdqchl(char ch, gaint fn, gadouble w) { return (gxCqchl(ch, fn, w)); }
...
void gxdXflush(void) { XFlush(display); }
...
void gxsetpatt(gaint pnum) { gxCpattrset(pnum); }

void gxdsignal(gaint sig) {
  if (sig == 1)
    gxCflush(1); /* finish rendering */
  if (sig == 2)
    gxCaa(0); /* disable anti-aliasing */
  if (sig == 3)
    gxCaa(1); /* enable anti-aliasing */
  if (sig == 4)
    gxCpush(); /* push */
  if (sig == 5)
    gxCpop(); /* pop */
...
void gxdclip(gadouble xlo, gadouble xhi, gadouble ylo, gadouble yhi) { gxCclip(xlo, xhi, ylo, yhi); }
