...
void gxdgeo(char *arg) { ugeom = arg; }
...
void gxdbgn(gadouble xsz, gadouble ysz) {
  ...
  win = XCreateSimpleWindow(display, RootWindow(display, snum), ipos, jpos, dw, dh, border, WhitePixel(display, snum),
                            BlackPixel(display, snum));
  gxdbck(0); /* set background to zero, and then update local variable devbck */
  devbck = gxdbkq();

  /* Set up icon pixmap */

  icon_pixmap = XCreateBitmapFromData(display, win, (char *)icon_bitmap_bits, icon_bitmap_width, icon_bitmap_height);
  ...
  XSelectInput(display, win, ButtonReleaseMask | ButtonPressMask | ButtonMotionMask | ExposureMask | StructureNotifyMask);

  /* Get a Graphics Context */

  gc = XCreateGC(display, win, 0L, &values);
  XSetForeground(display, gc, cvals[1]);
  XSetLineAttributes(display, gc, 0L, LineSolid, CapButt, JoinBevel);
  lwidth = 1;
  owidth = 0;
  lcolor = 1;
  ...
  xfnam = gxgsym("GAXFS");
  if (xfnam)
    flist = XListFonts(display, xfnam, 1, &i);
  else
    flist = NULL;
  if (flist == NULL) {
  ...
  if (flist == NULL) {
    ...
    if (font1 == NULL)
      printf("ERROR: Unable to open a basic font!!!\n");
...
void gxdfrm(gaint iact) {
  ...
  if (iact == 7)
    ...
    while (XCheckMaskEvent(
        display, ButtonReleaseMask | ButtonPressMask | ButtonMotionMask | KeyPressMask | ExposureMask | StructureNotifyMask,
        &report)) {
...
void gxdeve(gaint flag) {
  ...
  if (flag && evbase)
    flag = 0; /* Don't wait if an event stacked */
  wflg = flag;
  eflg = 0;
  rdrflg = 0;
  while (1) {
    if (wflg && !rdrflg) {
      XMaskEvent(display,
                 ButtonReleaseMask | ButtonPressMask | ButtonMotionMask | KeyPressMask | ExposureMask | StructureNotifyMask,
                 &report);
      rc = 1;
    } else {
      rc = XCheckMaskEvent(
    ...
    case ButtonPress:
      ...
      while (ii < 512 && obj[ii].type > -1) {
        if (obj[ii].type != 0 && i > obj[ii].i1 && i < obj[ii].i2 && j > obj[ii].j1 && j < obj[ii].j2) {
...
gaint gxbcol(XStandardColormap *best, XColor *cell) {
  ...
  min = 65536;
  for (i = 0; i < 256; i++) {
    d = abs(cell->red - colors[i].red) + abs(cell->green - colors[i].green) + abs(cell->blue - colors[i].blue);
    if (d < min) {
      min = d;
      bestpixel = i;
    }
...
void gxdrmu(gaint mnum, struct gdmu *pmu, gaint redraw, gaint nstat) {
  ...
  if (gmu->oc1 > -1) {
    ...
    XDrawLine(display, drwbl, gc, ilo, jhi, ihi, jhi);
    if (lw > 1)
      XDrawLine(display, drwbl, gc, ilo + 1, jhi - 1, ihi - 1, jhi - 1);
    if (lw > 2)
      XDrawLine(display, drwbl, gc, ilo + 2, jhi - 2, ihi - 2, jhi - 2);
    XDrawLine(display, drwbl, gc, ihi, jhi, ihi, jlo);
    if (lw > 1)
      XDrawLine(display, drwbl, gc, ihi - 1, jhi - 1, ihi - 1, jlo + 1);
    if (lw > 2)
      XDrawLine(display, drwbl, gc, ihi - 2, jhi - 2, ihi - 2, jlo + 2);
  }
  if (gmu->oc2 > -1) {
    gxdcol(gmu->oc2);
    XDrawLine(display, drwbl, gc, ihi, jlo, ilo, jlo);
    if (lw > 1)
      XDrawLine(display, drwbl, gc, ihi - 1, jlo + 1, ilo + 1, jlo + 1);
    if (lw > 2)
      XDrawLine(display, drwbl, gc, ihi - 2, jlo + 2, ilo + 2, jlo + 2);
    XDrawLine(display, drwbl, gc, ilo, jlo, ilo, jhi);
    if (lw > 1)
      XDrawLine(display, drwbl, gc, ilo + 1, jlo + 1, ilo + 1, jhi - 1);
    if (lw > 2)
      XDrawLine(display, drwbl, gc, ilo + 2, jlo + 2, ilo + 2, jhi - 2);
  }
  if (gmu->fc > -1) {
    len = 0;
    while (*(gmu->ch + len))
...
void gxrdrw(int flag) {
  ...
  if (flag) {
    ...
    for (i = 0; i < 32; i++) {
      if (rbb2[i].num > -1)
        gxdrbb(i, rbb2[i].type, rbb2[i].xlo, rbb2[i].ylo, rbb2[i].xhi, rbb2[i].yhi, rbb2[i].mb);
    }
    for (i = 0; i < 200; i++) {
      if (dmu2[i].num > -1)
        gxdrmu(i, NULL, 1, -1);
    ...
    for (i = 0; i < 32; i++) {
      if (rbb[i].num > -1)
        gxdrbb(i, rbb[i].type, rbb[i].xlo, rbb[i].ylo, rbb[i].xhi, rbb[i].yhi, rbb[i].mb);
    }
    for (i = 0; i < 200; i++) {
      if (dmu[i].num > -1)
        gxdrmu(i, NULL, 1, -1);
...
void gxrswd(gaint flag) {
  ...
  if (flag != 2) {
    for (i = 0; i < 256; i++) {
      if (btn[i].num > -1 && btn[i].ch != NULL)
        gree(btn[i].ch, "f501");
      btn[i].num = -1;
      btn[i].ch = NULL;
    }
    for (i = 0; i < 200; i++) {
      if (dmu[i].num > -1 && dmu[i].ch != NULL)
        gree(dmu[i].ch, "f502");
      dmu[i].num = -1;
      dmu[i].ch = NULL;
    }
  ...
  if (flag) {
    for (i = 0; i < 256; i++) {
      if (flag != 2) {
        if (btn2[i].num > -1 && btn2[i].ch != NULL)
          gree(btn2[i].ch, "f503");
      }
      btn2[i].num = -1;
      btn2[i].ch = NULL;
    ...
    for (i = 0; i < 200; i++) {
      if (flag != 2) {
        if (dmu2[i].num > -1 && dmu2[i].ch != NULL)
          gree(dmu2[i].ch, "f504");
      }
      dmu2[i].num = -1;
      dmu2[i].ch = NULL;
...
void gxrs1wd(int wdtyp, int wdnum) {
  ...
  if (wdtyp < 1 || wdtyp > 3)
    return;
  if (wdtyp == 1 && (wdnum < 0 || wdnum > 255))
    return;
  if (wdtyp == 2 && (wdnum < 0 || wdnum > 31))
    return;
  if (wdtyp == 3 && (wdnum < 0 || wdnum > 199))
    return;
  ...
  if (wdtyp == 1) {
    ...
    if (btn[wdnum].num > -1 && btn[wdnum].ch != NULL)
      gree(btn[wdnum].ch, "f505");
    btn[wdnum].num = -1;
    btn[wdnum].ch = NULL;
  } else if (wdtyp == 2) {
    ...
    if (dmu[wdnum].num > -1 && dmu[wdnum].ch != NULL)
      gree(dmu[wdnum].ch, "f506");
    dmu[wdnum].num = -1;
    dmu[wdnum].ch = NULL;
  }
}
...
void gxevbn(struct gevent *geve, int iobj) {
  ...
  j1 = btn[jj].jlo;
  j2 = btn[jj].jhi;
  if (!(btn[jj].state && btn[jj].oc1 == btn[jj].otc2 && btn[jj].oc2 == btn[jj].otc1)) {
    if (c2 > -1 && c1 > -1) {
      gxdwid(btn[jj].thk);
      gxdcol(c2);
    ...
    if (report.type == MotionNotify) {
      if (xoflg) {
        if (typ == 1)
          ...
          XDrawLine(display, win, gc, i1o, j1o, i2o, j2o);
      }
      if (i < report.xmotion.x) {
        i1 = i;
        i2 = report.xmotion.x;
      } else {
        i2 = i;
      ...
      if (j2 > jhi)
        j2 = jhi;
      if (typ == 1)
        XDrawRectangle(display, win, gc, i1, j1, i2 - i1, j2 - j1);
      else
        XDrawLine(display, win, gc, i1, j1, i2, j2);
      i1o = i1;
      i2o = i2;
      j1o = j1;
      j2o = j2;
...
int gxpopdm(struct gdmu *gmu, int iobj, int porig, int iorig, int jorig) {
  ...
  while (i < gmu->len) {
    ...
    eflag = 1;
    while (*(ch + len)) {
      if (i + len + 4 < gmu->len && *(ch + len) == '>' && (*(ch + len + 1) >= '0' && *(ch + len + 1) <= '9') &&
          (*(ch + len + 2) >= '0' && *(ch + len + 2) <= '9') && *(ch + len + 3) == '>') {
        ich[0] = *(ch + len + 1);
        ich[1] = *(ch + len + 2);
        ich[2] = '\0';
        ptrs[cnt] = atoi(ich);
      ...
      if (i + len + 5 < gmu->len && *(ch + len) == '>' && (*(ch + len + 1) >= '0' && *(ch + len + 1) <= '1') &&
          (*(ch + len + 2) >= '0' && *(ch + len + 2) <= '9') && (*(ch + len + 3) >= '0' && *(ch + len + 3) <= '9') &&
          *(ch + len + 4) == '>') {
        ich[0] = *(ch + len + 1);
        ich[1] = *(ch + len + 2);
        ich[2] = *(ch + len + 3);
  ...
  if (cascf) {
    ...
    ich[3] = 'x';
    if (gfont == 1 && font1i)
      w = XTextWidth(font1i, ich, 3);
    if (gfont == 2 && font2i)
      w = XTextWidth(font2i, ich, 3);
    if (gfont == 3 && font3i)
      w = XTextWidth(font3i, ich, 3);
    maxw = maxw + w;
  }
  h = h * 2;
  jsiz = h * cnt;
  ...
  pop = XCreateWindow(display, win, imin, jmin, isiz, jsiz, 0, CopyFromParent, CopyFromParent, CopyFromParent, CWSaveUnder, &xsetw);
  XSelectInput(display, pop, ButtonReleaseMask | ButtonPressMask | ButtonMotionMask | ExposureMask | StructureNotifyMask);
  ...
  if (gmu->thk > 11)
    lw = 3;
  XSetLineAttributes(display, gcp, lw, LineSolid, CapButt, JoinBevel);
  ...
  if (gfont == 1 && font1i)
    XSetFont(display, gcp, font1i->fid);
  if (gfont == 2 && font2i)
    XSetFont(display, gcp, font2i->fid);
  if (gfont == 3 && font3i)
    XSetFont(display, gcp, font3i->fid);
  cnt = 0;
  ch = gmu->ch;
  len = 0;
  i = 0;
  ...
  XTranslateCoordinates(display, win, RootWindow(display, snum), 0, 0, &absx, &absy, &dummy);
  ...
  while (1) {
    ...
    if (report.type == ButtonRelease && item > -1)
      break;
    if (report.type == MotionNotify) {
      i = report.xmotion.x;
      j = report.xmotion.y;
      ...
      if (i > imax || i < imin || j > jmax || j < jmin) {
        ...
        if (dmrecu > 0) {
          for (itt = 0; itt < dmrecu; itt++) {
            if (i >= dmi1[itt] && i <= dmi2[itt] && j >= dmj1[itt] && j <= dmj2[itt]) {
              itt2 = (j - dmj1[itt]) / h;
              if (itt2 != dmcur[itt]) {
                dmflag = -99;
                itt = 999;
      ...
      if (item == -1) {
        ii = 0;
        while (ii < 512 && obj[ii].type > -1) {
          if (ii != iobj && obj[ii].type == 3 && i > obj[ii].i1 && i < obj[ii].i2 && j > obj[ii].j1 && j < obj[ii].j2) {
            dmflag = ii;
            ii = 100000; /* Exit loop */
          }
          ii++;
  ...
  if (ptrs[itemold] > -1 && dmsel[dmrecu + 1] < 0)
    dmsel[dmrecu] = -1;
...
char *gxdlg(struct gdlg *qry) {
  ...
  *(tch + tlen) = '\0';
  ...
  if (qry->x == -1)
    ...
    imin = qry->x * xscl + 0.5 - isiz / 2;

  if (qry->y == -1)
    jmin = (height - jsiz) / 2;
  else
    jmin = height - (qry->y * yscl + 0.5 + jsiz / 2);
  pimin = imin;
  pjmin = jmin;
  pisiz = isiz;
  pjsiz = jsiz;
  ...
  pop = XCreateWindow(display, win, pimin, pjmin, pisiz, pjsiz, 0, CopyFromParent, CopyFromParent, CopyFromParent, CWSaveUnder,
                      &xsetw);
  ...
  XSetLineAttributes(display, gcp, 0L, LineSolid, CapButt, JoinBevel);
  XMapWindow(display, pop);
  flag = 1;
  while (flag) {
    XMaskEvent(display, ExposureMask | StructureNotifyMask, &report);
  ...
  if (qry->th > 5)
    XDrawRectangle(display, pop, gcp, 1, 1, pisiz - 3, pjsiz - 3);
  if (qry->fc > -1)
    cn = qry->fc;
  else
    cn = 0;
  ...
  if (dflag) {
    ...
    if (plen > 0)
      XDrawString(display, pop, gcp, i, j, pch, plen);
    if (qry->pc > -1)
      cn = qry->pc;
    else
      cn = 1;
    XSetForeground(display, gcp, cvals[cn]);
    if (plen > 0)
      XDrawString(display, pop, gcp, i - 1, j - 1, pch, plen);
    if (gfont == 1 && font1)
      XSetFont(display, gcp, font1->fid);
    if (gfont == 2 && font2)
      XSetFont(display, gcp, font2->fid);
    if (gfont == 3 && font3)
      XSetFont(display, gcp, font3->fid);
    if (dflag == 1) {
      j = h * 4;
      j1 = h * 2 + 1;
      j2 = jsiz - j1 - 1;
    ...
    if (gfont == 3 && font3)
      XSetFont(display, gcp, font3->fid);
    if (tlen > 0)
      XDrawString(display, pop, gcp, i, j, tch, tlen);
    strcpy(rch, tch);
    rlen = tlen;
  } else {
    if (gfont == 1 && font1)
  ...
  lastBtnDown = 0;
  while (1) {
    XMaskEvent(display, ButtonReleaseMask | ButtonPressMask | ButtonMotionMask | KeyPressMask | ExposureMask | StructureNotifyMask,
               &report);
    ...
    if (report.type == ButtonPress && report.xbutton.time < lastBtnDown + 300) {
      ...
      if (2 * i1 > isiz - w && 2 * i1 < isiz + w)
        c1 = (i1 - (isiz - w) / 2 + 4) * rlen / w;
      if (2 * i1 <= isiz - w)
        c1 = 0;
      if (2 * i1 >= isiz + w)
        c1 = rlen;
      ...
      if (2 * i2 > isiz - w && 2 * i2 < isiz + w)
        c2 = (i2 - (isiz - w) / 2 + 8) * rlen / w;
      if (2 * i2 <= isiz - w)
        c2 = c0;
      if (2 * i2 >= isiz + w)
        c2 = rlen;
    ...
    if (report.type == KeyPress) {
      ...
      if (cnt > 0) {
        if (keysym == XK_Return || keysym == XK_Linefeed) {
          if (qry->nu == 1) {
            ...
            if (n1 == 1 && p1 == 1)
              ii = 1; /* require digit before decimal  */
            if (n1 == 1 && p1 == 2 && p2 == 1 && n2 == 1)
              ii = 1;
            if (n1 == 1 && p1 == 2 && p3 == 1 && n3 == 1)
              ii = 1;
            if (n1 == 1 && p1 == 2 && p2 == p4 + 1 && n2 == 2)
              ii = 1;
            if (n1 == 1 && p1 == 2 && p3 == p4 + 1 && n3 == 2)
              ii = 1;
            if (n1 == 1 && p1 == 2 && p2 == p5 + 1 && n2 == 2)
              ii = 1;
            if (n1 == 1 && p1 == 2 && p3 == p5 + 1 && n3 == 2)
              ii = 1;
            if (n1 > 1)
              ii = 2; /* too many decimal points */
            if (n2 >= 1 && p2 > p4 + 1 && n4 == 1)
              ii = 3; /* misplaced + sign in exponent */
            if (n2 >= 1 && p2 > p5 + 1 && n5 == 1)
              ii = 3;
            if (n3 >= 1 && p3 > p4 + 1 && n4 == 1)
              ii = 4; /* misplaced - sign in exponent */
            if (n3 >= 1 && p3 > p5 + 1 && n5 == 1)
              ii = 4;
            if (n2 >= 1 && p2 > 1 && n4 + n5 == 0)
              ii = 5; /* misplaced + sign in number */
            if (n2 >= 1 && p2 > 1 && p2 < p4 && n4 == 1)
              ii = 5;
            if (n2 >= 1 && p2 > 1 && p2 < p5 && n5 == 1)
              ii = 5;
            if (n3 >= 1 && p3 > 1 && n4 + n5 == 0)
              ii = 6; /* misplaced - sign in number */
            if (n3 >= 1 && p3 > 1 && p3 < p4 && n4 == 1)
              ii = 6;
            if (n3 >= 1 && p3 > 1 && p3 < p5 && n5 == 1)
              ii = 6;
            if (n1 == 1 && p1 > p4 && n4 == 1)
              ii = 7; /* decimal in exponent */
            if (n1 == 1 && p1 > p5 && n5 == 1)
              ii = 7;
            if (n4 > 0 && p4 == tlen)
              ii = 8; /* missing exponent value */
            if (n5 > 0 && p5 == tlen)
              ii = 8;
            if (n4 > 0 && p4 == tlen - 1 && p2 == tlen)
              ii = 8;
            if (n5 > 0 && p5 == tlen - 1 && p2 == tlen)
              ii = 8;
            if (n4 > 0 && p4 == tlen - 1 && p3 == tlen)
              ii = 8;
            if (n5 > 0 && p5 == tlen - 1 && p3 == tlen)
              ii = 8;
            if (n7 == 0)
              ii = 9; /* missing number value */
            if (n4 > 0 && p4 == 1)
              ii = 9;
            if (n5 > 0 && p5 == 1)
              ii = 9;
            ...
            if (n5 > 0 && p5 == 2 && p3 == 1)
              ii = 9;
            if (n2 > 2)
              ii = 10; /* too many '+' signs */
            if (n3 > 2)
              ii = 11; /* too many '-' signs */
            if (n4 + n5 > 1)
              ii = 12; /* too many exponent symbols */
            if (n6 > 0)
              ii = 13; /* # sign still present, number not entered */
            if (n0 > 0)
              ii = 14;    /* number includes an alpha char */
      ...
      if ((keysym == XK_KP_Left || keysym == XK_Left) && c1 > c0) {
        if (c1 == c2)
          c2--;
        c1--;
      }
      if ((keysym == XK_KP_Right || keysym == XK_Right) && c1 < rlen) {
        if (c1 == c2)
          c2++;
        c1++;
      }
      if ((keysym == XK_KP_Down || keysym == XK_Down) && c2 > c1)
        c2--;
      if ((keysym == XK_KP_Up || keysym == XK_Up) && c2 < rlen)
        c2++;
    }
    if (rlen >= 0 && c1 >= c2) {
      if (qry->bc > -1)
      ...
      XSetForeground(display, gcp, cvals[cn]);
      if (gfont == 1 && font1)
        w = XTextWidth(font1, rch, rlen);
      if (gfont == 2 && font2)
        w = XTextWidth(font2, rch, rlen);
      if (gfont == 3 && font3)
        w = XTextWidth(font3, rch, rlen);
      if (w + h < isiz)
        i = (isiz - w) / 2;
      else
        i = isiz - w - h / 2;
      if (pflag && !dflag && w + h < isiz)
        i = h / 2;
      if (c1 > 0)
        strncpy(tch, rch, (size_t)c1);
      *(tch + c1) = '\0';
      if (gfont == 1 && font1)
        w1 = XTextWidth(font1, tch, c1);
      if (gfont == 2 && font2)
        w1 = XTextWidth(font2, tch, c1);
      if (gfont == 3 && font3)
        w1 = XTextWidth(font3, tch, c1);
      ii = i + w1;
      if (ii <= 0) {
        i = -w1;
        XDrawString(display, pop, gcp, i, j, rch, rlen);
    ...
    if (c1 < c2) {
      ...
      *(tch + c1) = '\0';
      ...
      for (ii = 0; ii < c2 - c1; ii++)
        *(tch + ii) = *(rch + ii + c1);
      *(tch + c2 - c1) = '\0';
      if (gfont == 1 && font1)
        w2 = XTextWidth(font1, tch, c2 - c1);
      if (gfont == 2 && font2)
        w2 = XTextWidth(font2, tch, c2 - c1);
      if (gfont == 3 && font3)
        w2 = XTextWidth(font3, tch, c2 - c1);
      i += w1;
      if (qry->fc > -1)
        cn = qry->fc;
      else
      ...
      *(tch + rlen - c2) = '\0';
      ...
      if (qry->th > 5)
        XDrawRectangle(display, pop, gcp, 1, 1, isiz - 3, jsiz - 3);
    }
    for (i = 0; i < 512; i++)
      *(tch + i) = '\0';
...
void gxdssh(int cnt) {
  ...
  if (cnt < 0 || cnt > 199)
    return;
  if (pfilld[cnt])
    XCopyArea(display, pmaps[cnt], drwbl, gc, 0, 0, width, height, 0, 0);
}
...
gaint win_data(struct xinfo *xinf) {
  ...
  if (display == (Display *)NULL || win == (Window)NULL)
    return 0;
  if (!XGetWindowAttributes(display, win, &result))
    return 0;
  if (!XTranslateCoordinates(display, win, RootWindow(display, snum), 0, 0, &absx, &absy, &dummy))
    return 0;
  xinf->winid = (int)win;
...
gadouble gxdch(char ch, gaint fn, gadouble x, gadouble y, gadouble w, gadouble h, gadouble rot) { return (-999); }
gadouble gxdqchl(char ch, gaint fn, gadouble w) { return (-999); }
...
void gxdcfg(void) { printf("X%d.%d ", X_PROTOCOL, X_PROTOCOL_REVISION); }
/* Check to see if this display backend supports fonts */
gaint gxdckfont(void) { return (0); }
