...
void gxclev(gadouble *r, gaint is, gaint js, gaint ib, gaint ie, gaint jb, gaint je, gadouble v, char *u, struct gxcntr *pcntr) {
  ...
  iw = is + 2;
  jw = js + 2; /* Dimensions of flag grid    */
  iww = iw;
  jww = jw;
  iss = is; /* Set shared values          */
  vv = v;
  ...
  if (lwksiz < iw * jw * 2) { /* If storage inadaquate then */
    ...
    if (fwksiz < 500)
      fwksiz = 500; /* Insure big enough for small grids */
  ...
  p2 = p1 + 1;
  p4 = p1 + is;                     /*   at start of last row           */
  p1u = u + is * (je - 1) + ib - 1; /* Set undef pntrs to corner values */
  p2u = p1u + 1;
  p4u = p1u + is;             /*   at start of last row           */
...
void gxcflw(gaint i, gaint j, gaint iside, gaint dr) {
  ...
  if (iside == 1)
    goto side1; /* Jump in based on side      */
  if (iside == 2)
    goto side2;
  if (iside == 3)
    goto side3;
...
side1:                                /* Exit side 1; Enter side 3  */
  ...
  *l1 = 0;  /* Indicate we were here      */
  ...
  if ((xy < xyllim) || (xy > xyulim))
    goto done; /* Don't exceed buffer    */
  l3 = l1;
  l1 -= iww;
  l4 -= iww;
  ...
  if (*l1 && *l2 && *l4) { /* Handle col point           */
    if (pathln(*p1, *p2, *p3, *p4))
      goto side4;
    else
      goto side2;
  }
  if (*l4)
    goto side4; /* Find exit point            */
  if (*l1)
    goto side1;
  if (*l2)
    goto side2;
...
gaint gxcspl(gaint frombuf, struct gxcntr *pcntr) {
  ...
  if (pcntr->ltype == 2)
    ...
    dacum = 1.0;   /* Accumulated length between labels */
  ...
  if ((*xystrt == *xyend) && (*(xystrt + 1) == *(xyend + 1)))
    icls = 1;
    ...
    if (pcntr->ltype && clabel[0]) {
     ...
     and if not too close to another label */
      ...
      if (icls) {
        ...
        if (dequal(*(xyend + 1), *(xystrt + 3), 1e-12) == 0 || dequal(*(xyend - 1), *(xyend + 1), 1e-12) == 0)
          c1 = 0.0;
        else
  ...
  if (pcntr->spline == 0) {
    ...
    for (xy = xystrt + 2; xy < xyend - 1; xy += 2) {
      ...
      if (!frombuf) {
        dacum += hypot(*xy - *(xy - 2), *(xy + 1) - *(xy - 1));
        if ((*(xy + 1) - *(xy - 1)) * (*(xy + 3) - *(xy + 1)) < 0.0 && gxlabn < LABMAX && dacum > dcmin) {
          if (clabel[0]) {
            gxlabx[gxlabn] = (*xy);
            gxlaby[gxlabn] = (*(xy + 1));
            gxlabs[gxlabn] = 0.0;
    ...
    if (!frombuf) {
      dacum += hypot(*xyend - *(xyend - 2), *(xyend + 1) - *(xyend - 1));
      if (icls) {
        if ((*(xyend + 1) - *(xystrt + 3)) * (*(xyend - 1) - *(xyend + 1)) < 0.0 && gxlabn < LABMAX && dacum > dcmin) {
          if (clabel[0]) {
            gxlabx[gxlabn] = (*xyend);
  ...
  do { /* Loop to find prior point   */
    ...
    if (x2 < xystrt)
      goto exit;                          /* No valid line to draw      */
  ...
  do { /* Loop to find next point    */
    ...
    if (x1 > xyend)
      goto exit;                          /* Exit if no valid line      */
  ...
  for (i = 0; i < gxlabn; i++) {
    ...
    if (gxlabs[i] == 0.0) {               /* contour label is not rotated */
      ...
      if (colflg > -1)
        ...
        fcol = gxlabc[i];
      if (fcol == bcol)
        gxcolr(1); /* if label color is same as background, use foreground */
      else
        gxcolr(fcol);
...
void gxpclin(void) {
  ...
  while (pclbuf) {
    if (pclbuf->lxy) {
      ...
      for (i = 0; i < 2 * pclbuf->len; i++)
        *(xystrt + i) = *(pclbuf->lxy + i);
      gxcolr(pclbuf->color);
      gxstyl(pclbuf->style);
      gxwide(pclbuf->width);
