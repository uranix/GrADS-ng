...
void houtch(char ch, gaint fn, gadouble x, gadouble y, gadouble w, gadouble h, gadouble ang) {
  gaint rc;
  signed char *ccc;
  char *ucc;
  if (mbuferror)
...
void gxhfrm(gaint iact) {
  ...
  if (!dbmode)
    mbuferror = 0; /* Reset error state on clear command */
}
