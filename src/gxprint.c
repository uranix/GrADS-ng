...
void gxpcfg(void) { gxCcfg(); }

/* Check to see if this printing backend supports fonts */
gaint gxpckfont(void) { return (1); }
...
void gxpinit(gadouble xsz, gadouble ysz) { gxCbatch(xsz, ysz); }

/* Tell Cairo to destroy the surface for checking string lengths. */

void gxpend(void) { gxCend(); }
...
gaint gxprint(char *fnout, gaint xin, gaint yin, gaint bwin, gaint fmtflg, char *bgImage, char *fgImage, gaint tcolor,
              gadouble border) {

  /* Make sure we don't try to print an unsupported format */
  if (fmtflg != 1 && fmtflg != 2 && fmtflg != 3 && fmtflg != 4 && fmtflg != 5)
    return (9);

  if (tcolor != -1)
    gxdbsettransclr(tcolor); /* tell graphics database about transparent color override */
...
void gxpcol(gaint col) { /* new color */ gxCcol(col); }
void gxpacol(gaint col) { /* new color definition */ /* this is a no-op for cairo */ }
void gxpwid(gaint wid) { /* new line thickness */ gxCwid(wid); }
void gxprec(gadouble x1, gadouble x2, gadouble y1, gadouble y2) { /* filled rectangle */ gxCrec(x1, x2, y1, y2); }
void gxpcirc(gadouble x, gadouble y, gadouble r, gaint flg) { /* open/filled circle */ gxCcirc(x, y, r, flg); }
void gxpbpoly(void) { /* start a polygon fill */ gxCbfil(); }
gaint gxpepoly(gadouble *xybuf, gaint xyc) { /* terminate a polygon fill */
  gxCfil(xybuf, xyc);
  return (0);
}
void gxpmov(gadouble xpos, gadouble ypos) { /* move to */ gxCmov(xpos, ypos); }
void gxpdrw(gadouble xpos, gadouble ypos) { /* draw to */ gxCdrw(xpos, ypos); }
void gxpflush(void) { /* finish drawing */ gxCflush(1); }
...
gadouble gxpqchl(char ch, gaint fn, gadouble w) { /* query character length */ return (gxCqchl(ch, fn, w)); }
void gxpclip(gadouble x1, gadouble x2, gadouble y1, gadouble y2) { /* set clipping area */ gxCclip(x1, x2, y1, y2); }
