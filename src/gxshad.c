  ...
  for (jmn = 1; jmn < jmax; jmn += 3) {
    ...
    for (k = 0; k < lvs; k++) {
      ...
      for (j = jmn; j <= jmx; j++) {
        ...
        for (i = imn; i <= imx; i++) {
          if (i < imx && (*f1 == 0 || *f1 == 8) && ((*p1 <= val && *p2 > val) || (*p1 > val && *p2 <= val))) {
            if (j == jmx)
              rc = gxsflw(i, j - 1, 3);
            else
              rc = gxsflw(i, j, 1);
          ...
          if (j < jmx && (*f4 == 0 || *f4 == 8) && ((*p1 <= val && *p4 > val) || (*p1 > val && *p4 <= val))) {
            if (i == imx)
              rc = gxsflw(i - 1, j, 2);
            else
              rc = gxsflw(i, j, 4);
      ...
      for (j = jmn; j <= jmx; j++) {
        ...
        for (i = imn; i <= imx; i++) {
          rc = 0;
          if (i < imx) {
            if (j == jmn && *f1 == 0 && *p1 > val && *p2 > val)
              rc = gxsflw(i, j, 5);
            if (j == jmx && *f1 == 0 && *p1 > val && *p2 > val)
              rc = gxsflw(i, j, 6);
            if (*f1 == 8 && *p1 > val && *p2 > val)
              rc = gxsflw(i + 1, j, 9);
          }
          if (j < jmx) {
            if (i == imn && *f4 == 0 && *p1 > val && *p4 > val)
              rc = gxsflw(i, j, 7);
            if (i == imx && *f4 == 0 && *p1 > val && *p4 > val)
              rc = gxsflw(i, j, 8);
            if (*f4 == 8 && *p1 > val && *p4 > val)
              rc = gxsflw(i, j + 1, 10);
          }
          if (rc)
            goto err;
...
void shdcmp(void) {
  ...
  i = 0;
  for (j = 1; j < xycnt; j++) {
    if (*(xybuf + (i * 2)) != *(xybuf + (j * 2)) || *(xybuf + (i * 2 + 1)) != *(xybuf + (j * 2 + 1))) {
      i++;
      if (i != j) {
        *(xybuf + (i * 2)) = *(xybuf + (j * 2));
