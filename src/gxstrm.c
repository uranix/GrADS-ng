...
void gxstrm(gadouble *u, gadouble *v, gadouble *c, gaint is, gaint js, char *umask, char *vmask, char *cmask, gaint flag,
            gadouble *shdlvs, gaint *shdcls, gaint shdcnt, gaint den, gadouble strmarrd, gadouble strmarrsz, gaint strmarrt) {
  gadouble x, y, xx, yy;
  ...
  if (den < 0) { /* Support very high resolution grids */
    ...
    if (ii > 10)
      ii = 10;
    if ((is < 200 || js < 100) && ii > 2)
      ii = 2; /* Limit downscaling to only high res */
    if ((is < 500 || js < 250) && ii > 5)
      ii = 5;
    if ((is < 1000 || js < 500) && ii > 10)
      ii = 10;
    if ((is < 1500 || js < 750) && ii > 15)
      ii = 15;
    if (ii > 20)
      ii = 20;
  }
  ...
  for (i = 0; i < siz; i++) {
    ...
    if (iacc == 0) {
      ...
      while (x >= 0.0 && x < (gadouble)(is - 1) && y >= 0.0 && y < (gadouble)(js - 1)) {
        ...
        if (*upmask == 0 || *(upmask + 1) == 0 || *(upmask + is) == 0 || *(upmask + is + 1) == 0)
          break;
        if (*vpmask == 0 || *(vpmask + 1) == 0 || *(vpmask + is) == 0 || *(vpmask + is + 1) == 0)
          break;
        if (flag) {
          cp = c + jj * is + ii;
          cpmask = cmask + jj * is + ii;
          if (*cpmask == 0 || *(cpmask + 1) == 0 || *(cpmask + is) == 0 || *(cpmask + is + 1) == 0)
            icol = 15;
          else {
            cv1 = *cp + (*(cp + 1) - *cp) * xx;
        ...
        if (dacum > strmarrd) {
          if (icol > -1)
            strmar(xxsv, yysv, xx, yy, strmarrsz, strmarrt);
          acnt = 0;
          dacum = 0.0;
        }
        xold = xx;
      ...
      while (x >= 0.0 && x < (gadouble)(is - 1) && y >= 0.0 && y < (gadouble)(js - 1)) {
        ...
        if (dacum > strmarrd) {
          if (icol > -1)
            strmar(xx, yy, xxsv, yysv, strmarrsz, strmarrt);
          acnt = 0;
          dacum = 0.0;
        }
        xxsv = xx;
...
void strmar(gadouble xx1, gadouble yy1, gadouble xx2, gadouble yy2, gadouble sz, gaint type) {
  ...
  xy[0] = xx2;
  xy[1] = yy2;
  xy[2] = xx2 + sz * cos(dir + a150);
  xy[3] = yy2 + sz * sin(dir + a150);
  xy[4] = xx2 + sz * cos(dir - a150);
  xy[5] = yy2 + sz * sin(dir - a150);
  xy[6] = xx2;
  xy[7] = yy2;
  if (type == 1) {
    gxplot(xx2, yy2, 3);
