...
gaint gxstrt(gadouble xmx, gadouble ymx, gaint batch, gaint hbufsz, char *gxdopt, char *gxpopt, char *xgeom) {
  ...
  if (intflg) {
    if (xgeom[0] != '\0')
      dsubs.gxdgeo(xgeom);  /* tell display software about geometry override */
...
void gxsignal(gaint sig) {
  if (intflg)
    dsubs.gxdsignal(sig); /* tell the rendering layer about the signal */
...
gadouble gxqchl(char ch, gaint fn, gadouble w) {
  ...
  if (fn == 3)
    return (-999.9); /* symbol font */
  if (fn > 5 && fn < 10)
    return (-999.9); /* user-defined font file */
  if (fn < 6 && !gxdbqhersh())
...
gaint gxacol(gaint clr, gaint red, gaint green, gaint blue, gaint alpha) {
  ...
  if (intflg)
    rc = dsubs.gxdacol(clr, red, green, blue, alpha); /* tell hardware */
  return (rc);
}
...
void gxdraw(gadouble x, gadouble y) { /* Draw to x,y   */
  ...
  if (cflag) {
    ...
    hout2(-10, xnew, ynew);
    if (intflg)
      dsubs.gxdmov(xnew, ynew);
  }
  oldx = x;
  oldy = y;
  gxvcon(x, y, &x, &y);
  if (maskflg > 0)
    pos = ((gaint)(y * 100.0)) * maskx + (gaint)(x * 100.0);
...
void gxclip(gadouble xmin, gadouble xmax, gadouble ymin, gadouble ymax) {
  ...
  if (intflg)
    dsubs.gxdclip(clxmin, clxmax, clymin, clymax);
...
void gxscal(gadouble xmin, gadouble xmax, gadouble ymin, gadouble ymax, gadouble smin, gadouble smax, gadouble tmin,
            gadouble tmax) {
...
void gxvpag(gadouble xmax, gadouble ymax, gadouble smin, gadouble smax, gadouble tmin, gadouble tmax) {
  gadouble xmin, ymin;
  ...
  if ((smax - smin) / rxsize < 0.49 || (tmax - tmin) / rysize < 0.49)
    lwflg = 1;
  else
    lwflg = 0;
...
void gxppvp2(gadouble x, gadouble *s) { /* character width, virtual->real */ *s = (x) / vxm; }
...
void gxproj(void (*fproj)(gadouble s, gadouble t, gadouble *x, gadouble *y)) { fconv = fproj; }
...
void gxgrid(void (*fproj)(gadouble s, gadouble t, gadouble *x, gadouble *y)) { gconv = fproj; }
...
void gxback(void (*fproj)(gadouble s, gadouble t, gadouble *x, gadouble *y)) { bconv = fproj; }
...
void gxrecf(gadouble xlo, gadouble xhi, gadouble ylo, gadouble yhi) {
  ...
  if (xhi <= clminx || xlo >= clmaxx || yhi <= clminy || ylo >= clmaxy)
    return;
  if (xlo < clminx)
    xlo = clminx;
  if (xhi > clmaxx)
...
void gxptrn(gaint typ, gaint den, gaint ang) {
  ...
  if (intflg)
    dsubs.gxdptn(typ, den, ang);
}

/* query line width */

gaint gxqwid(void) { return (lwide); }

/* query color */

gaint gxqclr(void) { return (lcolor); }

/* query style */

gaint gxqstl(void) { return (lstyle); }
...
void gxtitl(char *chrs, gadouble x, gadouble y, gadouble height, gadouble width, gadouble angle) {
  gadouble xx, yy;
  gaint len, i;
...
void bdterp(gadouble x1, gadouble y1, gadouble x2, gadouble y2, gadouble *x, gadouble *y) {

  if (x1 < clminx || x2 < clminx || x1 > clmaxx || x2 > clmaxx) {
    *x = clminx;
    if (x1 > clmaxx || x2 > clmaxx)
      *x = clmaxx;
    *y = y1 - ((y1 - y2) * (x1 - *x) / (x1 - x2));
    if (*y < clminy || *y > clmaxy)
      goto sideh;
    return;
  }
...
void gxmaskrec(gadouble xlo, gadouble xhi, gadouble ylo, gadouble yhi) {
  ...
  if (xlo < clminx && xhi < clminx)
    return;
  if (xlo > clmaxx && xhi > clmaxx)
    return;
  if (ylo < clminy && yhi < clminy)
    return;
  if (ylo > clmaxy && yhi > clmaxy)
    return;
...
gaint gxmaskrq(gadouble xlo, gadouble xhi, gadouble ylo, gadouble yhi) {
  ...
  if (xlo < clminx || xhi > clmaxx || ylo < clminy || yhi > clmaxy)
    return (1);
