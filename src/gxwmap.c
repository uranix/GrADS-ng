  ...
  while (1) {
    ...
    if (i == 2) {
      ...
      if (sln1 > 360.0)
        ...
        if (slt2 <= mopt->ltmin || slt1 >= mopt->ltmax)
          flag = 0;
        else {
          lnfact = 0.0;
          while (sln2 + lnfact > mopt->lnmin)
            lnfact -= 360.0;
          lnfact += 360.0;
          if (sln1 + lnfact >= mopt->lnmax)
            flag = 0;
          else
            flag = 1;
        }
    ...
    while (lnmin + lnfact < mopt->lnmax) {
      ...
      for (i = 1; i < num; i++) {
        ...
        while (llsum < lldist + llinc) {
          ...
          if (lntmp + lnfact < mopt->lnmin || lntmp + lnfact > mopt->lnmax || lttmp < mopt->ltmin || lttmp > mopt->ltmax) {
            if (ipen == 2) {
...
int gxscld(struct mapprj *mpj, int xflip, int yflip) {
  ...
  if (mpj->lnmn >= mpj->lnmx)
    return (1);
  if (mpj->ltmn >= mpj->ltmx)
    return (1);
  if (mpj->xmn >= mpj->xmx)
    return (1);
  if (mpj->ymn >= mpj->ymx)
...
int gxltln(struct mapprj *mpj) {
  ...
  gxscal(mpj->axmn, mpj->axmx, mpj->aymn, mpj->aymx, mpj->lnmn, mpj->lnmx, mpj->ltmn, mpj->ltmx);
  gxproj(NULL);
  adjtyp = 0;
  return (0);
}
...
int gxrobi(struct mapprj *mpj) {
  ...
  if (lonmn < -180.0 || lonmx > 180.0 || latmn < -90.0 || latmx > 90.0) {
    if (dequal(lonmn, 0.0, 1e-7) || dequal(lonmx, 360.0, 1e-7))
      return (1);
    else
      fudge = 180.0;
  }
...
gadouble rob1[37] = {-1.349, -1.317, -1.267, -1.206, -1.138, -1.066, -0.991, -0.913, -0.833, -0.752, -0.669, -0.586, -0.502,
                     -0.418, -0.334, -0.251, -0.167, -0.084, 0.000,  0.084,  0.167,  0.251,  0.334,  0.418,  0.502,  0.586,
                     0.669,  0.752,  0.833,  0.913,  0.991,  1.066,  1.138,  1.206,  1.267,  1.317,  1.349};
gadouble rob2[37] = {1.399, 1.504, 1.633, 1.769, 1.889, 1.997, 2.099, 2.195, 2.281, 2.356, 2.422, 2.478, 2.532,
                     2.557, 2.582, 2.602, 2.616, 2.625, 2.628, 2.625, 2.616, 2.602, 2.582, 2.557, 2.532, 2.478,
                     2.422, 2.356, 2.281, 2.195, 2.099, 1.997, 1.889, 1.769, 1.633, 1.504, 1.399};
...
int gxortg(struct mapprj *mpj) {
  ...
  if (mpj->axmn > -999.0) {
    ...
    ylmn = mpj->aymn;
    ylmx = mpj->aymx;
    if (xlmn >= -1.0 && xlmn <= 1.0 && xlmx >= -1.0 && xlmx <= 1.0 && ylmn >= -1.0 && ylmn <= 1.0 && ylmx >= -1.0 && ylmx <= 1.0 &&
        ylmx > ylmn && xlmx > xlmn)
      lflg = 1;
  }
...
void gxlamcp(gadouble rlon, gadouble rlat, gadouble *x, gadouble *y) {
  ...
  if (phis != phin)
    cone = (log10(cos(phis * d2r)) - log10(cos(phin * d2r))) / (log10(term1) - log10(term2));
  else
...
gadouble *gxmpoly(gadouble *xy, gaint cnt, gadouble llinc, gaint *newcnt) {
  ...
  if (newxy == NULL)
    return (NULL); /* caller issues error */
...
void gxwseek(gaint pos) { mcpos = pos; }
