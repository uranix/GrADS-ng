...
gaint main(gaint argc, char *argv[]) {
  ...
  if (argc > 1) {
    ...
    while (iarg < argc) {
      ...
      if (*(ch) == '-' && *(ch + 1) == 'h' && *(ch + 2) == 'e' && *(ch + 3) == 'l' && *(ch + 4) == 'p') {
        ...
        if (i == 0)
          goto err1;
        if (maxlevels < 10 || maxlevels > 10000)
          goto err1a;
      } else if (*ch == '-' && *(ch + 1) == '0') {
        noread = 1;
        flg = 0;
  ...
  if (!pfi->tmplat) {
    ...
    fpos = 0;
    if (verb || (!quiet))
      printf("Binary data file open: %s\n", pfi->name);
  }
  while (i < pfi->dnum[3]) {
    if (verb || (!quiet))
    ...
    if (noread == 0) {
      while (1) {
        ...
        if (hdr.flag < 0 || hdr.flag > 1 || hdr.nlev < 0 || hdr.nlev > 10000 || hdr.t < -2.0 || hdr.t > 2.0) {
          ...
          if (pfi->seqflg)
            printf("                    Invalid sequential format\n");
...
gaint skstn(void) {
  ...
  if (rc != 0) {
    ...
    if (pfi->seqflg)
      printf("  Possible cause:  Invalid sequential format\n");
    if (pfi->tmplat)
      printf("  File name = %s\n", fn);
    return (1);
  }
...
gaint rdstn(char *rec, gaint siz) {
  ...
  if (rc < siz) {
    ...
    if (pfi->seqflg)
      printf("  Possible cause: Invalid sequential data\n");
    if (pfi->tmplat)
      printf("  File name = %s\n", fn);
    return (1);
  }
